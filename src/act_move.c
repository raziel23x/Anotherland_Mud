...
void move_char( CHAR_DATA *ch, int door )
{
    ...
    if(IS_CLASS(ch,CLASS_RPONLY)&&ch->level<4&& ch->in_room->vnum==3001)
    {
        do_recall(ch,"");
        send_to_char("You must remain in the RP area.\n\r",ch);
        return;
    }
    if ( ch->furn )
    {
        if ( ch->position == POS_STANDING )
            act("You're standing in $p right now.",ch,ch->furn,NULL,TO_CHAR);
        else if ( ch->position == POS_SITTING )
            act("You're sitting in $p right now.",ch,ch->furn,NULL,TO_CHAR);
        else act("You're in $p right now.",ch,ch->furn,NULL,TO_CHAR);
        return;
    }
    ...
    {
        if (!IS_NPC(ch) && IS_CLASS(ch, CLASS_WEREWOLF) &&ch->power[DISC_WERE_BOAR] > 0 && !IS_SET(pexit->exit_info, EX_PRISMATIC_WALL))
        {
    ...
    if ((IS_SET(pexit->exit_info, EX_NEWBIE_WALL) && (get_age(ch) - 17) < 5) && (!IS_IMMORTAL(ch)))
    {
        stc( "You are unable to pass due to newbie hours.\n\r",ch);
        return;
    }

    if (IS_SET(pexit->exit_info, EX_PRISMATIC_WALL) &&  IS_SET(pexit->exit_info, EX_CLOSED) && (!IS_IMMORTAL(ch)))
    {
    ...
    }
    {
        ...
        {
            if (rch->spec_fun == spec_wall_force && (ch->level <= 100) )
            {
                int isBlocked = 0;
    ...
    if ( (IS_LEG_L(ch,BROKEN_LEG) || IS_LEG_L(ch,LOST_LEG)) &&
        ...
        get_eq_char(ch, WEAR_WIELD) != NULL) && (!IS_IMMORTAL(ch)))
    {
    ...
    if ( !IS_NPC(ch) )
    {
            ...
            ||   to_room->sector_type == SECT_AIR )
        {
            if ( !IS_AFFECTED(ch, AFF_FLYING) &&
                ...
                (!IS_NPC(ch) && !IS_VAMPAFF(ch, VAM_WINGS) &&
        ...
        if (IS_SET(pexit->exit_info, EX_NOPASS) && IS_SET(pexit->exit_info, EX_CLOSED)
            && IS_AFFECTED(ch, AFF_PASS_DOOR) && (!IS_IMMORTAL(ch)))
        {
            send_to_char("You can't seem to get through it even while being translucent.\n\r",ch);
            return;
        }
        if (IS_SET(pexit->exit_info, EX_QPFILTER) && (ch->pcdata->quest < 50 ) && (!IS_IMMORTAL(ch)) )
        {
            send_to_char("You need a minimum of 50 qps to get pass this thing.\n\r",ch);
            return;
        }

        if (IS_SET(pexit->exit_info, EX_MUSHROOM_WALL) && (!IS_IMMORTAL(ch)))
        {
        ...
        if (IS_SET(pexit->exit_info, EX_ICE_WALL)
            && ch->power[DISC_DAEM_GELU] < 5 && (!IS_IMMORTAL(ch)))
        {
    ...
    switch ( to_room->vnum )
    {
        ...
        case 72022:
        case 72023:
            /* This was fucking retarded- Beorn */
            /*  if ( ch->fight_timer > 0 )
                   {
                   char buf[MAX_INPUT_LENGTH];
                   sprintf(buf,"#R%s#r tried to enter a training room with a fight timer! #R[#r+10 fight timer#R]#n",
                       ch->name);
                   do_info(ch,buf);
                   char_to_room(ch,get_room_index(72001));
        ...
        case 100907:
            ...
            {
                char buf[MAX_INPUT_LENGTH];
                sprintf(buf,"#R%s#r tried to enter a temple with a fight timer! #R[#r+10 fight timer#R]#n",
                    ch->name);
                do_info(ch,buf);
                char_to_room(ch,get_room_index(72001));
                ch->position = POS_SLEEPING;
    ...
    if ( IS_SET(to_room->room_flags,ROOM_NODECAP) && ch->fight_timer > 0 )
    {
        char buf[MAX_INPUT_LENGTH];
        sprintf(buf,"#R%s#r tried to enter a no-decap room with a fight timer! #R[#r+10 fight timer#R]#n",
            ch->name);
        do_info(ch,buf);
...
void do_north( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch) && IS_SET(ch->act, ACT_DUMMY)) return;
...
void do_west( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch) && IS_SET(ch->act, ACT_DUMMY))
    {
        send_to_char( " You are a dummy.\n\r",ch );
        return;
    }
...
void do_enter( CHAR_DATA *ch, char *argument )
{
    ...
    {
        //  send_to_char("The portal will not work in the astral plane.\n\r",ch);
        //   return;
    }
...
void do_stand( CHAR_DATA *ch, char *argument )
{
    if ( argument && *argument )
    {
        ...
        if ( ch->position == POS_FIGHTING )
            return send_to_char("You're a bit busy right now, eh?\n\r", ch);
        if ( ch->position != POS_STANDING )
            return send_to_char("Stand up first, please.\n\r",ch);
        if ( !(obj = get_obj_room(ch,argument)) )
            return send_to_char("You can't find it.\n\r",ch);
        if ( obj->item_type == ITEM_FURNITURE && obj->value[0] == 2 )
        {
            if ( obj->value[1] == 0 || obj->value[2] < obj->value[1] )
            {
                ch->furn = obj;
                obj->value[2]++;
                act("You stand in $p.",ch,obj,NULL,TO_CHAR);
                act("$n stand in $p.",ch,obj,NULL,TO_ROOM);
            }
            else
                act("$p is full right now.\n\r",ch,obj,NULL,TO_CHAR);
        }
        else send_to_char("You can't stand in that!\n\r",ch);
        return;
    }
    ...
    switch ( ch->position )
    {
        ...
        case POS_STANDING:
            if ( ch->furn )
            {
                act("You step out of $p.",ch,ch->furn,NULL,TO_CHAR);
                act("$n steps out of $p.",ch,ch->furn,NULL,TO_CHAR);
                ch->furn->value[2]--;
                ch->furn = NULL;
            } else send_to_char( "You are already standing.\n\r", ch );
            break;
    ...
    if ( ch->position == POS_STANDING && ch->furn )
        { ch->furn->value[2]--; ch->furn = NULL; }
...
void do_sit( CHAR_DATA *ch, char *argument )
{
    if ( argument && *argument )
    {
        ...
        else send_to_char("You can't sit in that!\n\r",ch);
        return;
    }
...
void do_home( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if ( ch->pcdata->kingdom == 0 && ch->in_room->vnum >= 86000 && ch->in_room->vnum <= 86099 )
        {
            send_to_char( "You cant make this your home idiot. Hey, your not even in a kingdom!\n\r",ch);
            send_to_char( "Kingdom Punt in action!\n\r",ch);
            act( "$n disappears.", ch, NULL, NULL, TO_ROOM );
            do_recall( ch, "");
            sprintf(buf, "%s, has tried unsucessfully to make a kingdom not his own his home!\n\r", ch->pcdata->switchname);
            do_info(NULL,buf);
            return;
        }
        if ( ch->pcdata->kingdom != 1 && ch->in_room->vnum >= 86090 && ch->in_room->vnum <= 86099 )
        {
            send_to_char( "You cant make this your home idiot.\n\r",ch);
            send_to_char( "Kingdom Punt in action!\n\r",ch);
            act( "$n disappears.", ch, NULL, NULL, TO_ROOM );
            do_recall( ch, "");
            sprintf(buf, "%s, has tried unsucessfully to make a kingdom not his own his home!\n\r", ch->pcdata->switchname);
        ...
        if ( ch->pcdata->kingdom != 2 && ch->in_room->vnum >= 86010 && ch->in_room->vnum <= 86019 )
        {
            send_to_char( "You cant make this your home idiot.\n\r",ch);
            send_to_char( "Kingdom Punt in action!\n\r",ch);
            act( "$n disappears.", ch, NULL, NULL, TO_ROOM );
        ...
        if ( ch->pcdata->kingdom != 3 && ch->in_room->vnum >= 86000 && ch->in_room->vnum <= 86009 )
        {
            send_to_char( "You cant make this your home idiot.\n\r",ch);
            send_to_char( "Kingdom Punt in action!\n\r",ch);
            act( "$n disappears.", ch, NULL, NULL, TO_ROOM );
        ...
        if ( ch->pcdata->kingdom != 4 && ch->in_room->vnum >= 86020 && ch->in_room->vnum <= 86029 )
        {
            send_to_char( "You cant make this your home idiot.\n\r",ch);
            send_to_char( "Kingdom Punt in action!\n\r",ch);
            act( "$n disappears.", ch, NULL, NULL, TO_ROOM );
        ...
        if ( ch->pcdata->kingdom != 5 && ch->in_room->vnum >= 86030 && ch->in_room->vnum <= 86039 )
        {
            send_to_char( "You cant make this your home idiot.\n\r",ch);
            send_to_char( "Kingdom Punt in action!\n\r",ch);
            act( "$n disappears.", ch, NULL, NULL, TO_ROOM );
...
void do_escape( CHAR_DATA *ch, char *argument )
{
    ...
    if( ch->fight_timer > 0)
    {
        send_to_char( "You can not escape with a fight timer.\n\r", ch);
        return;
    }
    ...
    if (ch->in_room != NULL)
    {
        if (IS_SET(ch->in_room->room_flags, ROOM_ARENA) || in_fortress(ch))
        {
...
void do_train( CHAR_DATA *ch, char *argument )
{
    ...
    {
        max_stat = max_stat + ((ch->pcdata->upgrade_level) * 25);
    }

    if (IS_NPC(ch)) return;
    if (IS_CLASS(ch, CLASS_RPONLY))
    {
        send_to_char("RP chars cannot use this command.\n\r",ch);
        return;
    }
    ...
    if (!str_cmp(arg1,"hp"))
    {
        ...
        if ( ch->race >= 1)
        {
            statcap = UMIN(1000000, (200000 + (4000 * ch->race) + (get_curr_con(ch) * 1000)));
        }
        if ( ch->race == 0)
        {
            statcap = UMIN(1000000, (200000 + (get_curr_con(ch) * 1000)));
        }
        /*  statcap = statcap + (ch->pcdata->mod_con * 0.05);
          if (ch->pcdata->mod_con > 0)
          {
          statcap = statcap + (ch->pcdata->mod_con * 0.05);
          }  */
        ...
        {
            if (ch->max_hit < 0 && ch->exp < UMAX(1, (-1 * (ch->max_hit + 1) ) ) )
            {
                send_to_char( "You don't have enough Exp to train your Negative stat...\n\r",ch);
                return;
            }
    ...
    if (!str_cmp(arg1,"move"))
    {
        ...
        int urin = 0;
        int urin_counter = 0;
        int statcap = UMIN(500000, 200000 + (4000 * ch->race));
        ...
        for (urin = 0; urin < amount ; urin++)
        {
            if (ch->max_move < 0 && ch->exp < UMAX(1, (-1 * (ch->max_move + 1) ) ) )
            {
                send_to_char( "You don't have enough Exp to train your Negative stat...\n\r",ch);
                return;
            }
    ...
    if (!str_cmp(arg1,"mana"))
    {
        ...
        for (urin = 0; urin < amount ; urin++)
        {
            if (ch->max_mana < 0 && ch->exp < UMAX(1, (-1 * (ch->max_mana + 1) ) ) )
            {
                send_to_char( "You don't have enough Exp to train your Negative stat...\n\r",ch);
                return;
            }
    ...
    if (!str_cmp(arg1,"slash") || !str_cmp(arg1,"stab") || !str_cmp(arg1,"smash") || !str_cmp(arg1,"beast") || !str_cmp(arg1,"grab") || !str_cmp(arg1,"charm") || !str_cmp(arg1,"heat")|| !str_cmp(arg1,"cold") || !str_cmp(arg1,"lightning") || !str_cmp(arg1,"acid") || !str_cmp(arg1,"drain") || !str_cmp(arg1,"voodoo") || !str_cmp(arg1,"hurl")|| !str_cmp(arg1,"backstab") || !str_cmp(arg1,"kick") || !str_cmp(arg1,"disarm")|| !str_cmp(arg1,"steal"))
    {
        ...
        if (IS_IMMUNE(ch,IMM_STEAL))
            immune_counter+=1;
        if (immune_counter > 15)
        {
            send_to_char("You already have 15 immunities/resistances.\n\r", ch);
    ...
    {
        if( ch->level >= 3 ) return stcf(ch,"You're already an avatar.\n\r");
        if( ch->max_hit < 2000)
        {
        ...
        ch->pcdata->last_killer = NULL;
        act( "You become an avatar!", ch, NULL, NULL, TO_CHAR );
        ch->pcdata->bounty += 5000;
    ...
    */
    ...
    else
    {
        ...
        if ( ch->max_mana      < (500000) )
        {
            sprintf( buf, "Mana		   - %d exp per point.\n\r",(ch->max_mana+1));
            send_to_char( buf, ch );
        }
        if ( ch->max_move      < (500000) )
        {
    ...
    {
        qcost =  ch->pcdata->perm_str*100*1.5;
    }
    ...
    {
        qcost = ch->pcdata->perm_int*100*1.5;
    }
    ...
    {
        qcost = ch->pcdata->perm_wis*100*1.5;
    }
    ...
    {
        qcost = ch->pcdata->perm_dex*100*1.5;
    }
    ...
    {
        qcost = ch->pcdata->perm_con*100*1.5;
    }
    ...
    if (!str_cmp(arg1, "magic"))
        ...
        ch->exp     -= cost;
    if ( qcost > ch->pcdata->quest )
    {
        if (last) stcf(ch, "You don't have enough quest points. You need [%d] more quest points to gain a stat.\n\r", qcost - ch->pcdata->quest);
        return;
    }
    else ch->pcdata->quest -= qcost;
...
void do_mount( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "You cannot mount a person!\n\r",ch );
        return;
    }
    if( victim->mounted > 0 )
    {
    ...
    {
        send_to_char( "You cannot mount a nonmountable creature.\n\r",ch );
        return;
    }
    if ( IS_NPC(ch) && (victim->level > ch->level) )
    {
        send_to_char( "They are too strong for you.\n\r",ch);
        return;
    }
    if ( !IS_NPC(ch) && ( victim->level > ( getMight(ch) * 4 ) ) )
    {
        stc( "You get punched in the gut trying to mount the beast\n\r",ch);
        ch->hit -= 100+number_range(1,100);
        return;
    }
...
void do_tie( CHAR_DATA *ch, char *argument )
{
    ...
    /*if (getMight(victim) < 100 )
    {
    send_to_char( "You can't hurt a person who is new.\n\r",ch );
    return;
    }*/
    if ((victim->position > POS_STUNNED || victim->hit > 0) && (!IS_IMMORTAL(ch)))
    {
    ...
    else sprintf(buf,"#P%s #yhas been tied up by #R%s#n",victim->name,ch->name);
    do_info(ch,buf);

    /*Magna - Log Ties*/
    sprintf(buf, "%s tied up by %s at %d.", victim->name, ch->name, ch->in_room->vnum);
...
void do_gag( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(victim) ) return stcf(ch,"A Mob? Right...\n\r");
    if (victim == ch && !IS_EXTRA(victim,GAGGED) && IS_EXTRA(victim,TIED_UP))
    {
        send_to_char( "You cannot gag yourself!\n\r", ch );
        return;
    }
    if ((!IS_EXTRA(victim,TIED_UP) && !IS_EXTRA(victim,GAGGED)) && (!IS_IMMORTAL(ch)))
    {
...
void do_blindfold( CHAR_DATA *ch, char *argument )
{
    ...
    if ((!IS_EXTRA(victim,TIED_UP) && !IS_EXTRA(victim,BLINDFOLDED)) && (!IS_IMMORTAL(ch)))
    {
