...
void do_martyr( CHAR_DATA *ch, char *argument )
{
    ...
    for (ich = ch->in_room->people; ich != NULL; ich = ich->next_in_room)
        if (!IS_NPC(ich) && ich->fight_timer == 0 ) do_restore(ch,ich->pcdata->switchname);
    ch->level = 3;
    ch->hit = 1;
    ch->move = 1;
    ch->mana = 1;
...
void do_angelfly( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
...
void do_angelicarmor( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if      (!str_cmp(arg,"sword"               )) vnum = 110200;
        else if (!str_cmp(arg,"ring"                )) vnum = 110201;
        else if (!str_cmp(arg,"necklace"            )) vnum = 110202;
        else if (!str_cmp(arg,"bracer"              )) vnum = 110203;
        else if (!str_cmp(arg,"plate"               )) vnum = 110204;
        else if (!str_cmp(arg,"helmet"              )) vnum = 110205;
        else if (!str_cmp(arg,"leggings"            )) vnum = 110206;
        else if (!str_cmp(arg,"boots"               )) vnum = 110207;
        else if (!str_cmp(arg,"gauntlets"           )) vnum = 110208;
        else if (!str_cmp(arg,"sleeves"             )) vnum = 110209;
        else if (!str_cmp(arg,"cloak"               )) vnum = 110210;
        else if (!str_cmp(arg,"belt"                )) vnum = 110211;
        else if (!str_cmp(arg,"visor"               )) vnum = 110212;
        else
        {
            do_angelicarmor(ch,"");
            return;
