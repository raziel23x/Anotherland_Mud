...
#define ARENA_HVNUM     70                                  // upper vnum for the arena
...
void do_arenastats(CHAR_DATA *ch, char *argument)
{
    ...
    if (!arenaopen) return stcf(ch,"#RT#rhe #Ra#rrena is not #Ro#rpen#R.#n\n\r");
...
void close_arena()
{
    ...
    if (arenaplayers <= 1)
    {
        ...
        do_info(NULL, "The Arena fight was cancelled because there's too many pussies!");
        return;
    }
...
void do_arenajoin(CHAR_DATA *ch, char *argument)
{
    ...
    if (getMight(ch) < 100 )
    {
        stc("You don't got enough mightscore.\n\r",ch);
        return;
    }
    ...
    if ( ((get_age(ch) - 17) < 5) )
    {
        send_to_char("You must have 5 hours to join the arena.\n\r",ch);
        return;
    }
    if (!arenaopen)
    {
    ...
    {
        send_to_char("The arena is crowded at the moment.\n\r",ch);
        return;
    }
    char_from_room(ch);
...
void do_resign(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->in_room != NULL)
    {
        if (!IS_SET(ch->in_room->room_flags, ROOM_ARENA))
        {
            send_to_char("You're not in the arena.\n\r",ch);
            return;
        }
    }
...
/*void do_oldresign(CHAR_DATA *ch, char *argument)
{
  CHAR_DATA *victim;
  CHAR_DATA *gch;
  int found = 0;
...
void do_openarena(CHAR_DATA * ch, char *argument)
{
    if (IS_NPC(ch))
        return;
    if (arenaopen)
        return;
    arenaopen = TRUE;
    next_arena_room = ARENA_LVNUM;
    arena_base = FALSE;
    do_info(NULL,"The arena is now open for EVERYONE '#Larenajoin#n'");
    return;
}

void do_2openarena( CHAR_DATA *ch, char *argument )
{
    bool base;

    if (arenaopen && str_cmp(argument,"close") ) return stcf(ch,"The arena is already open!\n\r");

    if      ( !str_cmp(argument,"base") ) base = TRUE;
    else if ( !str_cmp(argument,"all") )  base = FALSE;
    else if ( !str_cmp(argument,"close" ) )
    {
        close_arena();
        return;
    }
    else return stcf(ch,"Open the arena for #Lbase#n classes or #Lall#n classes?\n\r");
