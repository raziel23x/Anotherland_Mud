...
#define L_SUP (MAX_LEVEL)                                   /* if not already defined */
...
BOARD_DATA boards[MAX_BOARD] =
{
    ...
    { "Roleplay",   "Role Playing",              0, 2,     "all", DEF_INCLUDE, 14, NULL, FALSE},
...
bool is_note_to (CHAR_DATA *ch, NOTE_DATA *note)
{
    ...
    if( ((ch->pcdata->kingdom != 0) && is_full_name(kingdom_table[ch->pcdata->kingdom].name, note->to_list)) || IS_IMMORTAL(ch) )
    {
        return TRUE;
    }
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->pcdata->in_progress && (!ch->pcdata->in_progress->text))
    {
        ...
        sprintf(buf2," %s is Writing a note.\n\r",ch->name);
        do_info(NULL,buf2);
    }
...
static void do_nremove (CHAR_DATA *ch, char *argument)
{
    ...
    if (str_cmp(ch->pcdata->switchname,p->sender) && ch->trust < MAX_LEVEL - 1)
    {
...
void do_note (CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char( " You have lost your senses.\n\r",ch );
        return;
    }
...
void do_board (CHAR_DATA *ch, char *argument)
{
    ...
    if (!argument[0])                                       /* show boards */
    {
        ...
        count = 1;
        send_to_char("#7Num #0Name        #0New#R/#0All  #7Description#n\n\r", ch);
        send_to_char("#0=== ========== ========= ===========#n\n\r", ch);
        for (i = 0; i < MAX_BOARD; i++)
        {
