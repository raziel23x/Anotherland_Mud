...
void do_level( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(skill,"#RSlice  :#7 %-4d#R  Stab : #7%-4d#R  Slash:#7 %-4d#R  Pierce:#7 %-4d #n\n\r",
        ch->wpn[1], ch->wpn[2], ch->wpn[3], ch->wpn[11]);
    send_to_char( skill, ch );
    sprintf(skill,"#RClaw   :#7 %-4d#R  Pound: #7%-4d#R  Crush:#7 %-4d#R  Bite  :#7 %-4d #n\n\r",
        ch->wpn[5], ch->wpn[7], ch->wpn[8], ch->wpn[10]);
    send_to_char( skill, ch );
    sprintf(skill,"#RWhip   :#7 %-4d#R  Blast: #7%-4d#R  Grep :#7 %-4d#R  Suck  :#7 %-4d\n\r",
        ch->wpn[4], ch->wpn[6], ch->wpn[9], ch->wpn[12]);
    send_to_char( skill, ch );
    sprintf(skill,"#RUnarmed:#7 %-4d#R #n\n\r", ch->wpn[0]);
    send_to_char( skill, ch );
    ...
    send_to_char("\n#7Stance Levels#n\n\r",ch);
    if ( !IS_SET(ch->act, PLR_BRIEF) )
        send_to_char("#y_.:*~*:._.:*~*:._.:*~*:._.:*~*:._.:*~*:..:*~*:._.:*~*:._.:*~*:._.:*~*:._.:*~*:._#n\n\r",ch);
    sprintf(skill,"#yViper   :#G %-4d  ", ch->stance[1]);
    send_to_char( skill, ch );
    sprintf(skill,"#yCrane   :#G %-4d  ", ch->stance[2]);
    send_to_char( skill, ch );
    sprintf(skill,"#yCrab    :#G %-4d  ", ch->stance[3]);
    send_to_char( skill, ch );
    sprintf(skill,"#yMongoose:#G %-4d  ", ch->stance[4]);
    send_to_char( skill, ch );
    sprintf(skill,"#yBull    :#G %-4d \n\r", ch->stance[5]);
    send_to_char( skill, ch );
    sprintf(skill,"#yMantis  :#G %-4d  ", ch->stance[6]);
    send_to_char( skill, ch );
    sprintf(skill,"#yDragon  :#G %-4d  ", ch->stance[7]);
    send_to_char( skill, ch );
    sprintf(skill,"#yTiger   :#G %-4d  ", ch->stance[8]);
    send_to_char( skill, ch );
    sprintf(skill,"#yMonkey  :#G %-4d  ", ch->stance[9]);
    send_to_char( skill, ch );
    sprintf(skill,"#ySwallow :#G %-4d  \n\r", ch->stance[10]);
    send_to_char( skill, ch );
    send_to_char("#0~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",ch);
    send_to_char("\n\r",ch);
    ...
    sprintf(skill,"#l  SS1 : #P%-4d", ch->stance[13]);
    send_to_char( skill, ch );
    sprintf(skill,"#g  SS2 : #P%-4d", ch->stance[14]);
    send_to_char( skill, ch );
    sprintf(skill,"#l  SS3 : #P%-4d", ch->stance[15]);
    send_to_char( skill, ch );
    sprintf(skill,"#g  SS4 : #P%-4d", ch->stance[16]);
    send_to_char( skill, ch );
    sprintf(skill,"#l  SS5 : #P%-4d\n\r", ch->stance[17]);
    send_to_char( skill, ch );
    send_to_char("#0~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~",ch);
    send_to_char("\n\r",ch);
    ...
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#5Purple#n : #7%-4d", ch->spl[0] + 75);
    else sprintf(skill,"#5Purple#n: #7%-4d", ch->spl[0]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#4Blue#n   : #7%-4d", ch->spl[2] + 75);
    else sprintf(skill,"#4Blue#n: #7%-4d", ch->spl[2]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#1Red#n    : #7%-4d", ch->spl[1] + 75);
    else sprintf(skill,"#1Red#n: #7%-4d", ch->spl[1]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#2Green#n  : #7%-4d", ch->spl[3] + 75);
    else sprintf(skill,"#2Green#n: #7%-4d", ch->spl[3]);
    send_to_char( skill, ch );
    if (IS_ITEMAFF(ch, ITEMA_AFFENTROPY)) sprintf(skill,"#3Yellow#n : #7%-4d\r\n", ch->spl[4] + 75);
    else sprintf(skill, "#3Yellow#n: #7%-4d\n", ch->spl[4]);
    send_to_char( skill, ch );
    ...
    if( IS_CLASS(ch, CLASS_PHOENIX))
        stc( "You are a Phoenix.\n\r",ch);
    if( IS_CLASS(ch, CLASS_WRAITH))
        stc( "You are a hideous wraith!\n\r",ch);
    ...
    if( IS_CLASS(ch, CLASS_ELF))
    {
        if (IS_SET(ch->special,SPC_ELF_MAGE))
            stc( "You are an Elven Mage.\n\r",ch);
        if (IS_SET(ch->special,SPC_ELF_WARRIOR))
            stc( "You are an Elven Warrior.\n\r",ch);
        if (IS_SET(ch->special,SPC_ELF_ARCHER))
            stc( "You are an Elven Archer.\n\r",ch);
    }
    if (IS_CLASS(ch, CLASS_TANARRI))
    {
...
void do_melt( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "But they are not covered with ice!\n\r", ch );
        return;
    }

    if ( number_percent() > (ch->level*10) )
    {
        act( "You try to melt the ice around $N but fail!",  ch, NULL, victim, TO_CHAR    );
        act( "$n tries to melt the ice around you but fails!", ch, NULL, victim, TO_VICT    );
        act( "$n tries to melt the ice around $N but fails!",  ch, NULL, victim, TO_NOTVICT );

        if ( number_percent() > 98 && !IS_AFFECTED(ch,AFF_FREEZING) )
        {
            act( "A small blue ball from $N's body sets you on fire!",  ch, NULL, victim, TO_CHAR    );
            act( "A small blue ball from your body sets $n on fire!", ch, NULL, victim, TO_VICT    );
            act( "A small blue ball from $N's body sets $n on fire!",  ch, NULL, victim, TO_NOTVICT );
            SET_BIT(ch->affected_by, AFF_FREEZING);
            do_humanity(ch,"");
        }
        return;
    }

    act( "You manage to melt the ice around $M!",  ch, NULL, victim, TO_CHAR    );
    act( "$n manages to melt the ice around you!", ch, NULL, victim, TO_VICT    );
    act( "$n manages to melt the ice around $N!",  ch, NULL, victim, TO_NOTVICT );
    REMOVE_BIT(victim->affected_by, AFF_FREEZING);
    do_humanity(ch,"");
    return;
}
...
void do_nightsight( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if (ch->pcdata->classthree[KAHN_TIGER] < 1)
        {
            stc("You must learn your tiger to level 1 first.\n\r",ch);
            return;
        }
    }
    else if (!IS_CLASS(ch, CLASS_VAMPIRE) )
...
void do_shadowsight( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_CLASS(ch, CLASS_ELF) && IS_SET(ch->special,SPC_ELF_ARCHER) && ch->pcdata->classtwo[ELF_ARC_ARCPOWER] < 2)
    {
        send_to_char("You have not learned your archer powers to level 2 yet.\n\r",ch);
        return;
    }
...
void do_class( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
        ...
        send_to_char("None, Demon, Drow, Werewolf, Monk, Vampire, Mage, Ninja, Kahn, Dragon, Rponly\n\r", ch);
        send_to_char("Tanarri, Shapeshifter, Spiderdroid, Samurai, Undeadknight, Angel, Lich, Elf, Doomtrooper\n\r",ch);
        send_to_char("#rTesting: Mecha#n\n\r", ch );
        return;
    }
    ...
    if ( !str_cmp( arg2, "doomtrooper"      ) )
    {
        OBJ_INDEX_DATA *pObjIndex;
        OBJ_DATA *obj;
        int vnum = 0;
        victim->class = CLASS_MILITARY;
        victim->pcdata->Bootcamp[5]=28;
    ...
    else if (!str_cmp(arg2,"rponly"))
    {
        victim->class = CLASS_RPONLY;
        if(victim->level<4)victim->home=100906;
        do_recall(victim,"");
        victim->hit=10;
        victim->max_hit=10;
        victim->mana=10;
        ...
        victim->max_move=10;
        victim->move=10;
        send_to_char("You can now only RP.\n\r",victim);
    }
    ...
    else if (!str_cmp(arg2,"mecha"))
    {
        ...
        send_to_char("You are now a mecha.\n\r",victim);
    }
    ...
    else if (!str_cmp(arg2,"salamander"))
    {
        victim->class = CLASS_SALAMANDER;
        send_to_char("You are now a Salamander.\n\r",victim);
    }
    else if (!str_cmp(arg2,"kahn"))
    {
        victim->class = CLASS_KAHN;
        send_to_char("You are now a Kahn.\n\r",victim);
    }
    else if (!str_cmp(arg2,"elf"))
    {
        victim->class = CLASS_ELF;
        send_to_char("You are now an Elf.\n\r",victim);
    }
    else if (!str_cmp(arg2,"runemaster"))
    {
        victim->class = CLASS_RUNEMASTER;
        send_to_char("You are now a Rune Master.\n\r",victim);
    }
    else if (!str_cmp(arg2,"wizard"))
    {
        victim->class = CLASS_WIZARD;
        send_to_char("You are now a High Wizard.\n\r",victim);
    }
...
void do_scry( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_ITEMAFF(ch, ITEMA_VISION) && !ragnarok)
    {
        if (!IS_CLASS(ch, CLASS_VAMPIRE) && !IS_CLASS(ch, CLASS_MONK)
            && !IS_CLASS(ch, CLASS_DEMON) && !IS_CLASS(ch, CLASS_KAHN)
            && !IS_CLASS(ch, CLASS_MAGE) && !IS_CLASS(ch,CLASS_DROID) && !IS_CLASS(ch, CLASS_DRAGON) )
        {
            stc("Huh?\n\r",ch);
            return;
        }
    ...
    if ( !IS_ITEMAFF(ch, ITEMA_VISION) && !ragnarok)
    {
        ...
        if (IS_CLASS(ch, CLASS_DRAGON) && (ch->pcdata->classfour[DRAGON_POWER] < 4))
        {
            send_to_char("You must learn dragon powers to level 4.\n\r", ch);
            return;
        }
        ...
        if (IS_CLASS(ch, CLASS_KAHN) && ch->pcdata->classthree[KAHN_RANKS] < 3)
        {
            send_to_char("You need you learn your rank to level 3 to use scry.\n\r",ch);
            return;
        }
        if ( ch->in_room->vnum >= 86000 && ch->in_room->vnum <= 86099)
        {
            send_to_char( "No scry in Kingdom!\n\r",ch);
            WAIT_STATE(ch,24);
            return;
        }
    ...
    if ( IS_CLASS(victim, CLASS_VAMPIRE) && IS_SET(victim->flag2, AFF_SPIRITGUARD) && !IS_NPC(ch) )
    {
...
void do_readaura( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_CLASS(ch, CLASS_KAHN) && ch->pcdata->classthree[KAHN_RANKS] < 3)
    {
        send_to_char("You need you learn your rank to level 3 to use readaura.\n\r",ch);
        return;
    }
...
void do_shield( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_CLASS(ch, CLASS_VAMPIRE) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_WEREWOLF) &&
        ...
        !IS_CLASS(ch, CLASS_DROID) && !IS_CLASS(ch, CLASS_ELF))
    {
        send_to_char("Huh?\n\r",ch);
        return;
    }
    ...
    if (IS_CLASS(ch, CLASS_ELF) && ch->pcdata->classtwo[ELF_GEN] < 4 )
    {
        stc("You must learn your basic elven powers to level 4.\n\r",ch);
        return;
    }
...
void do_werewolf( CHAR_DATA *ch, char *argument )
{
    ...
    /*  Edited out to allow WW's to weapon train - Magna
        if ((obj = get_eq_char(ch,WEAR_WIELD)) != NULL && (!IS_SET(obj->spectype,
    SITEM_WOLFWEAPON) || obj->pIndexData->vnum ==
    OBJ_VNUM_PROTOPLASM))
        {
...
void do_vanish( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->move < 5000 ) return stcf(ch,"You don't have the 5,000 move points to use vanish.\n\r");
    if ( IS_SET(ch->act, PLR_SUPERINVIS) )
    {
        REMOVE_BIT(ch->act, PLR_SUPERINVIS);
...
void do_calm( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_CLASS(ch, CLASS_PHOENIX)&& !IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_DEMON) && !IS_CLASS(ch, CLASS_NINJA))
    {
        send_to_char("Huh?\n\r",ch);
        return;
    }
    if (IS_CLASS(ch, CLASS_PHOENIX))
    {
        send_to_char("You bathe in the sun and calm yourself.\n\r",ch);
        act("$n bathes in the sun and tries to calm $mself.",ch,NULL,NULL,TO_ROOM);
        ch->rage = 0;
    }
...
void do_web( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_CLASS(ch, CLASS_WEREWOLF) && !IS_CLASS(ch, CLASS_DROW) && !IS_CLASS(ch, CLASS_DROID)
        && !IS_CLASS(ch, CLASS_TANARRI) && !IS_CLASS(ch, CLASS_SAMURAI) && !IS_CLASS(ch, CLASS_ELF)
        && !IS_CLASS(ch, CLASS_MILITARY))
    {
        send_to_char("Huh?\n\r",ch);
        return;
    }
    ...
    if (IS_CLASS(ch, CLASS_MILITARY) && ch->pcdata->Bootcamp[0]<3)
    {
        send_to_char("You don't your weapons to level 4 yet.\n\r",ch);
        return;
    }
    ...
    if (IS_CLASS(ch, CLASS_ELF) && IS_SET(ch->special,SPC_ELF_WARRIOR) && ch->pcdata->classtwo[ELF_WAR_TECH] < 1)
    {
        send_to_char("You have not learned your technique to level 1 yet.\n\r",ch);
        return;
    }

    if (IS_CLASS(ch, CLASS_ELF) && IS_SET(ch->special,SPC_ELF_ARCHER) && ch->pcdata->classtwo[ELF_ARC_ARCPOWER] < 1)
    {
        send_to_char("You have not learned your archer powers to level 1 yet.\n\r",ch);
        return;
    }
...
void do_mitsukeru( CHAR_DATA *ch, char *argument )
{
    ...
    chroom = ch->in_room;
    victimroom = victim->in_room;
    // Fucking ninjas are supposed to ignore this shit - Beorn
    /*   if (!IS_NPC(victim) && IS_IMMUNE(victim,IMM_SHIELDED) && !ragnarok)
       {
          send_to_char( "You cannot find them.\n\r",ch);
          return;
