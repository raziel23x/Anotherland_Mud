...
/*  char global_chat[MAX_INPUT_LENGTH]; */
/*End Additions by Maar*/
...
int main( int argc, char **argv )
{
    ...
    arena = FIGHT_OPEN;
    sprintf( log_buf, "#rAnother #lLand#7 is ready to rock on port #R%d#n.", port );
...
{
    int iStart;
        int iBegin;                                         // for queue commands by Devik
    ...
    for ( ; ; )
    {
        ...
        else if ( nRead == 0 )
        {
            log_string( "EOF encountered on read." );
                wiznet( NULL, WIZ_LINKS, 0, "EOF encountered on read." );
                return FALSE;
        }
        #if !defined(WIN32)
    ...
    {
        // List all spammed commands
        if ( !memcmp(d->inbuf+iBegin,"query",5) )
        {
            d->inbuf[iBegin] = '\0';
                stcf(d->character,"Buffered commands:\n\r%s\n\r",d->inbuf);
        }
        // Clear all spammed commands
        else if ( !memcmp(d->inbuf+iBegin,"nukebuf",7) )
        {
            d->inbuf[0] = '\0';
                stcf(d->character,"#RYou have nuked all buffered commands.#n\n\r");
        }
    }
        ...
        || d->connected == CON_PFILE))
    {
            ...
            else if ( IS_SET(ch->act, PLR_PROMPT) )
        {
            ...
            else if (ch->position == POS_FIGHTING)
            {
                ...
                {
                    sprintf(cond," %-3d%%",victim->hit*100/victim->max_hit);
                ...
                /*		else if ( victim->max_hit == 0 )
                        {
                            sprintf(cond,"-----");
                            ADD_COLOUR(ch, cond, D_RED);
                        }
                        else if (((victim->hit*100/victim->max_hit) < 25) && victim->max_hit > 0 )
                        {
                            sprintf(cond, "|----");
                            ADD_COLOUR(ch, cond, L_RED);
                        }
                        else if (((victim->hit*100/victim->max_hit) < 50) && victim->max_hit > 0 )
                        {
                            sprintf(cond, "||---");
                            ADD_COLOUR(ch, cond, L_BLUE);
                        }
                        else if (((victim->hit*100/victim->max_hit) < 75) && victim->max_hit > 0 )
                        {
                            sprintf(cond, "|||--");
                            ADD_COLOUR(ch, cond, L_GREEN);
                        }
                        else if (((victim->hit*100/victim->max_hit) < 100) && victim->max_hit > 0 )
                        {
                            sprintf(cond, "||||-");
                            ADD_COLOUR(ch, cond, YELLOW);
                        }
...
void panic_shutdown(int iSignal)
{
    ...
    /* this message will be send to all players */
        ...
        for (d = descriptor_list; d; d = d->next)
    {
        if ((gch = d->character) == NULL) continue;
            do_call(gch,"all");                             /* make sure they have all their items */
            do_save(gch,"");
            save_char_obj(gch);
        /* if theres time, we should try and save the
        players */
    }
        ...
        if (txt[i] == '#' && txt[i+1] != 'I' && txt[i+1] != 'N')
        {
                ...
                switch (txt[i])
            {
                default : break;
                    ...
                    case '=': tb[j++] = 7; break;
                    case 'B':
                case 'b': tb[j++] = 27; tb[j++] = '['; tb[j++] = '7'; tb[j++] = 'm'; break;
                    case 'a': tb[j++] = 27; tb[j++] = '['; tb[j++] = '5'; tb[j++] ='m'; break;
                    case 'u': tb[j++] = 27; tb[j++] = '['; tb[j++] = '4'; tb[j++] ='m'; break;
                    case 'i': tb[j++] = 27; tb[j++] = '['; tb[j++] = '6'; tb[j++] ='m'; break;
...
void nanny( DESCRIPTOR_DATA *d, char *argument )
{
        ...
        switch ( d->connected )
    {
        ...
        default:
            ...
            if (check_banned(d))
            {
                write_to_buffer( d, " Hope you had a good time here, you're banned.\n\r", 0 );
                    close_socket( d );
                    return;
            }
            ...
            {
                write_to_buffer( d, " Illegal name please try another.\n\r Enter thy name who dares enter: ", 0 );
                    return;
            }

            sprintf(kav,"%s trying to connect.", argument);
                log_string( kav );
                wiznet( NULL, WIZ_MISC, 0, kav );
            ...
            else
            {
                ...
                if ( newlock )
                {
                    write_to_buffer( d, " Currenly newbie locked ( Probably because some lamer is trying to spam us ).\n\r", 0 );
                        close_socket( d );
                        return;
                }
            ...
            strtime = ctime( &current_time );
                ...
                wiznet( NULL, WIZ_LINKS, 0, log_buf );
                log_string( log_buf );
                sprintf( log_buf, "%s has %d QP|%d Stores upon login.", ch->name, ch->pcdata->quest, ch->pcdata->store);
                wiznet( NULL, WIZ_MISC, 0, log_buf );
                log_string( log_buf );

            /*If their in the fortress on login, send them to recall.*/
            ...
            do_board(ch,"");
                players_logged++;
                load_kingdoms();
                kingdom_table[7].deaths++;
                if ( kingdom_table[7].deaths == 1001 || kingdom_table[7].deaths ==
                10001 || kingdom_table[7].deaths == 100001 )
            {
                sprintf( buf2, "%s has won %d QPS for being the %dth PLAYER!!!\n\r",ch->name, kingdom_table[7].deaths,kingdom_table[7].deaths );
                    do_info(NULL,buf2);
                    ch->pcdata->quest += kingdom_table[7].deaths;
            }
            save_kingdoms();
                if ( ( ch->level >= 8 ) || ( ch->trust >= 8 ) )
            {
                if (    /*str_cmp(ch->name,"Joshua"		) &&*/ str_cmp(ch->name,"Nibbles"    ) &&
                    str_cmp(ch->name,"Marisella") && str_cmp(ch->name,"Alesyira"  ) &&
                    /*str_cmp(ch->name,"Mavrick"	) &&*/ str_cmp(ch->name,"Raziel"     ) &&
                    str_cmp(ch->name,"Chrono"   ) && str_cmp(ch->name,"Reaver"    ) &&
                    str_cmp(ch->name,"Ghant"    ) && str_cmp(ch->name,"Silk"      ) &&
                    str_cmp(ch->name,"Talyn"    ) && str_cmp(ch->name,"Hughes"    ) &&
                    str_cmp(ch->name,"Kabal"    ) && str_cmp(ch->name,"Mavrick"   ) &&
                    str_cmp(ch->name,"Serena"   ) && str_cmp(ch->name,"Tracey"    ) &&
                    str_cmp(ch->name,"Eclipse"  ) && str_cmp(ch->name,"Dertaherup"))

                {
                    ch->level = 3;
                        ch->trust = 3;
                        send_to_char( " You have been set back for having an illegal level. Get registered with a coder or Admin.\n\r",ch );
                }
            }
            ...
            {
                if (!ragnarok) sprintf(buf,"#2%s #7enters #R%s.#n", ch->name, MUDNAME);
                    else sprintf(buf,"#2%s #7enters #R%s #y(#0Ragnarok#y).#n", ch->name, MUDNAME);
                    enter_info(ch,buf);
            }
            ...
            ch->fight_timer = 0;
                ...
                do_mudannounce(ch,"list");
                update_wizlist(ch,ch->level);
                i = num_changes();
                if(i > 0)
            {
                sprintf(buf, "#R+++ #7There were #R#f%d #n#7changes made to the mud today type #GCHANGES#7 to view #R+++#n\n\r", i);
                    stc(buf, ch);
            }
            ...
            if((iCount = get_item_count(ch)) > 200)
            {
                sprintf(buf, "ITEM LIMIT BROKEN: %s has %d items on login.", ch->name, iCount);
                    log_string(buf);
                    send_to_char("#RWARNING:#L You have over 200 items.  Due to memory constraints, we must ask that you destroy useless items until you are under this limit.", ch);
            }
        }
        ...
        ch->embraced=NULL;
            ch->embracing=NULL;
            if ((multiCh = mmulticheck(ch)) != NULL)
        {
            send_to_char("\n\r#R====#0[#y**#0]#R====  #GWARNING  #R====#0[#y**#0]#R====#n\n\r",ch);
                ...
                if (( ch->level < 10 ) && (multiCh->level < 10))
            {
                sprintf( buf, "#rThese people are multiplaying: #R%s and %s#r. IP: #R%s#n", ch->name, multiCh->name, d->character->lasthost );
                    make_note( "Immortal", ch->name, "imm", "Multiplay Watch", 5, buf );
                //	     if ( !IS_SET(ch->act, PLR_DENY) )  SET_BIT(ch->act, PLR_DENY);
                //	     do_quit(ch,"");
            }
        }
        break;
            ...
            case CON_READ_MOTD:
            write_to_buffer( d,     "\n\r#3**#6Welcome to #RA#rn#RO#rt#RH#re#RR #rL#Ra#rN#Rd #3**\n\r#3**#6Where #1Death#6 is a part of #7Life#3**\n\r", 0
                );
                ch->next    = char_list;
                char_list   = ch;
                d->connected    = CON_PLAYING;
            ...
            do_board(ch,"");
                ...
                SET_BIT(ch->act, PLR_BRIEF5);
                SET_BIT(ch->act, PLR_BRIEF6);
            ...
            //	SET_BIT(ch->act, PLR_LOG);
                ...
                clear_stats(ch);
                do_wear(ch, "all");
                new = number_range(1,10);
                if (new == 1 )
            {
                sprintf(newbie,"Please remember to type help changes for a list of changes done. Have a good time %s.\n\r",ch->name);
            }
            if (new == 2 )
            {
                sprintf(newbie,"We have added a few new help files %s. Help changes lists them under Modified help files.\n\r",ch->name);
            }
            if (new == 3 )
            {
                sprintf(newbie,"Welcome to the realm of %s. Any questions you may have, please post them or talk to a immortal.\n\r",MUDNAME );
            }
            if (new == 4 )
            {
                sprintf(newbie,"If you have any questions %s, please feel free to ask the friendly staff and players.\n\r",ch->name);
            }
            if (new == 5 )
            {
                sprintf(newbie,"Welcome to this dystopian mud %s. Please type help changes for a list of changes.\n\r",ch->name);
            }
            if (new == 6 )
            {
                sprintf(newbie,"New to the world of dystopia? Walk North. A old timer? Help changes. Pleasant day to you %s.\n\r",ch->name);
            }
            if (new == 7 )
            {
                sprintf(newbie,"Many new things have changed since the opening of this mud. Help changes %s and you will see what has changed.\n\r",ch->name);
            }
            if (new == 8 )
            {
                sprintf(newbie,"You here to have a good time %s? Well, come on board and play with us.\n\r",ch->name);
            }
            if (new == 9 )
            {
                sprintf(newbie,"Hey hey, Welcome there %s. Come join in and see a new way of playing.\n\r",ch->name);
            }
            if (new == 10 )
            {
                sprintf(newbie,"May your stay be a delightful one %s.\n\r",ch->name);
            }
            ...
            if (mmulticheck(ch))
        {
            ...
            {
                if ( !IS_SET(ch->act, PLR_DENY) ) SET_BIT(ch->act, PLR_DENY);
                    sprintf( buf, "#rI have denied #R%s#r. Reason: Multiplaying.#n", ch->name );
...
bool check_parse_name( char *name )
{
    ...
    if ( is_name( name, "all wtf auto immortal self someone gaia none save quit why who noone" ) )
        ...
        else if (is_contained("fuck",name) || is_contained("self",name) || is_contained("bitch",name) || is_contained("whore",name)) return FALSE;
...
void send_to_char( const char *txt, CHAR_DATA *ch )
{
    ...
    strcpy(buf,txt);
        if ( !str_cmp(buf,"Huh?\n\r") )
        strcpy(buf,random_huh());
        if ( IS_SET(ch->extra,EXTRA_FRYBALLS) )
        randcolor(buf);
...
void act( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type )
{
    ...
    static char * const his_hers [] = { "its", "his", "hers" };

    char frybuf[MAX_STRING_LENGTH];
    ...
    for ( ; to != NULL; to = to->next_in_room )
    {
            ...
            else
            {
                switch ( *str )
                {
                    ...
                    /* Thx alex for 't' idea */
                        ...
                        case 'u': i = his_hers[URANGE(0, ch  ->sex, 2)];    break;
                        case 'U': i = his_hers[URANGE(0, vch ->sex, 2)];    break;
        ...
        *point++ = '\n';
        ...
        buf[0]   = UPPER(buf[0]);
            strcpy(frybuf,buf);
            if ( !IS_NPC(to) && IS_SET(to->extra,EXTRA_FRYBALLS) )
            randcolor(frybuf);
            if (to->desc && (to->desc->connected == CON_PLAYING))
            //	write_to_buffer( to->desc, frybuf, point - frybuf );
            write_to_buffer( to->desc, frybuf, strlen(frybuf) );
...
void kavitem( const char *format, CHAR_DATA *ch, const void *arg1, const void *arg2, int type )
{
     ...
     * Discard null and zero-length messages.
     */
        if ( format == NULL || format[0] == '\0' || !str_cmp(format,"normal") || !str_cmp(format,"Normal"))
        return;
        ...
        while( *str != '\0' )
    {
            ...
            switch( *str )
        {
            default :
                ...
                {
                    sprintf(buf2," %-3d%%",victim->hit*100/victim->max_hit);
                        if ( (victim->hit*100/victim->max_hit) < 25)
                    ...
                    /*               if ((victim->hit*100/victim->max_hit) < 25)
                                   {sprintf(buf2, "|----");ADD_COLOUR(ch, buf2, L_RED);}
                                   else if ((victim->hit*100/victim->max_hit) < 50)
                                   {sprintf(buf2, "||---");ADD_COLOUR(ch, buf2, L_BLUE);}
                                   else if ((victim->hit*100/victim->max_hit) < 75)
                                   {sprintf(buf2, "|||--");ADD_COLOUR(ch, buf2, L_GREEN);}
                                   else if ((victim->hit*100/victim->max_hit) < 100)
                                   {sprintf(buf2, "||||-");ADD_COLOUR(ch, buf2, YELLOW);}
                                   else if ((victim->hit*100/victim->max_hit) >= 100)
                                   {sprintf(buf2, "|||||");ADD_COLOUR(ch, buf2, L_CYAN);}*/
                }
                i = buf2; break;
                case 'F' :
                if ( ( victim = ch->fighting ) == NULL )
                ...
                {
                    sprintf(buf2," %-3d%%",tank->hit*100/tank->max_hit);
