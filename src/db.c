...
KILL_DATA       kill_table  [MAX_KILLLEVEL];
OBJ_DATA *      object_list;
TIME_INFO_DATA      time_info;
WEATHER_DATA        weather_info;
sh_int          gsn_stuntubes;
sh_int          gsn_laserbeam;
...
sh_int          gsn_ancestorswill;
sh_int          gsn_swordslash;
sh_int          gsn_wishsong;
sh_int          gsn_firearrow;
sh_int          gsn_icearrow;
sh_int          gsn_lightningarrow;
sh_int          gsn_elvenmagic;
sh_int          gsn_elfstones;
sh_int          gsn_equilibrium;
sh_int          gsn_clawslash;
sh_int          gsn_dragondive;
sh_int          gsn_firebreath;
sh_int          gsn_fireblast;
sh_int          gsn_icebreath;
sh_int          gsn_iceblast;
sh_int          gsn_hatredbreath;
sh_int          gsn_hatredblast;
sh_int          gsn_acidbreath;
sh_int          gsn_acidblast;

sh_int          gsn_darkness;
sh_int          gsn_paradox;
sh_int          gsn_forge;
sh_int          gsn_deathslash;
sh_int          gsn_soundwave;
...
void boot_db(bool fCopyOver)
{
     ...
     */
    {
        ...
        area_update();
        log_string("Updated areas.\n\r");
        load_bans();
        log_string("Loaded bans.\n\r");
        load_topboard();
        log_string("Loading CCenter");
        load_ccenter();
        log_string("Loaded TopBoard.\n\r");
        //        load_leaderboard();
        //	log_string("Loaded LeaderBoard.\n\r");
        load_kingdoms();
        log_string("Loaded Kingdoms.\n\r");
        Load_Economy();
        log_string("Loaded Economy.\n\r");
        load_boards();
	load_sysdata();
        log_string("Loaded Boards.\n\r");
        load_pkstore();
        log_string("Loaded Pkstore.\n\r");
        save_notes();
        log_string("Saved Notes.\n\r");
        load_changes();
        log_string("Loaded Changes.\n\r");
        load_auction();
        log_string("Loaded Disabled Commands.\n\r");
        LoadAnnounce();
        log_string("Loaded Announce Board.\n\r");
        do_updatemud();
        log_string("Updated Mud PID.\n\r");
        load_social_table();
        log_string("Loaded Social Table.\n\r");
        //	load_wizlist();
        //	log_string("Loaded Wizlist.\n\r");
        load_disabled();
        log_string("Loaded Disabled.\n\r");
        SetupAreas();
        load_best();
        log_string("Loaded Best Data.\n\r");
        log_string("Set proper area permissions.\n\r");
...
void load_mobiles( FILE *fp )
{
    ...
    for ( ; ; )
    {
        ...
        kill_table[URANGE(0, pMobIndex->level, MAX_KILLLEVEL-1)].number++;
    }
...
void load_objects( FILE *fp )
{
    ...
    for ( ; ; )
    {
        ...
        pObjIndex->weapflags        = fread_number( fp );
...
void area_update( void )
{
    ...
    for ( pArea = area_first; pArea != NULL; pArea = pArea->next )
    {
         ...
         */
        if ( ( IS_SET(pArea->area_flags,AREA_NORESET) && pArea->nplayer == 0 ) || pArea->age >= 15 )
        {
            ROOM_INDEX_DATA *pRoomIndex;
    ...
    for ( pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next )
    {
        ...
        switch ( pReset->command )
        {
            ...
            case 'O':
                ...
                if ( ( IS_SET(pRoom->area->area_flags,AREA_NORESET) && pRoom->area->nplayer > 0 )
...
void reset_area( AREA_DATA *pArea )
{
    ...
    /*    for ( pChar = char_list; pChar != NULL; pChar = pChar->next )
        {
            if ( pChar->in_room != NULL && pChar->in_room->area == pArea )
                return;
        }*/
...
CHAR_DATA *create_mobile( MOB_INDEX_DATA *pMobIndex )
{
    ...
    if(tempvalue > 3000000 || tempvalue < 0) mob->max_hit = 3000000;
    ...
    if (number_range(1,400) == 1)
    {
        OBJ_DATA *obj;
        int nObject = number_range(30050,30071);
        if( nObject == 30052 )
        {
            if (number_range(1,5) == 2)
            {
                obj = create_object( get_obj_index( nObject ), 0 );
                obj_to_char(obj,mob);
            }
        }
        else
    ...
    if (number_range(1, 1000) == 2)
    {
        ...
        //		nObject = 27;
        if(number_range(1,30)==4)
            nObject = 113028;
        else if(number_range(1,40)==4)
            nObject = 113029;
    ...
    {
        //OBJ_DATA *obj;
        int nObject = number_range(90,92);
        for ( obj = object_list; obj != NULL; obj = obj->next )
        {
            if ( obj->pIndexData->vnum == 91 || obj->pIndexData->vnum == 90 )
            {
                number++;
                continue;
            }
        ...
        if (pMobIndex->vnum >= 40700)
        {
            if (pMobIndex->vnum <= 40799)
            {
                mob->next       = char_list;
                char_list       = mob;
                pMobIndex->count++;
        ...
        if (pMobIndex->vnum >= 102900)
        {
            if (pMobIndex->vnum <= 102999)
            {
                mob->next       = char_list;
                char_list       = mob;
                pMobIndex->count++;
...
OBJ_DATA *create_object( OBJ_INDEX_DATA *pObjIndex, int level )
{
    ...
    *obj        = obj_zero;
    ...
    obj->weapflags  = pObjIndex->weapflags;
    ...
    if (obj->pIndexData->vnum == 30039)                     /* Questcards */
    {
        ...
        SET_BIT(obj->quest,QUEST_RELIC);
                                                            ...
                                                            /* AnotherLandEq */
    else if (obj->pIndexData->vnum >= 101000 && obj->pIndexData->vnum <= 101099)
    {
        ...
        obj->cost       =  100000000;
                                                            ...
                                                            /* artifacts */
    else if (obj->pIndexData->vnum >=  33960 && obj->pIndexData->vnum <= 34000)
    {
        ...
        obj->cost       =  1000000;
                                                            ...
                                                            /* Prizes */
    else if (obj->pIndexData->vnum >=  33800 && obj->pIndexData->vnum <= 33899)
    {
        ...
        obj->cost       =  1000000;
                                                            ...
                                                            /* normal class eq */
    else if (obj->pIndexData->vnum >= 33000 && obj->pIndexData->vnum <= 33333)
    {
        obj->condition = 100;
        obj->toughness = 100;
        obj->resistance = 1;
        SET_BIT(obj->quest,QUEST_RELIC);
                                                            ...
                                                            /* Undead Knight eq */
    else if (obj->pIndexData->vnum >= 29775 && obj->pIndexData->vnum <= 29990)
    {
        obj->condition = 100;
        obj->toughness = 100;
        obj->resistance = 1;
        SET_BIT(obj->quest,QUEST_RELIC);
                                                            ...
                                                            /* Wraith eq */
    else if (obj->pIndexData->vnum >= 110070 && obj->pIndexData->vnum <= 110082)
    {
        obj->condition = 100;
        obj->toughness = 100;
        obj->resistance = 1;
    ...
    else if(obj->pIndexData->vnum >= 810 && obj->pIndexData->vnum <= 825)
        /* rune EQ */
    {
        SET_BIT(obj->quest, QUEST_RELIC);
        obj->condition = 100;
    ...
    else if(obj->pIndexData->vnum == 102100)                /* rune EQ */
    {
        SET_BIT(obj->quest, QUEST_RELIC);
        obj->condition = 100;
        obj->toughness = 100;
        obj->resistance = 1;
                                                            ...
                                                            /* merc EQ */
    else if(obj->pIndexData->vnum >= 110000 && obj->pIndexData->vnum <= 111000)
    {
        SET_BIT(obj->quest, QUEST_RELIC);
        obj->condition = 100;
        obj->toughness = 100;
    ...
    switch ( obj->item_type )
    {
        ...
        case ITEM_STAFF:
        case ITEM_RUNE:
        case ITEM_RUNESCROLL:
        case ITEM_TITANIUM:
        case ITEM_DIAMOND:
        case ITEM_EXCAVIER:
        case ITEM_GREENSTONE:
        case ITEM_REDSTONE:
        case ITEM_BLUESTONE:
        case ITEM_TELEPORTS:
        case ITEM_LIQUID_SHADOW:
        case ITEM_SOCKET:
        case ITEM_FLAKE:
            break;
        ...
        case ITEM_WEAPON:
            if (!IS_SET(obj->quest, QUEST_ARTIFACT)
                ...
                QUEST_PRIZE) && !IS_SET(obj->quest, QUEST_ANLA))
            {
