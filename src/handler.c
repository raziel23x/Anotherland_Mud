...
int get_curr_str( CHAR_DATA *ch )
{
    ...
    max = 32000;
    if(IS_CLASS(ch, CLASS_SAMURAI) || IS_IMMORTAL(ch) ) max = 32000;

    return URANGE( 6, ch->pcdata->perm_str + ch->pcdata->mod_str, max );
}
...
void affect_modify( CHAR_DATA *ch, AFFECT_DATA *paf, bool fAdd, OBJ_DATA *obj )
{
    ...
    if( !IS_NPC(ch) && obj != NULL && IS_CLASS(ch, CLASS_SAMURAI)
        && (obj->pIndexData->vnum == 33177 || IS_SET(obj->quest,QUEST_ARTIFACT) || IS_SET(obj->extra_flags, ITEM_SAMAFFECT) ) )
    {
        switch ( paf->location )
        {
            default:
     ...
     * Guard against recursion (for weapons with affects).
        ...
        if ( depth == 0 )
        {
            ...
            act( "$n drops $p. Recursion", ch, wield, NULL, TO_ROOM );
            obj_from_char( wield );
...
void affect_remove_obj(OBJ_DATA * obj, AFFECT_DATA * paf)
{
    ...
    if(obj->wear_loc != ITEM_TAKE)
    {
        affect_modify( obj->carried_by, paf, FALSE, obj );
    }
    ...
    else
    {
        ...
        if ( prev == NULL )
        {
            bug( "Affect_remove_obj: cannot find paf.", 0 );
            return;
        }
    }
...
void char_from_room( CHAR_DATA *ch )
{
    ...
    if ( ch == NULL )
    {
        bug( "Char_from_room: NULL Character.", 0 );
        return;
    }
    if ( ch->in_room == NULL )
    {
...
void char_to_room( CHAR_DATA *ch, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    if ( ch == NULL ) return;
    if ( !IS_NPC(ch) && IS_SET(pRoomIndex->area->area_flags,AREA_NOPLAYER) && get_trust(ch) < 7 )
    ...
    /*Calculates damcap when a character is placed in a room.*/
    if(!IS_NPC(ch))
    {
        update_damcap(ch, ch);
    }
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    /*
            ...
            if (IS_SET(obj->spectype, SITEM_MECHA))
    ...
    */
    ...
    if (((obj->pIndexData->vnum >= 33300 && obj->pIndexData->vnum <= 33313))
        && (IS_NPC(ch) || !IS_CLASS(ch,CLASS_ELF)))
    {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR);
        act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM);
        obj_from_char(obj);
    ...
    if (((obj->pIndexData->vnum >= 33280 && obj->pIndexData->vnum <= 33293))
        && (IS_NPC(ch) || !IS_CLASS(ch,CLASS_KAHN)))
    {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR);
        act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM);
        obj_from_char(obj);
    ...
    if (IS_NPC(ch) && IS_SET(obj->quest,QUEST_ANLA))
    {
        act( "You are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
        act( "$n is zapped by $p and drops it.",  ch, obj, NULL, TO_ROOM );
        obj_from_char( obj );
    ...
    if ( IS_NPC(ch) || !IS_CLASS(ch, CLASS_SAMURAI) || (IS_CLASS(ch, CLASS_SAMURAI)
        ...
        || IS_SET(obj->quest, QUEST_ARTIFACT) || IS_SET(obj->extra_flags,ITEM_SAMAFFECT) )
    ...
    {
        isruneeq = TRUE;
        if ( is_rune_robe_obj(obj,"020000000") )
            sn = 11;
        else if ( is_rune_robe_obj(obj,"200000000") )
            sn = 12;
        else if ( is_rune_robe_obj(obj,"002000000") )
            sn = 10;
        else if ( is_rune_robe_obj(obj,"000000200") )
            sn = 20;
        else if ( is_rune_robe_obj(obj,"222020000") )
            sn = 15;
        else isruneeq = FALSE;
    }
        ...
        || isruneeq )
    {
        ...
        else if ((sn == 400) && (IS_ITEMAFF(ch, ITEMA_RETORT)))      return;
        else if ((sn == 401) && (IS_ITEMAFF(ch, ITEMA_BLOCK)))      return;
        else if ((sn == 402)  && (IS_ITEMAFF(ch, ITEMA_STANCEA)))    return;
        ...
        else if (sn == 21)
        {
            ...
            send_to_char("You are surrounded by a golden shield.\r\n",ch);
            act("$n is surrounded by a golden shield.\r\n",ch,NULL,NULL,TO_ROOM);
        }
        else if (sn == 22)
        {
        ...
        else if (sn == 25 && !IS_NPC(ch))
        {
            ...
            {
                ch->rage = (1000+((ch->pcdata->upgrade_level)+1)* 100);
                do_werewolf(ch,"");
            }
            ...
            SET_BIT(ch->itemaffect, ITEMA_RETORT);
        else if (sn == 401)
            SET_BIT(ch->itemaffect, ITEMA_BLOCK);
        else if (sn == 402)
            SET_BIT(ch->itemaffect, ITEMA_STANCEA);
...
void unequip_char( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if ( obj->wear_loc == WEAR_SCABBARD_L ||
        ...
        if ( IS_NPC(ch) || !IS_CLASS(ch, CLASS_SAMURAI) || (IS_CLASS(ch, CLASS_SAMURAI)
        && obj->pIndexData->vnum == 29694 ) || IS_SET(obj->quest, QUEST_ARTIFACT) || IS_SET(obj->extra_flags,ITEM_SAMAFFECT) )
        ...
        || isruneeq )
    {
        ...
        else if (sn == 25 && !IS_NPC(ch) && IS_ITEMAFF(ch, ITEMA_RAGER))
        {
            ...
            REMOVE_BIT(ch->itemaffect, ITEMA_RETORT);
        else if (sn == 401)
            REMOVE_BIT(ch->itemaffect, ITEMA_BLOCK);
        else if (sn == 402)
        {
            REMOVE_BIT(ch->itemaffect, ITEMA_STANCEA);
            /*autostance("none",ch);*/}
    ...
    bool can_see( CHAR_DATA *ch, CHAR_DATA *victim )
    {
        ...
        /*This added by Magna*/
        if (IS_NPC(ch) && (!IS_NPC(victim) && IS_IMMORTAL(victim) && IS_SET(victim->act, PLR_WIZINVIS))) return FALSE;

        if (!IS_NPC(ch) && !IS_NPC(victim) && IS_IMMORTAL(ch) && IS_IMMORTAL(victim) && ch->level >= victim->level ) return TRUE;
        if (!IS_NPC(ch) && ( ch->level > victim->level ) ) return TRUE;
        if (!IS_NPC(victim) && IS_SET(victim->act, PLR_WIZINVIS) && ch->level < victim->level ) return FALSE;
        if ( IS_ITEMAFF(ch, ITEMA_VISION) ) return TRUE;
        ...
        if ( !IS_NPC(ch) && (!IS_CLASS(ch,CLASS_VAMPIRE) && !IS_CLASS(ch,CLASS_WEREWOLF)) &&
            IS_SET(victim->act,PLR_SUPERINVIS) && !IS_AFFECTED(ch, AFF_SHADOWSIGHT) &&
            (IS_CLASS(victim,CLASS_WEREWOLF) || IS_CLASS(victim,CLASS_VAMPIRE) || IS_CLASS(victim,CLASS_NINJA)
            || IS_CLASS(ch, CLASS_KAHN)))
            return FALSE;
        if ( !IS_NPC(ch) && IS_CLASS(victim,CLASS_KAHN) && !IS_CLASS(ch, CLASS_KAHN) && IS_SET(victim->act, PLR_SUPERINVIS)) return FALSE;
        if ( !IS_NPC(ch) && IS_CLASS(victim,CLASS_ELF) && !IS_CLASS(ch, CLASS_ELF) && IS_SET(victim->act, PLR_SUPERINVIS)) return FALSE;
        if ( !IS_NPC(ch) && IS_CLASS(victim,CLASS_VAMPIRE) && !IS_CLASS(ch, CLASS_VAMPIRE) && IS_SET(victim->act, PLR_SUPERINVIS)) return FALSE;
        if ( !IS_NPC(ch) && IS_CLASS(victim,CLASS_NINJA) && !IS_CLASS(ch, CLASS_NINJA) && IS_SET(victim->act, PLR_SUPERINVIS)) return FALSE;
        if ( !IS_NPC(ch) && IS_CLASS(victim,CLASS_MONK) && !IS_CLASS(ch, CLASS_MONK) && IS_SET(victim->act, PLR_SUPERINVIS)) return FALSE;
        if ( IS_HEAD(ch, LOST_EYE_L) && IS_HEAD(ch, LOST_EYE_R) )
            return FALSE;

        if ( IS_AFFECTED(ch, AFF_BLIND) && !IS_AFFECTED(ch, AFF_SHADOWSIGHT) && !IS_SET(ch->act, PLR_HOLYLIGHT) )
            return FALSE;
    ...
    char *item_type_name( OBJ_DATA *obj )
    {
        switch ( obj->item_type )
        {
            ...
            case ITEM_XPTOKEN:      return "exp token";
            case ITEM_STORETOKEN:   return "store token";
            case ITEM_PRTOKEN:      return "primal token";
            case ITEM_QUEST:        return "quest token";
            case ITEM_PIECE:            return "piece";
            case ITEM_FLAKE:            return "flake";
            ...
            case ITEM_DND_PAPER:    return "d&d-paper";
            case ITEM_RUNE:     return "rune";
            case ITEM_RUNESCROLL:   return "runescroll";
            case ITEM_TITANIUM:     return "titanium";
            case ITEM_DIAMOND:      return "zirconium";
            case ITEM_EXCAVIER:     return "excavier";
            case ITEM_GREENSTONE:   return "greenelfstone";
            case ITEM_REDSTONE:         return "redelfstone";
            case ITEM_BLUESTONE:        return "blueelfstone";
            case ITEM_TELEPORTS:    return "teleports";
            case ITEM_LIQUID_SHADOW:    return "mithril";
    ...
    char *affect_bit_name( int vector )
    {
        ...
        if ( vector & AFF_FREEZING      ) strcat( buf, " freezing"      );
    ...
    char *extra_bit_name( int extra_flags )
    {
        ...
        if ( extra_flags & ITEM_SAMAFFECT    ) strcat( buf, " samaffect"    );
        return ( buf[0] != '\0' ) ? buf+1 : "none";
    }
    ...
    int get_artifact_count(CHAR_DATA *ch)
    {
        OBJ_DATA * pObj;
        int iCount = 0;
        ...
        {
            if(IS_SET(pObj->quest, QUEST_ARTIFACT))
            {
                iCount += 1;
            }
            ...
            {
                iCount += recursive_artifact_count(pObj);
            }
        }
    ...
    int recursive_artifact_count(OBJ_DATA *pContainer)
    {
        OBJ_DATA * pObj;
        int iCount = 0;

        for(pObj = pContainer->contains; pObj != NULL; pObj = pObj->next_content)
        {
            if(IS_SET(pObj->quest, QUEST_ARTIFACT))
            {
                iCount += 1;
    ...
    int get_item_count(CHAR_DATA *ch)
    {
        OBJ_DATA * pObj;
        int iCount = 0;
        ...
        for(pObj = ch->carrying; pObj != NULL; pObj = pObj->next_content)
        {
            ...
            if(pObj->contains != NULL)
            {
                iCount += recursive_item_count(pObj);
            }
        }
    ...
    int recursive_item_count(OBJ_DATA *pContainer)
    {
        OBJ_DATA * pObj;
        int iCount = 0;
