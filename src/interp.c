...
void do_racecommands( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_CLASS(ch, CLASS_ELF))
    {
        ...
        send_to_char("	Elven Powers: elfbasics elfprofession elveneyes elvenears\n\r",ch);
        send_to_char("		      elvenintuition elvenprotection elvenvanish \n\r",ch);
        send_to_char("		      elvenstrength elftalk elfarmor elfblink\n\r",ch);

        if (IS_SET(ch->special,SPC_ELF_MAGE))
        {
            send_to_char("	 Mage  Powers: elfmagetrain earthbless naturestouch naturesfury\n\r",ch);
            send_to_char("		       firebless flamering firecurse waterbless waterwall\n\r",ch);
            send_to_char("		       icetouch stonecreate elfstones\n\r",ch);
            send_to_char("#0<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>#n\n\r", ch);
            return;
        }
        else if (IS_SET(ch->special,SPC_ELF_WARRIOR))
        {
            send_to_char("   Warrior Powers: elfwartrain elfweb swordslash wishsong\n\r",ch);
            send_to_char("#0<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>#n\n\r", ch);
            return;
        }
        else if (IS_SET(ch->special,SPC_ELF_ARCHER))
        {
            send_to_char("	Archer Powers: elfarctrain webarrow elvenaim archersfury\n\r",ch);
            send_to_char("		       archersrage\n\r",ch);
            send_to_char("#0<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>==<>#n\n\r", ch);
            return;
        }
        return;
            ...
            && cmd_table[cmd].discipline == 0)
        {
            ...
            if (i == 0)
            {
                switch (ch->class)
                {
                    ...
                    case 32768:sprintf(buf, " %15s : ", "Dragon Powers");break;
                    case 65536:sprintf(buf, " %15s : ", "Fae Powers");break;
                    case 131072:sprintf(buf, " %15s : ", "Rune Powers");break;
                    case 262144:sprintf(buf, " %15s : ", "Elven Powers");break;
                    case 524288:sprintf(buf, " %15s : ", "Kahn Powers");break;
                    case 1048576:sprintf(buf, " %15s : ", "Wizard Powers");break;
                    case 2097152:sprintf(buf, " %15s : ", "Blood Rites");break;
                    case 16777216:sprintf(buf, " %15s : ", "Phoenix Powers");break;
                    case 8388608:sprintf(buf, " %15s : ", "Military Commands");break;
                    case 33554432:sprintf(buf, " %15s : ", "Wraith Powers");break;
...
bool    arena2              = TRUE;
bool    arena_open      = TRUE;
bool    arena_base      = TRUE;
bool    arenaopen       = FALSE;
bool    ragnarok        = FALSE;
bool    fLogAll         = FALSE;
bool    global_exp      = FALSE;
bool    world_channel       = FALSE;
bool    world_quest     = FALSE;
bool    world_time      = -1;
...
int ragnarok_on_timer   = 15;
int ragnarok_safe_timer = 60;
int pulse_arena     = 10;
int pulse_doubleexp     = 2;
int pulse_doubleqp      = 5;
int pulse_autoquest     = 2;
int pulse_flake         = 5;
int players_decap       = 0;
...
const   struct  cmd_type    cmd_table   [] =
{
    ...
    /*Added by Maar (Tempest)*/
    { "setglobalchat",  do_setglobalchat, POS_STANDING,     0,  LOG_NORMAL, 0,0,0       },
    { "globalchat", do_globalchat,      POS_STANDING,   0,  LOG_NORMAL, 0,0,0       },
    /*End Maar Additions*/
    ...
    { "iscast",     do_iscast,      POS_FIGHTING,   7,  LOG_NORMAL, 0,0,0       },
    ...
    { "detox",      do_detox,       POS_STANDING,   0,  LOG_NORMAL, 0,0,0       },
    ...
    { "miscbag",    do_miscbag,     POS_DEAD,   1,  LOG_NORMAL, 0,0,0       },
    ...
    { "reimb",      do_reimb,       POS_MEDITATING, 10, LOG_ALWAYS, 0,0,0       },
    ...
    { "iwhere",     do_iwhere,      POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "version",    do_version,     POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "linkdead",   do_linkdead,        POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    //    { "openthearena",   do_openthearena,	POS_STANDING,	11,	LOG_NORMAL, 0,0,0		},
    /*
    ...
    { "autoclass",  do_autoclass,       POS_STANDING,   3,  LOG_ALWAYS, 0,0,0       },
    ...
    { "delete",     do_delete,      POS_STANDING,   1,  LOG_NEVER, 0,0,0        },

    // Devik's Bloodpower
    { "bloodpower", do_bloodpower,      POS_STANDING,   3,  LOG_ALWAYS, 0,0,0       },
    /*
     * Configuration commands.
     */
    { "alignment",  do_alignment,       POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "ddalignment",    do_rpalignment,     POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
     ...
     */
    /*    { "filtchan",	do_filtchan,		POS_DEAD,	7,	LOG_NORMAL, 0,0,0		},*/
    { "flame",      do_flame,       POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "chat",       do_chat,        POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "ooc",        do_ooc,     POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "rpchat",     do_rpchat,      POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "rchat",      do_rchat,       POS_DEAD,   0,  LOG_NORMAL, 0, 0, 0 },
    ...
    { "howl",       do_howl,        POS_DEAD,   1,  LOG_NORMAL, 4,0,0       },
    { "telepath",   do_telepath,        POS_DEAD,   1,  LOG_NORMAL, 512,0,0     },
    { "dtalk",      do_dtalk,       POS_DEAD,   2,  LOG_NORMAL, 0,0,0       },
    { "militalk",       do_militalk,            POS_DEAD,       2,      LOG_NORMAL, 0,0,0               },
    { "screech",        do_screech,             POS_DEAD,       2,      LOG_NORMAL, 16777216,0,0               },
    ...
    { "activequest",    do_activequest,     POS_STANDING,   0,  LOG_NEVER,  0,0,0       },
    { "say",        do_say,         POS_MEDITATING, 0,  LOG_NORMAL, 0,0,0       },
    { "rsay",       do_rsay,        POS_MEDITATING, 0,  LOG_NORMAL, 0, 0, 0},
    { "'",      do_say,         POS_MEDITATING, 0,  LOG_NORMAL, 0,0,0       },
    { "yell",       do_yell,        POS_SITTING,    2,  LOG_NORMAL, 0,0,0       },
    { "game",       do_game,        POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "powers",     do_racecommands,    POS_FIGHTING,   3,  LOG_NORMAL, 0,0,0       },

    /*
     * Immortal Specialized commands.
     */
    { "isharmlocal",    do_isharmlocal,     POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "isharmglobal",   do_isharmglobal,    POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "isharmroom", do_isharmroom,      POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "istransfer", do_istransfer,      POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "isotransfer",    do_isotransfer,     POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "ismload",    do_ismload,     POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "isoload",    do_isoload,     POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "ispurge",    do_ispurge,     POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "isslay",     do_isslay,      POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "isgoto",     do_isgoto,      POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "dismember",  do_dismember,       POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "spinebreak",     do_spinebreak,      POS_DEAD,   10, LOG_NORMAL, 0,0,0},
    { "groupgoto",  do_groupgoto,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0},
    { "snatch",     do_snatch,      POS_DEAD,   8,  LOG_NORMAL, 0,0,0},
    /*
    ...
    { "drop",       do_drop,        POS_SITTING,    0,  LOG_ALWAYS, 0,0,0       },
    { "discard", do_discard, POS_SITTING, 0, LOG_ALWAYS, 0,0,0 },
    ...
    { "take",       do_get,         POS_SITTING,    0,  LOG_ALWAYS, 0,0,0       },
    ...
    { "donate",     do_donate,      POS_SITTING,    1,  LOG_ALWAYS, 0,0,0       },
    ...
    { "classleader",    do_classleader,     POS_STANDING,   1,  LOG_NEVER, 0,0,0        },
    ...
    /* Chrono stuff weeee */
    { "wiznet", do_wiznet, POS_DEAD, 10, LOG_NORMAL, 0,0,0 },
    { "searchlog", do_search, POS_DEAD, 8, LOG_NORMAL, 0,0,0 },
    { "pipe", do_pipe, POS_DEAD, 12, LOG_NORMAL, 0,0,0 },

    /*JOSH RANDOMEQ TEST*/
    { "testing",         do_randomeq,             POS_STANDING,   1,      LOG_NORMAL, 0,0,0               },
    { "damtest",         do_damtest,              POS_STANDING,   1,      LOG_NORMAL, 0,0,0               },
    { "averagedamtest",  do_averagedamtest,       POS_STANDING,   1,      LOG_NORMAL, 0,0,0               },

    /* Raziel's stuff */

    { "buystatus",          do_buystatus,               POS_STANDING,   0,      LOG_ALWAYS, 0,0,0               },

    /*
     *
     * Wizard Commands needed here / Chrono
     */
    { "arts",    do_arts,    POS_STANDING,1, LOG_NORMAL, 1048576,0,0 },
    { "entropy",     do_entropy,     POS_STANDING,1, LOG_NORMAL, 1048576,0,0 },
    { "wizardarmor", do_wizardarmor, POS_DEAD,    1, LOG_NORMAL, 1048576,0,0 },
    { "wiztalk",     do_wiztalk, POS_DEAD,    1, LOG_NORMAL, 1048576,0,0 },
    { "resurrect",   do_resurrect,   POS_STANDING,1, LOG_NORMAL, 1048576,0,0 },
    { "wizardsight", do_wizardsight, POS_STANDING,1, LOG_NORMAL, 1048576,0,0 },
    { "magicsight",  do_magicsight,  POS_STANDING,1, LOG_NORMAL, 1048576,0,0 },
    { "shadowshift", do_shadowshift, POS_STANDING,1, LOG_NORMAL, 1048576,0,0 },
    { "thunderclap", do_thunderclap, POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "hellblades",  do_hellblades,  POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "wrath",   do_wrath,   POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "firebolt",    do_firebolt,    POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "pshift",  do_pshift,  POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "vortex",  do_vortex,  POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "icelance",    do_icelance,    POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "negativity",  do_negativity,  POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    { "chaos",   do_chaos,   POS_FIGHTING,1, LOG_NORMAL, 1048576,0,0 },
    /*
     * Josh's screwed up commands i mean class commands COB and testing
     */
    { "bootcamp",    do_bootcamp,    POS_STANDING,3, LOG_NORMAL, 8388608,0,0 },
    { "muutspecial", do_muutspecial, POS_STANDING,3, LOG_NORMAL, 0,0,0},
    { "montset",     do_montset,     POS_STANDING,3, LOG_NORMAL, 0,0,0},
    { "march",       do_march,       POS_STANDING,3, LOG_NORMAL, 8388608,0,0 },
    { "nightvision", do_nightvision, POS_STANDING,3, LOG_NORMAL, 8388608,0,0 },
    { "stimpack",    do_stimpack   , POS_FIGHTING,3, LOG_NORMAL, 8388608,0,0 },
    { "medkit",      do_medkit     , POS_STANDING,3, LOG_NORMAL, 8388608,0,0 },
    { "steroids",    do_steroids   , POS_STANDING,3, LOG_NORMAL, 8388608,0,0 },
    { "militaryeq",    do_militaryeq   , POS_STANDING,3, LOG_NORMAL, 8388608,0,0 },
    { "net",         do_web        , POS_STANDING,3, LOG_NORMAL, 8388608,0,0 },

    /*
     *RPONLY COMMANDS 4194304
     */
    {"rptoken",         do_rptoken,             POS_STANDING,   10,      LOG_ALWAYS, 0,0,0              },
    /*
     * Lich commands.
     */
    ...
    { "golemsummon",    do_summongolem,     POS_STANDING,   3,  LOG_NORMAL, 256,0,0     },
    ...
    { "angelfly",      do_angelfly,     POS_STANDING,   3,  LOG_NORMAL, 2048,0,0        },
    ...
    { "sanctum",    do_pkheal,      POS_FIGHTING,   3,  LOG_NORMAL, 0,0,0       },
    ...
    { "krecall",    do_krecall,     POS_FIGHTING,   0,  LOG_NORMAL, 0,0,0       },
    ...
    { "melt",       do_melt,        POS_STANDING,   0,  LOG_NORMAL, 0,0,0       },
    ...
    { "award",      do_award,       POS_STANDING,   2,  LOG_NORMAL, 0,0,0       },
    { "convert",    do_convert,     POS_STANDING,   2,  LOG_NORMAL, 0,0,0       },
    { "commit",     do_commit,      POS_STANDING,   2,  LOG_ALWAYS, 0,0,0       },
    { "realm",      do_realm,       POS_STANDING,   3,  LOG_ALWAYS, 0,0,0       },
    { "chaoseq",        do_chaoseq,             POS_STANDING,   3,      LOG_ALWAYS, 0,0,0               },
    { "validexchange",  do_validexchange,   POS_STANDING,   3,  LOG_NORMAL, 0,0,0       },
    { "primalgain", do_primalgain,      POS_STANDING,   3,  LOG_NEVER,  0,0,0       },
    { "happybuy",   do_happybuy,        POS_STANDING,   2,  LOG_ALWAYS, 0,0,0       },
    { "become",     do_avatar,      POS_STANDING,   12, LOG_ALWAYS, 0,0,0       },
    //    { "updatepid",	do_updatemud,		POS_STANDING,	13,	LOG_ALWAYS, 0,0,0		},
    { "store",      do_store,       POS_STANDING,   2,  LOG_NORMAL, 0,0,0       },
    { "token",      do_token,       POS_STANDING,   2,  LOG_ALWAYS, 0,0,0       },
    { "newbiepack", do_newbiepack,      POS_STANDING,   0,  LOG_ALWAYS, 0,0,0       },
    { "ddreroll",   do_reroll,      POS_STANDING,   3,  LOG_NEVER,  0,0,0       },
    //    { "pkreset",	do_pkreset,		POS_STANDING,	3,	LOG_NORMAL, 0,0,0		},
    { "dice",       do_dice,        POS_STANDING,   2,  LOG_NEVER,  0,0,0       },
    { "mdice",          do_mdice,               POS_STANDING,   2,      LOG_NEVER,  0,0,0               },
    { "ddamge",     do_dddamage,        POS_STANDING,   3,  LOG_NEVER,  0,0,0       },
    { "mightscore", do_mightscore,      POS_STANDING,   3,  LOG_NEVER,  0,0,0       },
    { "genbuy",     do_genbuy,      POS_STANDING,   3,  LOG_ALWAYS, 0,0,0       },
    { "twit",       do_twit,        POS_STANDING,   11, LOG_ALWAYS, 0,0,0       },
    { "railram",    do_railram,     POS_STANDING,   11, LOG_ALWAYS, 0,0,0        },
    { "unrestore",  do_unrestore,       POS_STANDING,   12, LOG_ALWAYS, 0,0,0        },
    { "nuke",       do_nuke,        POS_STANDING,   14, LOG_ALWAYS, 0,0,0       },
    ...
    { "qspell",     do_qspell,      POS_STANDING,   12, LOG_NORMAL, 0,0,0       },
    { "ignite",     do_ignite,      POS_STANDING,   9,  LOG_NORMAL, 0,0,0       },
    ...
    { "info",       do_info,        POS_DEAD,   7,  LOG_NORMAL, 0,0,0       },
    { "questchat",  do_questchat,       POS_STANDING,   10, LOG_NORMAL, 0,0,0       },
    ...
    //  { "resetarea",	do_resetarea,		POS_DEAD,	8,	LOG_ALWAYS, 0,0,0		},
    ...
    /*
     *
     * Phoenix Commands needed here / Chrono
         */
    {                                                       
        "firelearn",do_firelearn,POS_STANDING,1, LOG_NORMAL, 16777216,0,0
    },
    { "firedive", do_firedive, POS_STANDING,1, LOG_NORMAL, 16777216,0,0 },
    { "firestrike", do_firestrike, POS_FIGHTING,1, LOG_NORMAL, 16777216,0,0 },
    { "flare", do_flare, POS_FIGHTING,1, LOG_NORMAL, 16777216,0,0 },
    { "flameeq", do_flameeq, POS_STANDING,1,  LOG_NORMAL, 16777216,0,0 },
    { "phoenixsight", do_phoenixsight, POS_STANDING,1, LOG_NORMAL, 16777216,0,0 },
    { "phoenixclaws", do_phoenixclaws, POS_STANDING,1, LOG_NORMAL, 16777216,0,0 },
    { "phoenixform", do_phoenixshift, POS_STANDING,1, LOG_NORMAL, 16777216,0,0 },
    { "sunfire", do_sunfire, POS_FIGHTING,1, LOG_NORMAL, 16777216,0,0 },
    { "tailflare", do_tailflare, POS_STANDING,1, LOG_NORMAL, 16777216,0,0 },
    { "healingtears", do_healingtears,POS_STANDING,1,LOG_NORMAL,16777216,0,0},
    { "sunbathe", do_calm,POS_STANDING,1,LOG_NORMAL,16777216,0,0},
    { "rebirth", do_rebirth,POS_DEAD,1,LOG_NORMAL,16777216,0,0},
    ...
    { "bansite",    do_ban,         POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    ...
    { "hedit",      do_hedit,       POS_STANDING,   9,  LOG_NORMAL, 0,0,0       },
    { "freeze",     do_freeze,      POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "bitchslap",  do_freeze,      POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    ...
    { "insert",     do_insert,      POS_STANDING,   1,  LOG_NORMAL, 0,0,0       },
    { "connect",    do_connect,     POS_STANDING,   1,  LOG_NORMAL, 0,0,0        },
    ...
    { "weaken",         do_weaken,              POS_MEDITATING, 3,      LOG_NORMAL, 128,0,0             },
    //    { "dokuyaku",       do_dokuyaku,            POS_MEDITATING, 3,      LOG_NORMAL, 128,0,0             },
    ...
    { "addchange",  do_addchange,       POS_DEAD,   10, LOG_NORMAL, 0,0,0   },
    { "chsave",     do_chsave,      POS_DEAD,   10, LOG_NORMAL, 0,0,0   },
    ...
    { "startragbid",    do_startragbid,     POS_STANDING,   4,  LOG_NORMAL, 0,0,0       },
    { "showsilence",    do_showsilence,     POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "showcomp",   do_showcompress,    POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "implag",     do_implag,      POS_DEAD,   11, LOG_NORMAL, 0,0,0       },
    { "doublexp",   do_doublexp,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "trust",      do_trust,       POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "allow",      do_allow,       POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "bind",       do_bind,        POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "clearstats", do_clearstats,      POS_STANDING,   0,  LOG_NORMAL, 0,0,0       },
    { "create",     do_create,      POS_STANDING,   9,  LOG_NORMAL, 0,0,0       },
    { "deny",       do_deny,        POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "disable",    do_disable,     POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "disconnect", do_disconnect,      POS_DEAD,   12, LOG_NEVER,  0,0,0       },
    { "divorce",    do_divorce,     POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "distort",    do_distort,     POS_STANDING,    8, LOG_NORMAL, 0,0,0       },
    { "familiar",   do_familiar,        POS_STANDING,   13, LOG_NORMAL, 0,0,0       },
    { "fcommand",   do_fcommand,        POS_STANDING,   4,  LOG_NORMAL, 0,0,0       },
    { "freeze",     do_freeze,      POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "marry",      do_marry,       POS_DEAD,   7,  LOG_ALWAYS, 0,0,0       },
    { "paradox",    do_paradox,     POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "bully",      do_bully,       POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "qset",       do_qset,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "qstat",      do_qstat,       POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "qtrust",     do_qtrust,      POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "copyove",    do_huh,         POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "copyover",   do_copyover,        POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    //    { "shutdow",	do_shutdow,		POS_DEAD,	13,	LOG_NORMAL, 0,0,0		},
    { "shutdown",   do_shutdown,        POS_DEAD,   13, LOG_ALWAYS, 0,0,0       },
    { "users",      do_users,       POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "omni",       do_omni,        POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "checkpk",    do_checkpk,     POS_DEAD,   8,  LOG_NORMAL, 0,0,0        },
    { "hreload",    do_hreload,     POS_DEAD,   13, LOG_NORMAL, 0,0,0       },
    { "wizlock",    do_wizlock,     POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "newlock",    do_newlock,     POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "closemud",   do_closemud,        POS_DEAD,   13, LOG_ALWAYS, 0,0,0       },
    { "watche",     do_huh,         POS_DEAD,   2,  LOG_NEVER,  0,0,0       },
    { "watcher",    do_watcher,     POS_DEAD,   10, LOG_NEVER,  0,0,0       },
    { "force",      do_force,       POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "cforce",     do_cforce,      POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "pfile",      do_pfile,       POS_STANDING,   12, LOG_ALWAYS, 0,0,0       },
    { "asperson",   do_asperson,        POS_DEAD,   13, LOG_ALWAYS, 0,0,0       },
    { "offline",    do_offline,     POS_DEAD,   13, LOG_ALWAYS, 0,0,0       },
    { "exlist",     do_exlist,      POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "mload",      do_mload,       POS_DEAD,   8,  LOG_ALWAYS, 0,0,0       },
    { "undeny",     do_undeny,      POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "mset",       do_mset,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "worldpk",    do_worldpk,     POS_DEAD,   14, LOG_ALWAYS, 0,0,0        },
    { "worldtest",      do_worldtest,           POS_DEAD,       14,     LOG_ALWAYS, 0,0,0                },
    { "weapset",    do_weapset,     POS_DEAD,   10, LOG_ALWAYS, 0,0,0        },
    { "spellset",   do_spellset,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0        },
    { "stanceset",  do_stanceset,       POS_DEAD,   10, LOG_ALWAYS, 0,0,0        },
    { "multicheck", do_multicheck,      POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "resetpasswd",    do_resetpassword,   POS_DEAD,   14, LOG_ALWAYS, 0,0,0       },
    { "oclone",     do_oclone,      POS_DEAD,   11, LOG_ALWAYS, 0,0,0       },
    { "oload",      do_oload,       POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "oset",       do_oset,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "otransfer",  do_otransfer,       POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    ...
    { "pstat",      do_pstat,       POS_DEAD,   9,  LOG_NORMAL, 0,0,0       },
    { "purge",      do_purge,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "qmake",      do_qmake,       POS_DEAD,   9,  LOG_ALWAYS, 0,0,0       },
    { "release",    do_release,     POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "restore",    do_restore,     POS_DEAD,   9,  LOG_ALWAYS, 0,0,0       },
    { "rset",       do_rset,        POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "silence",    do_silence,     POS_DEAD,   11, LOG_NORMAL, 0,0,0       },
    //    { "sla",		do_sla, 		POS_DEAD,	11,	LOG_NORMAL, 0,0,0		},
    { "slay",       do_slay,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    //    { "slay2",		do_slay2,		POS_FIGHTING,	10,	LOG_NORMAL, 0,0,0		},
    { "decapitate", do_decapitate,      POS_STANDING,   3,  LOG_NORMAL, 0,0,0       },
    { "kdecapitate",    do_kdecapitate,     POS_STANDING,   3,  LOG_NORMAL, 0,0,0       },
    { "sset",       do_sset,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "transfer",   do_transfer,        POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "transport",  do_transport,       POS_DEAD,   1,  LOG_NORMAL, 0,0,0       },
    { "at",     do_at,          POS_DEAD,   9,  LOG_NORMAL, 0,0,0       },
    ...
    { "invis",      do_invis,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "log",        do_log,         POS_DEAD,   10, LOG_ALWAYS, 0,0,0       },
    { "memory",     do_memory,      POS_DEAD,   11, LOG_NORMAL, 0,0,0       },
    { "mfind",      do_mfind,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "mstat",      do_mstat,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "mwhere",     do_mwhere,      POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "owhere",     do_owhere,      POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "ofind",      do_ofind,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "sfind",      do_sfind,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "ostat",      do_ostat,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "oswitch",    do_oswitch,     POS_DEAD,   12, LOG_NORMAL, 0,0,0       },
    { "oreturn",    do_oreturn,     POS_DEAD,   12, LOG_NORMAL, 0,0,0       },
    { "rfind",      do_rfind,       POS_DEAD,   8,  LOG_NORMAL, 0, 0, 0},
    { "peace",      do_peace,       POS_DEAD,   9,  LOG_NORMAL, 0,0,0       },
    { "remtrans",   do_remtrans,        POS_DEAD,   9,  LOG_NORMAL, 0,0,0 },
    { "warn",       do_warn,        POS_DEAD,   10, LOG_NORMAL, 0,0,0 },
    { "unshadow",   do_unshadow,        POS_DEAD,   8,  LOG_NORMAL, 0,0,0 },
    { "cecho",      do_cecho,       POS_DEAD,   10, LOG_NORMAL, 0,0,0 },
    { "crecho",     do_crecho,      POS_DEAD,   10, LOG_NORMAL, 0,0,0 },
    { "recho",      do_recho,       POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "immreturn",  do_return,      POS_DEAD,   12, LOG_NORMAL, 0,0,0       },
    { "rstat",      do_rstat,       POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "slookup",    do_slookup,     POS_DEAD,   8,  LOG_NORMAL, 0,0,0       },
    { "snoop",      do_snoop,       POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "switch",     do_switch,      POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "affremove",  do_affremove,       POS_DEAD,   11, LOG_NORMAL, 0,0,0       },
    ...
    { "gemote",     do_gemote,      POS_DEAD,   0,  LOG_NORMAL, 0,0,0       },
    { "sedit",      do_sedit,       POS_DEAD,   7,  LOG_NORMAL, 0,0,0       },
    { "wrlist",     do_wrlist,      POS_DEAD,   7,  LOG_NORMAL, 0,0,0       },
    { "cprime",     do_cprime,      POS_STANDING,   11, LOG_NORMAL, 0,0,0       },

    /* New Database shit - JOBO */
    //    { "reason", 	do_reason,		POS_STANDING,	0,	LOG_ALWAYS, 0,0,0		},
    { "prename",    do_prename,     POS_STANDING,   11, LOG_ALWAYS, 0,0,0       },
    { "mobimplode", do_mobimplode,      POS_STANDING,   12, LOG_ALWAYS, 0,0,0       },
    { "neededhelpfiles",do_neededhelpfiles, POS_STANDING,   0,  LOG_NEVER,  0,0,0       },
    { "questfind",  do_questfind,       POS_STANDING,   3,  LOG_NEVER,  0,0,0       },
    { "showkills",  do_showkills,       POS_STANDING,   0,  LOG_NEVER,  0,0,0       },
    { "build",      do_build,       POS_STANDING,   0,  LOG_NEVER,  0,0,0       },
    { "renounce",   do_renounce,        POS_STANDING,   3,  LOG_ALWAYS, 0,0,0       },
    { "defect",     do_defect,      POS_STANDING,   3,  LOG_ALWAYS, 0,0,0       },
    { "mudannounce",    do_mudannounce,     POS_STANDING,   0,  LOG_NEVER,  0,0,0       },
    { "bank",       do_economytest,     POS_STANDING,   3,  LOG_NORMAL, 0,0,0       },
    { "pkbuy",      do_pkbuy,       POS_STANDING,   3,  LOG_ALWAYS, 0,0,0       },
    { "history",    do_lastmessage,     POS_STANDING,   3,  LOG_NEVER,  0,0,0       },
    { "pkedit",     do_pkedit,      POS_STANDING,   13, LOG_ALWAYS, 0,0,0       },
    { "deforge",    do_deforge,     POS_STANDING,   3,  LOG_NEVER,  0,0,0       },
    { "immlocate",  do_immlocate,       POS_STANDING,   10, LOG_NEVER,  0,0,0       },
    //    { "autoinvade",	do_autoinvade,		POS_STANDING,	10,	LOG_ALWAYS, 0,0,0		},
    { "invade",     do_invade,      POS_STANDING,   10, LOG_ALWAYS, 0,0,0       },
    { "scatter",    do_scatter,     POS_STANDING,   11, LOG_ALWAYS, 0,0,0       },
    { "oscatter",   do_oscatter,        POS_STANDING,   10, LOG_ALWAYS, 0,0,0       },

    { "randomeq", random_eq, POS_STANDING, 13, LOG_ALWAYS, 0,0,0 },

    { "randomload", do_randomload,      POS_STANDING,   10, LOG_ALWAYS, 0,0,0       },
    { "leader",     do_leader,      POS_STANDING,   2,  LOG_NORMAL, 0,0,0       },
    { "leaderclear",    do_leaderclear,     POS_DEAD,   11, LOG_NORMAL, 0,0,0       },
    { "cleaderclear", do_resetclassleader,    POS_DEAD,       14,     LOG_NORMAL, 0,0,0               },
    { "kingdoms",   do_kingdoms,        POS_DEAD,   1,  LOG_NORMAL, 0,0,0       },
    { "ktimerlist", do_ktimerlist,      POS_DEAD,   3,  LOG_NORMAL, 0,0,0       },
    { "resetktimer",    do_resetktimer,     POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    { "ktalk",      do_ktalk,       POS_DEAD,   2,  LOG_NORMAL, 0,0,0       },
    { "kstats",     do_kstats,      POS_DEAD,   2,  LOG_NORMAL, 0,0,0       },
    { "ktreasury",  do_ktreasury,       POS_DEAD,   2,  LOG_NORMAL, 0,0,0       },
    { "kbuy",       do_kbuy,        POS_DEAD,   11, LOG_NORMAL, 0,0,0       },
    { "kingset",    do_kingset,     POS_DEAD,   10, LOG_NORMAL, 0,0,0       },
    ...
    { "treasury",   do_treasury,        POS_STANDING,   2,  LOG_NORMAL, 0,0,0       },
    { "koutcast",   do_koutcast,        POS_STANDING,   2,  LOG_NORMAL, 0,0,0       },

    /* Spar/Deathmatch Arena stuff added by - Chrono */
    ...
    { "auction",    do_auction,     POS_RESTING,    3,  LOG_NORMAL, 0,0,0       },
    { "fryballs",   do_fryballs,        POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "psycho",     do_psycho,      POS_DEAD,   12, LOG_ALWAYS, 0,0,0       },
    { "fixhead",    do_fixhead,     POS_DEAD,   7,  LOG_NORMAL, 0,0,0       },
    { "arenaopen",  do_openarena,       POS_DEAD,   10, LOG_NORMAL, 0,0,0       },

    /* Rune Master commands */
    { "runecreate", do_runecreate,      POS_STANDING,   3,  LOG_NORMAL, CLASS_RUNEMASTER,0,0},
    { "runetrain",  do_runetrain,       POS_STANDING,   3,  LOG_NORMAL, CLASS_RUNEMASTER,0,0},
    { "runeforge",  do_runeforge,       POS_STANDING,   3,  LOG_NORMAL, CLASS_RUNEMASTER,0,0},
    { "runeweave",  do_runeweave,       POS_STANDING,   3,  LOG_NORMAL, CLASS_RUNEMASTER,0,0},
    { "runecount",  do_runecount,       POS_STANDING,   3,  LOG_NORMAL, CLASS_RUNEMASTER,0,0},
    { "runeread",   do_runeread,        POS_STANDING,   3,  LOG_NORMAL, 0,0,0       },

    /*
     * Kahn class commands created by Hutoshi.
     */
    { "gaiatalk",   do_gaiatalk,    POS_DEAD,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kahnmaster", do_kahnmaster,  POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kahnform",   do_kahnform,    POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "gaiasight",  do_gaiasight,   POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kstalk",     do_kstalk,  POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "warmth",     do_warmth,  POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "blitz",      do_blitz,   POS_FIGHTING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "darkroar",   do_darkroar,    POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "clawhold",   do_clawhold,    POS_FIGHTING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "gaiabless",  do_gaiabless,   POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "savageshred",    do_savageshred, POS_FIGHTING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "stealth",    do_stealth, POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "ambush",     do_ambush,  POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kahnfangs",  do_kahnfangs,   POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kahnclaws",  do_kahnclaws,   POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kahnawe",    do_kahnawe, POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kicetouch",  do_kicetouch,   POS_FIGHTING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "clawslash",  do_clawslash,   POS_FIGHTING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kahns",      do_kahns,   POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },
    { "kahneq",     do_kahneq,  POS_STANDING,   3,  LOG_NORMAL, CLASS_KAHN, 0,0 },

    /*
     * Elf class commands created by Hutoshi.
     */
    { "elfbasics",  do_elfbasics,       POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elfmagetrain",   do_elfmagetrain,    POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elfwartrain",    do_elfwartrain,     POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elfarctrain",    do_elfarctrain,     POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elfprofession",  do_elfprofession,   POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elveneyes",  do_elveneyes,       POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elvenears",  do_elvenears,       POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elvenintuition", do_elvenintuition,  POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elvenprotection",do_shield,      POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elvenvanish",    do_elvenvanish,     POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elvenstrength",  do_elvenstrength,   POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elftalk",    do_elftalk,     POS_DEAD,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elfarmor",   do_elfarmor,        POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elfblink",   do_elfblink,        POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    //  Elf Mage
    { "earthbless", do_earthbless,      POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "naturestouch",   do_naturestouch,    POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "naturesfury",    do_naturesfury,     POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "firebless",  do_firebless,       POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "flamering",  do_flamering,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "firecurse",  do_firecurse,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "waterbless", do_waterbless,      POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "waterwall",  do_waterwall,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "icetouch",   do_icetouch,        POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "stonecreate",    do_stonecreate,     POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elfstones",  do_elfstones,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    // Elf Warrior
    { "elfweb",     do_web,         POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "swordslash", do_swordslash,      POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "wishsong",   do_wishsong,        POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    // Elf Archer
    { "webarrow",   do_web,         POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "elvenaim",   do_shadowsight,     POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "archersfury",    do_archersfury,     POS_FIGHTING,   3,  LOG_NORMAL, CLASS_ELF,0,0},
    { "archersrage",    do_archersrage,     POS_STANDING,   3,  LOG_NORMAL, CLASS_ELF,0,0},

    /*
     * Wraith class commands by Loial      <<---obsolete class command (old wraiths) -dert
     *
     { "arcanoi",       do_arcanoi,             POS_STANDING,   3,      LOG_NORMAL, CLASS_WRAITH,0,0},
     */

    /*
     * New Wraiths - Dert
     */
    { "classme",    do_classme,         POS_STANDING,   1,  LOG_ALWAYS, 0,0,0        },
    { "spiritsoar", do_spiritsoar,      POS_STANDING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "wraitheq",   do_wraitheq,        POS_STANDING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "ability",    do_ability,         POS_STANDING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "deathslash", do_deathslash,      POS_FIGHTING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "deathaura",  do_deathaura,       POS_STANDING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "soundwave",  do_soundwave,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "shadowform", do_shadowform,      POS_STANDING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "wraithform", do_wraithform,      POS_STANDING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "wraithsight", do_wraithsight,    POS_STANDING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},
    { "dertslash",  do_dertslash,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_WRAITH,0,0},

    /*
     * Dragon class by Hutoshi
     */
    { "dragonarmor",   do_dragonarmor,     POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dgain",     do_dgain,       POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dfly",      do_dfly,        POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dclaws",    do_dclaws,      POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dfangs",    do_dfangs,      POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dsight",    do_dsight,      POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dtail",         do_dtail,       POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dshield",       do_dshield,     POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dgrasp",        do_dgrasp,      POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dshape",        do_dshape,      POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dstealth",      do_dstealth,        POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dscales",       do_dscales,     POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dwings",    do_dwings,      POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dform",         do_dform,               POS_STANDING,   3,      LOG_NORMAL, CLASS_DRAGON,0,0},
    { "dragondive",    do_dragondive,      POS_STANDING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "firebreath",    do_firebreath,      POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "fireblast", do_fireblast,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "icebreath", do_icebreath,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "iceblast",  do_iceblast,        POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "hatredbreath",  do_hatredbreath,    POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "hatredblast",   do_hatredblast,     POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "acidbreath",    do_acidbreath,      POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},
    { "acidblast", do_acidblast,       POS_FIGHTING,   3,  LOG_NORMAL, CLASS_DRAGON,0,0},

    /* 
     * New shit added by Hutoshi
     */

    //{ "weaponbuy",	do_weaponbuy,		POS_STANDING,	3,	LOG_NORMAL, 0,0,0},
    //{ "dojo",		do_dojo,		POS_STANDING,	3,	LOG_NORMAL, 0,0,0},
...
void interpret( CHAR_DATA *ch, char *argument )
{
    ...
    for ( cmd = 0; cmd_table[cmd].name[0] != '\0'; cmd++ )
    {
        ...
        && (can_interpret(ch, cmd) || IS_IMMORTAL(ch) ))
        {
        foundstar = TRUE;
        star++;
        ...
        if ( (pHelp = get_help( ch, agg )) == NULL )
        {
            ...
            bIsHelp = TRUE;
        sprintf( buf, "%s%-15s#n", bIsHelp? "#y" : "#R", cmd_table[cmd].name );
        send_to_char( buf, ch );
        if ( ++col % 5 == 0 )
            send_to_char( "\n\r", ch );
        }
    ...
    if (foundstar && star != 0)
    {
        sprintf(buf, "\n%d command%s found with %d of them not having a helpfile.\n\r", star,
        (star > 1) ? "s" : "", nNumberHelps);
        stc(buf, ch);
    }
    ...
    */
        ...
        ||   cmd_table[cmd].log == LOG_ALWAYS )
    {
        if (!IS_CREATOR(ch) && !IS_NPC(ch))
        {
            ...
            wiznet( NULL, WIZ_COMMANDS, 0, log_buf );
            log_string( log_buf );
        }
    }
