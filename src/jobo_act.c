...
void do_bountylist(CHAR_DATA *ch, char *argument)
{
    ...
    stc("#r==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==- -==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==#n\n\r",ch);
    stc(              "  #CName               Bounty     Status       Generation    Level    Pk Points\n\r#n",ch);
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
        if ( d->character != NULL )
        {
            ...
            if (d->character->generation == 0) fakegen = 1;
            else fakegen = (d->character->generation);
            if (!can_see(ch, d->character)) continue;
            sprintf(buf, "  #G%-15s#n    %5d       %5d           %d           %d          %-6d\n\r",d->character->name, d->character->pcdata->bounty,
                d->character->race,fakegen, d->character->pcdata->upgrade_level+1, d->character->pcdata->pkpoints);
            stc(buf,ch);
        }
    }
    stc("#r==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==- -==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==--==#L**#r==#n\n\r",ch);
...
{
    send_to_char("#y*************************************************#n\n\r",ch);
    send_to_char("#y*#P          This mud runs Another Land v1.3      #y*#n\n\r",ch);
    send_to_char("#y*#0         Jobo Vladd Tarasque Dracknuur         #y*#n\n\r",ch);
    send_to_char("#y*#L  You may acquire a copy of 1.2.5 codebase at: #y*#n\n\r",ch);
    send_to_char("#y*#G  http://www.geocities.com/another_land_mud/   #y*#n\n\r",ch);
    send_to_char("#y*#R     Modified and used under permission        #y*#n\n\r",ch);
    send_to_char("#y*************************************************#n\n\r",ch);
    return;
}
...
void do_runeeq( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char("You must at least upgrade 3 to buy Rune EQ.\n\r",ch);
        return;
    }
    ...
    if ( ch->pcdata->quest < 50000 )
    {
        send_to_char("You don't have the 50K qps needed.\n\r",ch);
        return;
    }
    ...
    if(!IS_IMMORTAL(ch)) ch->pcdata->quest -= 50000;
    obj = create_object(pObjIndex, 50);
    obj->questowner = str_dup(ch->pcdata->switchname);
    obj_to_char(obj, ch);
    ch->pcdata->rune_count++;
    act("You transform the 50K qp into $p.",ch,obj,NULL,TO_CHAR);
    act("$n forms $p out of 50K qp.",ch,obj,NULL,TO_ROOM);
    return;
}
...
{
    char buf[MIL];
    int nDifference = (ch->max_hit-ch->hit)/1000;
    if ( nDifference < 0 ) nDifference *= -1;
    if (IS_NPC(ch)) return;
    if (ch->in_room != NULL && ch->in_room->vnum != ROOM_VNUM_ALTAR && ch->in_room->vnum != 86000 && ch->in_room->vnum != 86010 && ch->in_room->vnum != 86020 && ch->in_room->vnum != 86030 && ch->in_room->vnum != 86099)
    {
    ...
    if (ch->pcdata->quest < nDifference)
    {
        sprintf( buf, "The healer demands %d qps as payment for his service.\n\r",nDifference);
        send_to_char(buf,ch);
        return;
    }
    sprintf( buf, "The healer heals your wounds and takes %d qps as payment.\n\r",nDifference);
    ch->pcdata->quest -= nDifference;
    ch->hit  = ch->max_hit;
    ch->mana = ch->max_mana;
    ch->move = ch->max_move;
    ch->loc_hp[0] = 0;
    ...
    ch->loc_hp[6] = 0;
    send_to_char(buf,ch);
    if( ch->in_room->vnum == 86000 || ch->in_room->vnum == 86010 || ch->in_room->vnum == 86020 || ch->in_room->vnum == 86030 || ch->in_room->vnum ==86099 )
    {
        ch->pcdata->quest -= 50;
        send_to_char( "Kingdom Penalty 50 more Qps.\n\r",ch);
    }
    WAIT_STATE(ch, 36);
    return;
}
...
void do_mudstat( CHAR_DATA *ch, char *argument)
{
    ...
    if (!worldpk)
        stcf(ch,"#LPk is turned #yON!#n\n\r");
    else
        stcf(ch,"#LPk is turned #yOFF!#n\n\r");
    if (worldtest)
        stcf(ch,"#LTest mode is #yON!#n\n\r");
    else
        stcf(ch,"#LTest mode is #yOFF!#n\n\r");

    if (global_exp)
        //    send_to_char("#LHappy Hour is #yON!#n",ch);
        stcf(ch,"#LHappy Hour is #yON!#L for #G%d#L minutes#n",pulse_doubleexp);
    else
        send_to_char("#LHappy Hour is off",ch);
    if (global_qp)
        //    send_to_char("#L and Questors Delight is #yON!#n\n\r\n\r#n",ch);
        stcf(ch,"#L and Questors Delight is #yON!#L for #G%d#L minutes#n\n\r\n\r",pulse_doubleqp);
    else
        send_to_char("#L and Questors Delight is off\n\r\n\r#n",ch);
    if (ragnarok)
        send_to_char("#CAPOCALYPSE MODE!!!!!!!#n  Everyone can kill everyone (HELP Apocalypse)\n\r\n\r",ch);
    else
    {
        sprintf(buf,"#CApocalypse is still %d qps from happening. Ragnarok <Amount> to bid.\n\r\n\r",ragnarok_cost);
        send_to_char(buf,ch);
    }
...
void do_pkscry(CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char("No no, No scry in Kingdom HQ.\n\r",ch);
        ch->fight_timer += 6;
        return;
    }
    if (IS_NPC(victim))
...
void do_pkheal(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_NPC(ch)) return;
    if ((ratio = ch->pcdata->upgrade_level) > 1)
    {
        send_to_char("huh?\n\r",ch);
        return;
    }
    ...
    send_to_char("#LA wave of harmony and peace flows over you.#n\n\r",ch);
    act("#LA wave of harmony and peace flows over $n.#n",ch,NULL,NULL,TO_ROOM);
    WAIT_STATE(ch,(12-level));
    if ( ch->fighting ) WAIT_STATE(ch,6);
    return;
}
...
void do_mastery(CHAR_DATA *ch, char *argument)
{
    ...
    else if (IS_CLASS(ch, CLASS_WIZARD)) vnum = 103114;
    else if (IS_CLASS(ch, CLASS_DRAGON)) vnum = 40964;
    else if (IS_CLASS(ch, CLASS_LICH)) vnum = 33233;
    else if (IS_CLASS(ch, CLASS_ELF)) vnum = 33313;
    else if (IS_CLASS(ch, CLASS_KAHN)) vnum = 33293;
    else if (IS_CLASS(ch, CLASS_MILITARY)) vnum = 110988;
    else if (IS_CLASS(ch, CLASS_PHOENIX)) vnum = 111000;
    else if (IS_CLASS(ch, CLASS_WRAITH)) vnum = 33333;
    ...
    {
        send_to_char("Missing object, inform a Coder.\n\r",ch);
        return;
    }
    obj = create_object(pObjIndex, 50);
    obj_to_char(obj,ch);
...
void do_setstance(CHAR_DATA *ch, char *argument)
{
    ...
    {
        currentstance = 23; max_lesser = 4;
        max_greater = 4; max_supreme = 2; min_cost = 200; max_cost = 400;
    }
...
void do_ragnarok( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char("You may not call for Apocalypse yet.\n\r",ch);
        return;
    }
    if (has_timer(ch)) return;
    if (arg[0] == '\0')
    ...
    if (!is_number(arg))
    {
        send_to_char("A number please, how many qps do you want to bid towards Apocalypse.\n\r",ch);
        return;
    }
    amount = atoi(arg);
    if (amount < 100)
    ...
    if (ragnarok_cost <= 0)
    {
        ...
        do_info(ch,"#0The world comes to an end, #yApocalypse#0 is over us all!!!!#n");
    }
    else do_info(ch,"The Apocalypse moves closer, the gods shiver with fear");
    return;
}
...
void do_timer( CHAR_DATA *ch, char *argument)
{
    ...
    if (!ragnarok)
    {
        sprintf(buf,"Bidding on apocalypse starts in :  %d hour and %d minutes.\n\r",
    ...
    stcf(ch,"Autoquest might happen in : %d minutes.\n\r",pulse_autoquest);
    return;
}
...
void do_favor( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(arg,"left"))
    {
        if ( !IS_SET(ch->act, PLR_LEFTHAND) ) SET_BIT(ch->act, PLR_LEFTHAND);
        if ( IS_SET(ch->act, PLR_RIGHTHAND) ) REMOVE_BIT(ch->act,PLR_RIGHTHAND );
        if ( IS_SET(ch->act, PLR_AMBI) ) REMOVE_BIT(ch->act,PLR_AMBI );
        send_to_char("You favor your left arm in combat.\n\r",ch);
    }
    else if (!str_cmp(arg,"right"))
    {
        if ( !IS_SET(ch->act, PLR_RIGHTHAND) ) SET_BIT(ch->act, PLR_RIGHTHAND);
        if ( IS_SET(ch->act, PLR_LEFTHAND) ) REMOVE_BIT(ch->act,PLR_LEFTHAND );
        if ( IS_SET(ch->act, PLR_AMBI) ) REMOVE_BIT(ch->act,PLR_AMBI );
    ...
    else if (!str_cmp(arg,"both"))
    {
        if ( IS_SET(ch->act, PLR_LEFTHAND) ) REMOVE_BIT(ch->act, PLR_LEFTHAND);
        if ( IS_SET(ch->act, PLR_RIGHTHAND) ) REMOVE_BIT(ch->act,PLR_RIGHTHAND );
        if ( !IS_SET(ch->act, PLR_AMBI) ) SET_BIT(ch->act,PLR_AMBI );
...
void do_setdecap(CHAR_DATA *ch, char *argument)
{
    ...
    smash_tilde(argument);
    if (strlen(argument) < 10 || strlen(argument) > 140)
    {
        send_to_char("Between 10 and 140 chars... thanks.\n\r",ch);
        return;
    }
