...
void load_leaderboard()
{
    ...
    if ((fp = fopen("../txt/leader.txt", "r")) == NULL)
    {
        ...
        log_string("Error: leader.txt does not exist, making one.");
        fprintf(fp, "None~\n");
        fprintf(fp, "0\n");
        fprintf(fp, "None~\n");
        fprintf(fp, "0\n");
    ...
    leader_board.ak_name = fread_string(fp);
    leader_board.ak_number = fread_number(fp);
    leader_board.ad_name = fread_string(fp);
    leader_board.ad_number = fread_number(fp);
...
void save_leaderboard()
{
    ...
    fprintf(fp, "%s~\n", leader_board.ak_name);
    fprintf(fp, "%d\n", leader_board.ak_number);
    fprintf(fp, "%s~\n", leader_board.ad_name);
    fprintf(fp, "%d\n", leader_board.ad_number);
    fclose (fp);
}
...
void do_leader( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char("   #oArena Master     #C--->    ",ch);
    stcf(ch, "#G%-13s",leader_board.ak_name);
    stcf(ch, " #owith #G%d #oarena kills#n\n\r",leader_board.ak_number);

    send_to_char("   #oArena Jester     #C--->    ",ch);
    stcf(ch, "#G%-13s",leader_board.ad_name);
    stcf(ch, " #owith #G%d #oarena deaths#n\n\r",leader_board.ad_number);
...
void check_leaderboard( CHAR_DATA *ch )
{
    ...
    if (ch->pcdata->awins > leader_board.ak_number)
    {
        leader_board.ak_number = ch->pcdata->awins;
        free_string(leader_board.ak_name);
        leader_board.ak_name = str_dup(ch->name);
        changed = TRUE;
    }
    if (ch->pcdata->alosses > leader_board.ad_number)
    {
        leader_board.ad_number = ch->pcdata->alosses;
        free_string(leader_board.ad_name);
        leader_board.ad_name = str_dup(ch->name);
        changed = TRUE;
    }
...
void do_leaderclear(CHAR_DATA *ch, char *argument )
{
    ...
    free_string(leader_board.ak_name);
    leader_board.ak_name = str_dup("Noone");
    leader_board.ak_number = 0;
    free_string(leader_board.ad_name);
    leader_board.ad_name = str_dup("Noone");
    leader_board.ad_number = 0;
    save_leaderboard();
