...
void do_gensteal(CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char("You're in a nodecap zone.\n\r",ch);
        return;
    }
    if (ragnarok)
    {
        send_to_char("Not while apocalypse is running.\n\r",ch);
        return;
    }
    if (arg[0] == '\0')
    {
    ...
    if (ch->generation == 0)
    {
        send_to_char("You are the lowest possible generation.\n\r",ch);
        return;
    }
    /* if (ch->race - 3 > victim->race)
     {
       send_to_char("They are not in range genius!\n\r",ch);
       return;
     }*/
