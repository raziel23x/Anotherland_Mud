    ...
    for ( d = descriptor_list; d != NULL; d = d->next )
    {
        ...
        for ( d2 = d->next; d2 != NULL; d2 = d2->next )
        {
            ...
            if (!str_cmp(d->host, d2->host))
            {
                ...
                if ( d2->character->level < 10 && d->character->level < 10
                    && str_cmp(d->character->name,"Envy")
                    && str_cmp(d->character->name,"Barenth")
                    && str_cmp(d->character->name,"Serena")
                    && str_cmp(d->character->name,"Omega")
                    && str_cmp(d->character->name,"Gandalf")
                    && str_cmp(d->character->name,"Deltoid")
                    && str_cmp(d->character->name,"Maserka")
                    && str_cmp(d->character->name,"Quiana")
                    && str_cmp(d->character->name,"Demonhunter")
                    && str_cmp(d->character->name,"Granite")
                    && str_cmp(d->character->name,"Drackus" )
                    && str_cmp(d->character->name,"Draven" )
                    && str_cmp(d->character->name,"Zin")
                    && str_cmp(d->character->name,"Xavin") )
                {
                    SET_BIT(d2->character->act, PLR_DENY);
                    SET_BIT(d->character->act, PLR_DENY);
                    bDenied = TRUE;
                }
                if ( bDenied == TRUE )
                {
                    sprintf( buf, "#rHas been denied: #R%s and %s for multiplaying#r.#n", d->character->name, d2->character->name );
                    make_note( "Immortal", d->character->name, "imm", "Frozen", 5, buf );
                    send_to_char(buf,ch);
                }
            }
        }
...
/* Stupid ass command anyways -Beorn
...
{
    if (IS_NPC(ch)) { stc("Your a mob.\n\r",ch); return;}
    if (arenaopen) { stc("Arena already open.\n\r",ch); return;}
    open_arena();
    return;
}
