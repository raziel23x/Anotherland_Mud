...
void do_train( CHAR_DATA *ch, char *argument )
{
 ...
 if (IS_CLASS(ch, CLASS_RPONLY)){
  ...
  if (!str_cmp(arg1,"hp"))
  {
    ...
    int urin = 0;
    int urin_counter = 0;
    int statcap = UMIN(1000000, (200000 + (4000 * ch->race)));
    statcap = statcap + (statcap*get_curr_con(ch)/250);
...
if (!str_cmp(arg1, "magic"))
    ...
    {
        if (last) send_to_char( "You don't have enough qps see help stateffect.\n\r", ch );
        return;
    }
