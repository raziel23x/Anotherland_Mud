...
void other_age( CHAR_DATA *ch, int extra, bool is_preg, char *argument )
{
    ...
    {
        if (yy > 0 || (dda > 0 && mm >= 9) || IS_SET(extra, EXTRA_LABOUR))
        {
...
void other_age2( CHAR_DATA *ch, int extra, bool is_preg, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    char buf2[MAX_STRING_LENGTH];
    char mstatus[20];
    ...
    if (da == 1) sprintf(dstatus,"day");
    else sprintf(dstatus,"days");
    if (!is_preg)
    {
        if (!IS_EXTRA(ch, EXTRA_BORN)) yy += 17;
        sprintf(buf,"         #GAge:#7 %d#G years,#7 %d #G%s and #7%d #G%s#n\n\r",
            yy,mm,mstatus,da,dstatus);
        send_to_char(buf,ch);
    }
    else
    {
        if (yy > 0 || (da > 0 && mm >= 9) || IS_SET(extra, EXTRA_LABOUR))
        {
            sprintf(buf,"She is ready to give birth.\n\r");
            send_to_char(buf,ch);
        }
...
void do_consent( CHAR_DATA *ch, char *argument )
{
    ...
    */
    ...
    act("You give $N consent to Fuck your brains out.",ch,NULL,victim,TO_CHAR);
    act("$n has given you consent to Fuck their brains out.",ch,NULL,victim,TO_VICT);
    return;
}
