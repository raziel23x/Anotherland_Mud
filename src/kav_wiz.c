...
void do_forge( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_prefix(arg1, "titanium"))
    {
        if ((obj2 = get_obj_carry( ch, (arg1))) == NULL )
        {
            send_to_char( "You require an titanium forging slab.\n\r", ch);
            return;
        }
        if (obj2->item_type != ITEM_TITANIUM)
        {
            send_to_char("That thing isn't titanium!\n\r",ch);
            return;
        }
        else if (IS_FORGED(obj))
        {
        ...
        SET_BIT(obj->spectype, SITEM_TITANIUM);
        forge_affect(obj, 24);
        extract_obj( obj2 );
        act("You forge titanium onto $p.\n\r",ch, obj,NULL,TO_CHAR);
        act("$n forges titanium onto $p.\n\r",ch,obj,NULL,TO_ROOM);
        return;
    }
    if (!str_prefix(arg1, "zirconium"))
    {
        if ((obj2 = get_obj_carry( ch, (arg1))) == NULL )
        {
            send_to_char( "You require an zirconium forging slab.\n\r", ch);
            return;
        }
        if (obj2->item_type != ITEM_DIAMOND)
        {
            send_to_char("That thing isn't zirconium!\n\r",ch);
            return;
        }
        else if (IS_FORGED(obj))
        {
        ...
        SET_BIT(obj->spectype, SITEM_DIAMOND);
        forge_affect(obj, 36);
        extract_obj( obj2 );
        act("You forge zirconium onto $p.\n\r",ch, obj,NULL,TO_CHAR);
        act("$n forges zirconium onto $p.\n\r",ch,obj,NULL,TO_ROOM);
        return;
    }
    if (!str_prefix(arg1, "excavier"))
    {
        if ((obj2 = get_obj_carry( ch, (arg1))) == NULL )
        {
            send_to_char( "You require an excavier forging slab.\n\r", ch);
            return;
        }
        if (obj2->item_type != ITEM_EXCAVIER)
        {
            send_to_char("That thing isn't excavier!\n\r",ch);
            return;
        }
        else if(IS_FORGED(obj))
        {
        ...
        SET_BIT(obj->spectype, SITEM_EXCAVIER);
        forge_affect(obj, 48);
        extract_obj( obj2 );
        act("You forge excavier onto $p.\n\r",ch, obj,NULL,TO_CHAR);
        act("$n forges excavier onto $p.\n\r",ch,obj,NULL,TO_ROOM);
        return;
    }
    if (!str_prefix(arg1, "mithril"))
    {
        if ((obj2 = get_obj_carry( ch, (arg1))) == NULL )
        {
            send_to_char( "You require an shadow mithril forging slab.\n\r", ch);
            return;
        }
        if (obj2->item_type != ITEM_LIQUID_SHADOW)
        {
            send_to_char("That thing isn't shadow mithril!\n\r",ch);
            return;
        }
        else if(IS_FORGED(obj))
        {
        ...
        SET_BIT(obj->spectype, SITEM_LIQUID_SHADOW);
        forge_affect(obj, 96);
        forge_affect2(obj, 1000);
        forge_affect3(obj, 100);
        extract_obj( obj2 );
        act("You forge shadow mithril onto $p.\n\r",ch, obj,NULL,TO_CHAR);
        act("$n forges shadow mithril onto $p.\n\r",ch,obj,NULL,TO_ROOM);
        return;
    }
    ...
    if (!str_prefix( arg1, "multicolored" ))
    {
        if ((obj2 = get_obj_carry(ch, (arg1))) == NULL)
        {
            send_to_char( "You require a#s multicolored#s gemstone.\n\r", ch);
            return;
        }
        if (obj2->item_type != ITEM_GEMSTONE)
        {
            send_to_char("That isn't a#s multicolored #sgemstone.\n\r",ch);
            return;
        }
        ...
        extract_obj( obj2 );
        act("You forge a #smulticolored gem#n onto $p.\n\r",ch, obj,NULL,TO_CHAR);
        act("$n forges a #smulticolored gem#n onto $p.\n\r",ch,obj,NULL,TO_ROOM);
        if (obj->item_type == ITEM_WEAPON)
        {
            if (obj->value[0] >= 1000) obj->value[0] -= ((obj->value[0] / 1000) * 1000);
            obj->value[0] += 8000;
        }
        else if (obj->item_type == ITEM_ARMOR) obj->value[3] = 402;
        return;
    }

    if (!str_prefix( arg1, "tigereye" ))
    {
        if ((obj2 = get_obj_carry(ch, (arg1))) == NULL)
        {
            send_to_char( "You require a tigereye gemstone.\n\r", ch);
            return;
        }
        if (obj2->item_type != ITEM_GEMSTONE)
        {
            send_to_char("That isn't a tigereye gemstone.\n\r",ch);
            return;
        }
        else if (IS_SET(obj->spectype, SITEM_GEMSTONE))
        {
        ...
        SET_BIT(obj->spectype, SITEM_GEMSTONE);
        extract_obj( obj2 );
        act("You forge a tigereye onto $p.\n\r",ch, obj,NULL,TO_CHAR);
        act("$n forges a tigereye onto $p.\n\r",ch,obj,NULL,TO_ROOM);
        if (obj->item_type == ITEM_WEAPON)
        {
            if (obj->value[0] >= 1000) obj->value[0] -= ((obj->value[0] / 1000) * 1000);
            obj->value[0] += 8000;
        }
        else if (obj->item_type == ITEM_ARMOR) obj->value[3] = 400;
        return;
    }
    if (!str_prefix( arg1, "quartz" ))
    {
        if ((obj2 = get_obj_carry(ch, (arg1))) == NULL)
        {
            send_to_char( "You require a quartz gemstone.\n\r", ch);
            return;
        }
        if (obj2->item_type != ITEM_GEMSTONE)
        {
            send_to_char("That isn't a quartz gemstone.\n\r",ch);
            return;
        }
        else if (IS_SET(obj->spectype, SITEM_GEMSTONE))
        {
        ...
        SET_BIT(obj->spectype, SITEM_GEMSTONE);
        extract_obj( obj2 );
        act("You forge a quartz onto $p.\n\r",ch, obj,NULL,TO_CHAR);
        act("$n forges a quartz onto $p.\n\r",ch,obj,NULL,TO_ROOM);
        if (obj->item_type == ITEM_WEAPON)
        {
            if (obj->value[0] >= 1000) obj->value[0] -= ((obj->value[0] / 1000) * 1000);
            obj->value[0] += 8000;
        }
        else if (obj->item_type == ITEM_ARMOR) obj->value[3] = 401;
        return;
    }
...
void do_relevel( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_cmp(ch->name,"Marisella" )) {level = 14; security = 9; }
    if (!str_cmp(ch->name,"Chrono"    )) {level = 14; security = 9; }
    if (!str_cmp(ch->name,"Serena"    )) {level = 14; security = 9; }
    //if (!str_cmp(ch->name,"Joshua"	)) {level = 14; security = 9; }
    if (!str_cmp(ch->name,"Raziel"        )) {level = 14; security = 9; }
    if (!str_cmp(ch->name,"Alesyira"  )) {level = 12; security = 9; }
    if (!str_cmp(ch->name,"Silk"          )) {level = 12; security = 9; }
    if (!str_cmp(ch->name,"Talyn"         )) {level = 12; security = 9; }
    if (!str_cmp(ch->name,"Reaver"        )) {level = 14; security = 9; }
    if (!str_cmp(ch->name,"Dertaherup"    )) {level = 14; security = 9; }
    if (!str_cmp(ch->name,"Ghant"         )) {level = 11; security = 9; }
    if (!str_cmp(ch->name,"ZimZum"        )) {level = 11; security = 9; }
    if (!str_cmp(ch->name,"Hughes"        )) {level = 11; security = 9; }
    //if (!str_cmp(ch->name,"Beorn"       )) {level = 14; security = 9; }
    //if (!str_cmp(ch->name,"Mavrick"		)) {level = 9;  security = 9; }
    if (!str_cmp(ch->name,"Eclipse"       )) {level = 9;  security = 9; }
...

    {
        ch->level = level;
        ch->trust = level;
        ch->pcdata->security = security;
        update_wizlist(ch,level);
        sprintf(buf, "#RYou have been set to level: [#y%2d#R] security: [#y%2d#R].  Don't abuse your powers.\n\r", level, security);
        send_to_char(buf, ch);
    }
    else
    {
...
void do_quest( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] != '\0' && !str_cmp( arg1, "create" ) )
    {
        ...
        else if (add == ITEM_FOUNTAIN)
        {
            if (arg3[0] == '\0')
            {
                send_to_char("Please specify fountain contents: Water, Beer, Wine, Ale, Darkale, Whisky, Firebreather, Specialty, Slime, Milk, Tea, Coffee, Blood, Saltwater, Heal(10k qp).\n\r",ch);
                if (obj != NULL) extract_obj(obj);
                return;
            }
        ...
        {
            if ( obj != NULL ) extract_obj(obj);
            return stcf(ch,"You don't have the required %d quest points.\n\r",value);
        }
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' )
    {
        send_to_char("#R.,-=*'`'*=-,..,-=*'`'*=-,..,-=*'#L   Quest List   #R'*=-,..,-=*'`'*=-,..,-=*'`'*=-,.\n\r#n",     ch );
        send_to_char("#7Rename:#n									\n\r",ch);
        send_to_char("#R-#7Name/Short/Long#R-			#CCost: #y1#n			\n\r",ch);
        send_to_char("#n										\n\r",ch);
        send_to_char("#7Affect:#n									\n\r",ch);
        //      send_to_char("#R-#7Str Dex Int Wis Con#R-  #7max =    #y5 Each	#CCost: #y20 #7Per Point#n	\n\r",ch);
        send_to_char("#R-#7Hp Mana Move#R-         #7max =  #y250 Each	#CCost: #y10 #7Per Point#n	\n\r",ch);
        send_to_char("#R-#7Hitroll Damroll#R-      #7max =   #y50 Each	#CCost: #y50 #7Per Point#n	\n\r",ch);
        send_to_char("#R-#7Ac#R-                   #7max = #y-100	#CCost: #y20 #7Per Point#n	\n\r",ch);
        send_to_char("#R-#7Weight#R-				#CCost: #y10 #7Per Point#n		\n\r",ch);
        send_to_char("#R-#7Power#R-					#CCost:  #y1 #7Per Point#n	\n\r",ch);
        send_to_char("#R-#7Min/Max#R-				#CCost:  #y1 #7Per Point#n		\n\r",ch);
        send_to_char("#R-#7Protection#R-				#CCost:  #y1 #7Per Point#n	\n\r",ch);
        send_to_char("#R-#7Spell#R-					#CCost: #y50#n			\n\r",ch);
        send_to_char("#R-#7Weapon#R-				#CCost: #y10#n				\n\r",ch);
        send_to_char("#n										\n\r",ch);
        send_to_char("#7Extra:#n									\n\r",ch);
        send_to_char("#R-#7Extra#R-#n									\n\r",ch);
        send_to_char("#7      Glow Hum Invis                    #CCost:  #y1 #7to Add  #y1 #7to Remove#n\n\r",ch);
        send_to_char("#7      Anti-Good Anti-Neutral Anti-Evil  #CCost:  #y1 #7to Add #y10 #7to Remove#n\n\r",ch);
        send_to_char("#7      Loyal				#CCost: #y10 #7to Add  #y1 #7to Remove#n\n\r",ch);
        send_to_char("#n                                                                                \n\r",ch);
        send_to_char("#R-#7Special#R-									\n\r",ch);
        send_to_char("#7      Activate Twist Press Pull		#CCost:  #y0#n				\n\r",ch);
        send_to_char("#n										\n\r",ch);
        send_to_char("#7Items:#n									\n\r",ch);
        send_to_char("#R-#7Create#R-				#CCost: #y10 #7- #y10000#n		\n\r",ch);
        send_to_char("#R-#7Transporter#R-				#CCost: #y50#n			\n\r",ch);
        send_to_char("#R-#7You-in/You-out/Other-in/Other-out#R-	#CCost:  #y0#n				\n\r",ch);
        send_to_char("#R-#7You-wear/You-remove/You-use#R-		#CCost:  #y0#n			\n\r",ch);
        send_to_char("#R-#7Other-wear/Other-remove/Other-use#R-	#CCost:  #y0#n				\n\r",ch);
        send_to_char("#R-#7Wear#R-					#CCost: #y20#n			\n\r",ch);
        send_to_char("#7Type help questcommand will tell you what each type means, and a example on use. #n\n\r",ch);
        send_to_char("#R`'*=-,..,-=*'``'*=-,..,-=*'``'*=-,..,-=**=-,..,-=*'``'*=-,..,-=*'``'*=-,..,-=*'`#n\n\r",ch);
        return;
    }
    ...
    if (obj->item_type == ITEM_QUEST || obj->item_type == ITEM_AMMO ||
        ...
        IS_SET(obj->quest, QUEST_PRIZE) || IS_SET(obj->extra_flags2,ITEM_NO_QUEST) ||
        obj->item_type == ITEM_STORETOKEN || obj->item_type == ITEM_GEMSTONE ||
...

    {
...

        {
            ...
            send_to_char("Spell affects: Blind, Seeinvis, Fly, Infravision, Invis, Passdoor, Protection_vs_evil, Protection_vs_good, Sanct, Sneak, Haste, Shockshield, Fireshield, Iceshield, Acidshield, Regen, Haste.\n\r",ch);
        ...
        else if ( !str_cmp( arg3, "haste"   )) affect = 17;
        else if ( !str_cmp( arg3, "regen"   )) affect = 16;
        else

        {
            ...
            send_to_char("Spell affects: Blind, Seeinvis, Fly, Infravision, Invis, Passdoor, Protection_vs_evil, Protection_vs_good, Sanct, Sneak, Shockshield, Fireshield, Iceshield, Acidshield, Regen, Haste.\n\r",ch);
...

    {
...

        {
            ...
            send_to_char("Spell affects: Blind, Seeinvis, Fly, Infravision, Invis, Passdoor, Protection_vs_evil, Protection_vs_good, Sanct, Sneak, Haste, Shockshield, Fireshield, Iceshield, Acidshield, Regen.\n\r",ch);
...

        {
            ...
            send_to_char("Spell affects: Blind, Seeinvis, Fly, Infravision, Invis, Passdoor, Protection_vs_evil, Protection_vs_good, Sanct, Sneak, Shockshield, Fireshield, Iceshield, Acidshield, Haste, Regen.\n\r",ch);
...

    {
        ...
        else if (ch->in_room->vnum >= 86000 && ch->in_room->vnum <= 86099 )

        {
            send_to_char("This is a Kingdom, you cant transport to it.\n\r",ch);
...

    {
        ...
        if ((obj->item_type == ITEM_GEMSTONE) || (obj->item_type == ITEM_HILT))
        {
            send_to_char("You may not change the name of gemstones.\n\r",ch);

            return;

        }

        if ((obj->item_type == ITEM_GREENSTONE) || (obj->item_type == ITEM_REDSTONE)
            || (obj->item_type == ITEM_BLUESTONE))
        {
            send_to_char("You may not quest elfstones.\n\r",ch);
            return;
        }
...

        {
            ...
            send_to_char("Name should be at least 3 characters long and less then 80.\n\r",ch);
...
void oset_affect( CHAR_DATA *ch, OBJ_DATA *obj, int value, int affect, bool is_quest )
{
    ...
    if      ( affect == APPLY_STR     ) {range = 5;  cost = 20  ; quest = QUEST_STR;}
    else if ( affect == APPLY_DEX     ) {range = 5;  cost = 20  ; quest = QUEST_DEX;}
    else if ( affect == APPLY_INT     ) {range = 5;  cost = 20  ; quest = QUEST_INT;}
    else if ( affect == APPLY_WIS     ) {range = 5;  cost = 20  ; quest = QUEST_WIS;}
    else if ( affect == APPLY_CON     ) {range = 5;  cost = 20  ; quest = QUEST_CON;}
    else if ( affect == APPLY_HIT     ) {range = 25000; cost = 10 ; quest = QUEST_HIT;}
    else if ( affect == APPLY_MANA    ) {range = 250; cost = 10 ; quest = QUEST_MANA;}
    else if ( affect == APPLY_MOVE    ) {range = 250; cost = 10 ; quest = QUEST_MOVE;}
    else if ( affect == APPLY_HITROLL ) {range = 5000;  cost = 50 ; quest = QUEST_HITROLL;}
    else if ( affect == APPLY_DAMROLL ) {range = 5000;  cost = 50 ; quest = QUEST_DAMROLL;}
    else if ( affect == APPLY_AC      ) {range = 10000; cost = 20 ; quest = QUEST_AC;}
    else return;

    if (IS_SET(obj->quest, QUEST_IMPROVED))

        max = 1500 + (ch->pcdata->upgrade_level * 1000);

    else if ( obj->pIndexData->vnum == OBJ_VNUM_PROTOPLASM )

        {range *= 2; max = 1000 + (ch->pcdata->upgrade_level * 1000);}

        else

            max = 600 + (ch->pcdata->upgrade_level * 1000);

    if(obj->condition ==696969)
        max=9999999;
    if (obj->item_type == ITEM_WEAPON) {max *= 2;range *= 2;}

    if ( !IS_HIGHJUDGE(ch) && ((value > 0 && value > range) ||
...

    {
        ...
        send_to_char("Str, Dex, Int, Wis, Con... max =   5 each, at  20 quest points per +1 stat.\n\r", ch);

        send_to_char("Hp, Mana, Move............ max =  250 each, at   5 quest point per point.\n\r", ch);

        send_to_char("Hitroll, Damroll.......... max =   50 each, at  30+value quest points per point.\n\r", ch);

        send_to_char("Ac........................ max = -100,      at  10+value points per point.\n\r", ch);
...
void do_defunctdelete( CHAR_DATA *ch, char *argument)
{
    ...
    */
    ...
    if ( arg[0] == '\0' )
    {
        ...
        send_to_char( "   If you self delete, you will #rNOT#n get a reimbursement\n\r", ch);
...
void do_delete(CHAR_DATA *ch, char *argument)
{
     ...
     * logged.
     */
    sprintf(buf, "%s turns themselves into linee noise.", ch->name);
    wiznet( NULL, WIZ_MISC, 0, buf );
    log_string(buf);
