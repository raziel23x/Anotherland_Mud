...
void do_planeshift( CHAR_DATA *ch, char *argument )
{
    ...
    act("$n fades almost out of existance.", ch, NULL, NULL, TO_ROOM);
    return;
}
...
void do_pentagram(CHAR_DATA *ch, char *argument )
{
    ...
    if ((victim = get_char_world(ch, arg)) == NULL
        ||   victim == ch
        ||   victim->in_room == NULL
        ||   ch->in_room->area->vnum == 6
        ||   (!IS_NPC(victim) && !IS_IMMUNE(victim, IMM_SUMMON))
        ||   IS_SET(ch->in_room->room_flags, ROOM_PRIVATE)
        ||   IS_SET(ch->in_room->room_flags, ROOM_SOLITARY)
        ||   IS_SET(ch->in_room->room_flags, ROOM_ASTRAL)
        ||   IS_SET(ch->in_room->room_flags, ROOM_NOTRANS)
        ||   IS_SET(victim->in_room->room_flags, ROOM_NOTRANS)
        ||   IS_SET(victim->in_room->room_flags, ROOM_ASTRAL)
        ||   IS_SET(victim->in_room->room_flags, ROOM_SAFE)
        ||   IS_SET(victim->act, ACT_DUMMY)
        ||   IS_SET(victim->act, ACT_NOPENT)
        ||   victim->in_room->vnum == ch->in_room->vnum)
    {
        send_to_char( "You failed.\n\r", ch );
        return;
    }
    if (victim->level > (ch->spl[1]*2) )
    {
        send_to_char( "The mob is too high of a level for your red spell.\n\r",ch);
        return;
    }
...
void do_soulsuck(CHAR_DATA *ch, char *argument )
{
    ...
    {
        {
            dam = ch->pcdata->powers[SPIRIT] * (number_range(2400,2360) + (ch->pcdata->upgrade_level * 440) + 1920)/ 2;
            if (dam >= 15000) dam = 15000;
            sprintf(buf,"You suck the soul out of $N and use the energy to heal yourself [#C%d#n]",dam);
            act(buf,ch,NULL,victim,TO_CHAR);
            sprintf(buf,"$n sucks away at your soul! [#R%d#n]",dam);
        ...
        if (IS_CLASS(ch, CLASS_LICH)) WAIT_STATE(ch,25);
    }

    else if (IS_CLASS(ch, CLASS_UNDEAD_KNIGHT))
    {
        dam = ch->pcdata->powers[SPIRIT] * (number_range(1500,2000) + (ch->pcdata->upgrade_level * 400) + 1900)/4;
        if (dam >= 15000) dam = 15000;
        sprintf(buf,"You suck the soul out of $N and use the energy to heal yourself [#C%d#n]",dam);
        act(buf,ch,NULL,victim,TO_CHAR);
        sprintf(buf,"$n sucks away at your soul! [#R%d#n]",dam);
    ...
    {
        if (IS_CLASS(ch, CLASS_UNDEAD_KNIGHT)) WAIT_STATE(ch,20);
        // else WAIT_STATE(ch,15);
    }
    return;
}
...
void do_licharmor(CHAR_DATA *ch, char *argument )
{
    ...
    if (arg[0] == '\0')
    {
        send_to_char("Choose which piece of lich armor you wish to make: Scythe Bracer Amulet Ring Plate Helmet Leggings Boots Gauntlets Sleeves Cloak Belt Mask.\n\r",ch);
        return;
    }
    if (ch->pcdata->upgrade_level >= 1)
    {
        if      (!str_cmp(arg,"scythe"              )) vnum = 110150;
        else if (!str_cmp(arg,"ring"                )) vnum = 110151;
        else if (!str_cmp(arg,"amulet"              )) vnum = 110152;
        else if (!str_cmp(arg,"bracer"              )) vnum = 110153;
        else if (!str_cmp(arg,"plate"               )) vnum = 110154;
        else if (!str_cmp(arg,"helmet"              )) vnum = 110155;
        else if (!str_cmp(arg,"leggings"            )) vnum = 110156;
        else if (!str_cmp(arg,"boots"               )) vnum = 110157;
        else if (!str_cmp(arg,"gauntlets"           )) vnum = 110158;
        else if (!str_cmp(arg,"sleeves"             )) vnum = 110159;
        else if (!str_cmp(arg,"cloak"               )) vnum = 110160;
        else if (!str_cmp(arg,"belt"                )) vnum = 110161;
        else if (!str_cmp(arg,"mask"                )) vnum = 110162;
        else
        {
            do_licharmor(ch,"");
            return;
    ...
    if (ch->practice < 150)
    {
        send_to_char("It costs 150 points of primal to create a piece of lich armor.\n\r",ch);
        return;
    }
...
void do_painwreck(CHAR_DATA *ch, char *argument )
{
    ...
    act("$n channels unholy energies into $N and watches $M writhe in pain.",ch,NULL,victim,TO_NOTVICT);
...
void do_creepingdoom(CHAR_DATA *ch, char *argument)
{
    ...
    for (ich = ch->in_room->people; dummychar != NULL; ich = dummychar )
    {
        ...
        if (IS_SET(ich->act, ACT_QUEST)) continue;
...
void do_summongolem( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->pcdata->followers > 8)
    {
        send_to_char("You cant have more followers.\n\r",ch);
        return;
    }
    ...
    if (!str_cmp(arg1, "fire"))
    {
        ...
        victim->level = ch->spl[RED_MAGIC] / 1.5;
        victim->hit = 75 * ch->spl[RED_MAGIC];
        victim->max_hit = 75 * ch->spl[RED_MAGIC];
        victim->hitroll = 2 * ch->spl[RED_MAGIC];
        victim->damroll = 2 * ch->spl[RED_MAGIC];
    ...
    else if (!str_cmp(arg1, "clay"))
    {
        ...
        victim->level = ch->spl[YELLOW_MAGIC] / 1.5;
        victim->hit = 75 * ch->spl[YELLOW_MAGIC];
        victim->max_hit = 75 * ch->spl[YELLOW_MAGIC];
        victim->hitroll = 2 * ch->spl[YELLOW_MAGIC];
        victim->damroll = 2 * ch->spl[YELLOW_MAGIC];
        char_to_room( victim, ch->in_room );
        add_follower( victim, ch );
        af.type      = skill_lookup ("charm person");
        af.duration  = 666;
    ...
    else if (!str_cmp(arg1, "stone"))
    {
        ...
        victim->level = ch->spl[GREEN_MAGIC] / 1.5;
        victim->hit = 75 * ch->spl[GREEN_MAGIC];
        victim->max_hit = 75 * ch->spl[GREEN_MAGIC];
        victim->hitroll = 2 * ch->spl[GREEN_MAGIC];
        victim->damroll = 2 * ch->spl[GREEN_MAGIC];
        char_to_room( victim, ch->in_room );
        add_follower( victim, ch );
        af.type      = skill_lookup ("charm person");
        af.duration  = 666;
    ...
    else if (!str_cmp(arg1, "iron"))
    {
        ...
        victim->level = ch->spl[BLUE_MAGIC] / 1.5;
        victim->hit = 75 * ch->spl[BLUE_MAGIC];
        victim->max_hit = 75 * ch->spl[BLUE_MAGIC];
        victim->hitroll = 2 * ch->spl[BLUE_MAGIC];
        victim->damroll = 2 * ch->spl[BLUE_MAGIC];
        char_to_room( victim, ch->in_room );
        add_follower( victim, ch );
        af.type      = skill_lookup ("charm person");
        af.duration  = 666;
