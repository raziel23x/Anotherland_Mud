...
void do_reveal(CHAR_DATA *ch, char *argument)
{
    ...
    for ( ich = ch->in_room->people; ich != NULL; ich = ich->next_in_room )
    {
        ...
        if (IS_SET(ich->affected_by, AFF_SHADOWPLANE))REMOVE_BIT(ich->affected_by, AFF_SHADOWPLANE);
...
void do_magearmor( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if      (!str_cmp(arg,"staff"            )) vnum = 110450;
        else if (!str_cmp(arg,"dagger"           )) vnum = 110451;
        else if (!str_cmp(arg,"ring"             )) vnum = 110452;
        else if (!str_cmp(arg,"collar"           )) vnum = 110453;
        else if (!str_cmp(arg,"bracer"           )) vnum = 110462;
        else if (!str_cmp(arg,"robe"             )) vnum = 110454;
        else if (!str_cmp(arg,"cap"              )) vnum = 110455;
        else if (!str_cmp(arg,"leggings"         )) vnum = 110456;
        else if (!str_cmp(arg,"boots"            )) vnum = 110457;
        else if (!str_cmp(arg,"gloves"           )) vnum = 110458;
        else if (!str_cmp(arg,"sleeves"          )) vnum = 110459;
        else if (!str_cmp(arg,"cape"             )) vnum = 110460;
        else if (!str_cmp(arg,"belt"             )) vnum = 110461;
        else if (!str_cmp(arg,"mask"            )) vnum = 110463;
        else
        {
            do_magearmor(ch,"");
            return;
...
void do_chaosmagic (CHAR_DATA *ch, char *argument)
{
    ...
    else if (random == 2) sn = skill_lookup("imp heal");
    else if (random == 3) sn = skill_lookup("imp fireball");
    else if (random == 4) sn = skill_lookup("imp faerie fire");
    else if (random == 5) sn = skill_lookup("imp teleport");
    else if (random == 6) sn = skill_lookup("shield");
    else if (random == 7) sn = skill_lookup("readaura");
    else if (random == 8) sn = skill_lookup("earthquake");
    else if (random == 9) sn = skill_lookup("gate");
    else if (random == 10) sn = skill_lookup("dispel magic");
    else if (random >= 11) sn = skill_lookup("desanct");
    else sn = 0;
...
void do_chant (CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg1, "heal"))
    {
        ...
        if (ch->mana < 7500)
        {
            send_to_char("You need 7500 mana.\n\r",ch);
            return;
        }
        ...
        WAIT_STATE(ch,8);
        ch->mana -= 7500;
        victim->hit = UMIN(victim->hit + (magic_power * 2.0), victim->max_hit);
        return;
    }
    else if (!str_cmp(arg1, "damage"))
    {
        ...
        if (ch->mana < 15000)
        {
            send_to_char("You need 15000 mana.\n\r",ch);
            return;
        }
        if (victim == ch)
        {
        ...
        WAIT_STATE(ch,number_range(15,19));
        ch->mana -= 15000;
        dam = number_range(red_magic*45, red_magic*65);
        /* dam += char_damroll(ch); */
        dam = cap_dam(ch,victim,dam);
        if (!IS_NPC(victim))
        {
            //      if (IS_CLASS(ch, CLASS_MAGE) && dam > 800) dam = number_range(780,820);
            //      if (dam > 1200) dam = number_range(1150,1250);
        }
        ...
        dam = number_range(blue_magic*35, blue_magic*55);
        /*  dam += char_damroll(ch);  */
        dam = cap_dam(ch,victim,dam);
        if (!IS_NPC(victim))
        {
        ...
        hurt_person(ch, victim, dam);
        if (victim->position == POS_DEAD || ch->in_room != victim->in_room) return;
        dam = number_range(green_magic*45, green_magic*65);
        /*  dam += char_damroll(ch); */
        dam = cap_dam(ch,victim,dam);
        if (!IS_NPC(victim))
        {
        ...
        hurt_person(ch, victim, dam);
        if (victim->position == POS_DEAD || ch->in_room != victim->in_room) return;
        dam = number_range(purple_magic*45, purple_magic*65);
        /* dam += char_damroll(ch);  */
        dam = cap_dam(ch,victim,dam);
        if (!IS_NPC(victim))
        {
        ...
        hurt_person(ch, victim, dam);
        if (victim->position == POS_DEAD || ch->in_room != victim->in_room) return;
        dam = number_range(yellow_magic*45, yellow_magic*65);
        /*   dam += char_damroll(ch);  */
        dam = cap_dam(ch,victim,dam);
        if (!IS_NPC(victim))
        {
...
void do_objectgate (CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(obj->quest, QUEST_ARTIFACT) || IS_SET(obj->quest, QUEST_RELIC) || IS_SET(obj->quest, QUEST_ANLA ) )
    {
    ...
    {
        send_to_char("You cannot grab Anotherland eq.\n\r",ch);
        return;
    }
    if ( IS_SET(obj->extra_flags, ITEM_NOLOCATE) )
    {
        send_to_char("So close yet so far away.\n\r",ch);
        return;
    }
