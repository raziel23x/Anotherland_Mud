...
int find_dam( CHAR_DATA *ch, CHAR_DATA *victim, int dam, bool foot )
{
    ...
    {
        if (ch->chi[CURRENT] > 0 && ch->chi[CURRENT] < 3) dam *= 1.1;
    ...
    if ( !IS_NPC(ch) ) dam = (dam * ((ch->wpn[0]+1) / 100));
    if ( !IS_NPC(ch) )
    {
        stance = ch->stance[0];
    ...
    if (!IS_NPC(victim) && dam > 5000) dam = number_range( 4000, 6000 );
    if (dam <= 0) dam = 2;
    return dam;
}
...
void do_palmstrike( CHAR_DATA *ch, char *argument )
{
    ...
    WAIT_STATE(ch, number_range(12,24));
    dam = number_range(1,1000);
...
void do_sweep( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_COMB(ch, COMB_REV1) && IS_COMB(ch, COMB_KNEE))
    {
        ...
        WAIT_STATE(victim, number_range(20,25));
        ch->monkcrap = 0;
        return;
    }
    act("You crouch down and sweep $N's legs out from under $M.",ch,NULL,victim,TO_CHAR);
...
void do_thrustkick( CHAR_DATA *ch, char *argument )
{
    ...
    dam = find_dam( ch, victim, dam, foot ) * 0.8;
...
void do_spinkick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_COMB(ch, COMB_THRUST1) && IS_COMB(ch, COMB_THRUST2) )
    {
        ...
        else if ( ch->chi[CURRENT] > 4 )
        {
            dam /= 1.5;
            damage( ch, victim, dam, gsn_lightningkick );
        }
        else if ( ch->chi[CURRENT] > 3 )
        {
            dam /= 1.4;
            damage( ch, victim, dam, gsn_lightningkick );
        }
        else if ( ch->chi[CURRENT] > 2 )
        {
            dam /= 1.3;
            damage( ch, victim, dam, gsn_lightningkick );
        }
        else if ( ch->chi[CURRENT] > 1 )
        {
            dam /= 1.2;
            damage( ch, victim, dam, gsn_lightningkick );
        }
