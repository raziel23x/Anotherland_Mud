...
bool CanEdit(CHAR_DATA *ch, AREA_DATA *pArea);
...
void redit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_BUILDER( ch, pArea ) && ch->level < 13 )
    {
        interpret( ch, arg );
        return;
    }
...
void oedit( CHAR_DATA *ch, char *argument )
{
    ...
    */
    ...
    if ( ( value = flag_value( extra2_flags, arg ) ) != NO_FLAG )
    {
    ...
    if ( ( value = flag_value( weap_flags, arg ) ) != NO_FLAG )
    {
        TOGGLE_BIT(pObj->weapflags, value);

        SET_BIT( pArea->area_flags, AREA_CHANGED );
        send_to_char( "Weapon affect flag toggled.\n\r", ch);
        return;
    }
...
bool CanEdit(CHAR_DATA *ch, AREA_DATA *pArea)
{
    if ( ch->trust == 13 || ch->level == 13 ) return TRUE;
    if ( IS_BUILDER(ch,pArea) ) return TRUE;
    if ( ch->pcdata->security < pArea->security ) return FALSE;
    if ( ch->pcdata->security == pArea->security ) return TRUE;
    if ( ch->pcdata->security == 0 ) return FALSE;
    return TRUE;
}
...
void do_aedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !CanEdit(ch,pArea)) return stcf(ch,"You can't modify this area.\n\r");
...
void do_redit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !CanEdit(ch,pRoom->area)) return stcf(ch,"You can't modify this area.\n\r");
    ...
    if ( command[0] == 'c' && !str_prefix( command, "create" ) )
    {
        if ( !IS_BUILDER(ch, pRoom->area) && ch->pcdata->security < 7 )
        {
            stc("Cannot modify this area/room/mob/object.\n\r",ch);
            return;
        }
...
void do_oedit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' && command[0] == '\0') return;
    if (!is_number(argument) && !is_number(command))
    {
        stc("A vnum.\n\r",ch);
        return;
    }
    ...
    if ( is_number( command ) )
    {
        ...
        if ( !CanEdit(ch,pObj->area)) return stcf(ch,"You can't modify this area.\n\r");
...
void do_medit( CHAR_DATA *ch, char *argument )
{
    ...
    if ( is_number( command ) )
    {
        ...
        if ( !CanEdit(ch,pMob->area)) return stcf(ch,"You can't modify this area.\n\r");
            ...
            || (!str_cmp( arg2, "obj" ) && is_number( arg3 )) )
        {
            ...
            if ( !str_cmp( arg2, "mob" ) )
            {
                ...
                pReset->arg1    = atol( arg3 );
                                                            /* Max # */
                pReset->arg2    = is_number( arg4 ) ? atol( arg4 ) : 1;
