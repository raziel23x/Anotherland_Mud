...
{
    {   "apply",    apply_flags,     "Object affects."      },
    ...
    {   "weapflags",    weap_flags,  "Weapon affects."      },
    {   "quest",    quest_flags,     "Quest attributes."        },
...
void show_obj_values( CHAR_DATA *ch, OBJ_INDEX_DATA *obj )
{
    ...
    switch( obj->item_type )
    {
        ...
        case ITEM_SOCKET:
            sprintf( buf,
                "#G[#yv0#G]#R to hit:   #7[#C%d#7]#n\n\r"
                "#G[#yv1#G]#R to dam:   #7[#C%d#7]#n\n\r",
                obj->value[0],
                obj->value[1] );
            send_to_char( buf,ch );
            break;
        ...
        case ITEM_MONEY:
            ...
            break;
        case ITEM_FURNITURE:
            sprintf( buf, "[v0] Use:      [%d]\n\r(1 = sit, 2 = stand)\n\r", obj->value[0] );
            send_to_char( buf, ch );
            sprintf( buf, "[v1] Capacity: [%d]\n\r", obj->value[1] );
            send_to_char( buf, ch );
            break;
        case ITEM_FLAKE:
            sprintf( buf, "[v0] Status:   [%d]\n\r", obj->value[0] );
            send_to_char( buf, ch );
            break;
    }
    return;
        ...
        case ITEM_QUESTCARD:
            switch( value_num )
            {
                default:
                    return FALSE;
                case 0:
                    send_to_char( "V0 Quest Item Set\n\r\n\r", ch);
                    pObj->value[0] = atoi( argument );
                    break;
                case 1:
                    send_to_char( "V1 Quest Item Set\n\r\n\r", ch);
                    pObj->value[1] = atoi( argument );
                    break;
                case 2:
                    send_to_char( "V2 Quest Item Set\n\r\n\r", ch);
                    pObj->value[2] = atoi( argument );
                    break;
                case 3:
                    send_to_char( "V3 Quest Item Set\n\r\n\r", ch);
                    pObj->value[3] = atoi( argument );
                    break;
                    /*                case 4:
                                        send_to_char( " Quest Item Level\n\r\n\r", ch);
...
bool oedit_show( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf, "Extra2 flags:[%s]\n\r",
        flag_string( extra2_flags, pObj->extra_flags2 ) );
    send_to_char( buf, ch );

    sprintf( buf, "Weapon flags:[%s]\n\r",
        flag_string( weap_flags,pObj->weapflags ) );
    send_to_char( buf, ch );
