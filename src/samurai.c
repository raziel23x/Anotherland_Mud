...
/*  Joshua's math is complicated and fucking annoying, so I rewrote it - Beorn
...
//  SET_BIT(obj->quest, QUEST_RELIC);
 ...
 //obj->value = 696969;
joshqval = ch->pcdata->quest;
ch->pcdata->quest = 999999;
 obj->condition = 696969;
  obj->toughness = 100;
  obj->resistance = 1;
  obj->points = 0;
  joshval=(ch->wpn[0]/70)*(1+(ch->pcdata->upgrade_level*.6));
  joshval+=(ch->stance[7]/70)*(1+(ch->pcdata->upgrade_level*.3));
//for(joshval=joshval;joshval>50;joshval-=50){
//if(joshval >=50)
//oset_affect(ch,obj,50,APPLY_DAMROLL,FALSE);};
oset_affect(ch,obj,joshval,APPLY_DAMROLL,TRUE);
ch->pcdata->quest += obj->points;
//ch->quest += obj->points;
  obj->points = 0;
  joshval=(ch->wpn[1]/70)*(1+(ch->pcdata->upgrade_level*.6));
  joshval+=(ch->stance[6]/70)*(1+(ch->pcdata->upgrade_level*.3));
//for(joshval=joshval;joshval>50joshval-=50){
//if(joshval >=50)
//oset_affect(ch,obj,50,APPLY_HITROLL,TRUE);};
oset_affect(ch,obj,joshval,APPLY_HITROLL,TRUE);
ch->pcdata->quest += obj->points;

//ch->quest += obj->points;
  obj->points = 0;
  joshval=(ch->wpn[2]/3)*ch->pcdata->upgrade_level+1;
  joshval+=(ch->stance[4]/3)*(ch->pcdata->upgrade_level+1);
//for(joshval=joshval;joshval>250;joshval-=250){
//if(joshval >32000)joshval=32000;
//oset_affect(ch,obj,250,APPLY_HIT,FALSE);};
oset_affect(ch,obj,joshval,APPLY_HIT,TRUE);
ch->pcdata->quest += obj->points;
//ch->quest += obj->points;
  obj->points = 0;
  joshval=(ch->wpn[3]/45)*(1+(ch->pcdata->upgrade_level*.3));
  joshval-=(ch->stance[4]/45)*(1+(ch->pcdata->upgrade_level*.3)) ;
joshval-=joshval+(joshval/2);
//for(joshval=joshval;joshval<-50;joshval+=50){
//if(joshval <=-50)
//oset_affect(ch,obj,-50,APPLY_AC,FALSE);};
oset_affect(ch,obj,joshval,APPLY_AC,TRUE);
ch->pcdata->quest += obj->points;
//ch->quest += obj->points;
obj->points = 0;
joshval = (((ch->wpn[7] + ch->stance[3])/70)*(2+(ch->pcdata->upgrade_level*.4))/4);
  obj->value[1] = joshval;
joshval=(((ch->wpn[5] + ch->stance[2])/40)*(2+(ch->pcdata->upgrade_level*.4))/4);
  obj->value[2] = joshval;
  obj->condition=100;
  ch->pcdata->quest = joshqval;
//    obj->questowner = str_dup(ch->pcdata->switchname);
  obj_to_char(obj, ch);
...
// end of joshua's do_katana
*/

void do_katana( CHAR_DATA *ch, char *argument )
// My new and cleaned up version -Beorn
{
    int sambonus;
    int samcost;
    OBJ_DATA *obj;
    ...
    samcost = ch->pcdata->quest;
    ch->pcdata->quest = 999999;
    obj->condition = 696969;
    obj->toughness = 100;
    obj->resistance = 1;
    obj->points = 0;
    sambonus = (ch->wpn[0]/70)*(1+(ch->pcdata->upgrade_level)/2);
    sambonus +=(ch->stance[7]/70)*(1+(ch->pcdata->upgrade_level)/2);
    oset_affect(ch,obj,sambonus,APPLY_DAMROLL,TRUE);
    ch->pcdata->quest += obj->points;
    obj->points = 0;
    sambonus = (ch->wpn[1]/70)*(1+(ch->pcdata->upgrade_level)/2);
    sambonus += (ch->stance[6]/70)*(1+(ch->pcdata->upgrade_level)/2);
    oset_affect(ch,obj,sambonus,APPLY_HITROLL,TRUE);
    ch->pcdata->quest += obj->points;
    obj->points = 0;
    sambonus=(ch->wpn[2]/3)*ch->pcdata->upgrade_level+1;

    sambonus+=(ch->stance[4]/2)*(ch->pcdata->upgrade_level+1);

    if (sambonus >= 25000)
        sambonus = 25000;

    oset_affect(ch,obj,sambonus,APPLY_HIT,TRUE);
    ch->pcdata->quest += obj->points;

    obj->points = 0;

    sambonus = (ch->wpn[3]/45)*(1+(ch->pcdata->upgrade_level)/2);
    sambonus += (ch->stance[4]/30)*(1+(ch->pcdata->upgrade_level)/2);
    //sambonus += sambonus + (sambonus/2);
    sambonus = sambonus * -1;
    oset_affect(ch,obj,sambonus,APPLY_AC,TRUE);
    ch->pcdata->quest += obj->points;
    obj->points = 0;
    sambonus = (((ch->wpn[7] + ch->stance[3])/100)*(2+(ch->pcdata->upgrade_level))/4);
    obj->value[1] = sambonus;
    sambonus=(((ch->wpn[5] + ch->stance[2])/60)*(2+(ch->pcdata->upgrade_level))/4);
    obj->value[2] = sambonus;
    obj->condition=100;
    ch->pcdata->quest = samcost;
    obj_to_char(obj, ch);
    act("A katana flashes into existance.",ch,NULL,NULL,TO_CHAR);
    act("A katana suddenly apear in $n's hands.",ch,NULL,NULL,TO_ROOM);
    return;
...
void do_slide(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->pcdata->powers[SAMURAI_FOCUS] > 50)
    {
...
void check_samuraiattack(CHAR_DATA *ch, CHAR_DATA *victim)
{
    ...
    switch(ch->pcdata->powers[SAMURAI_FOCUS])
    {
        ...
        case 35 :
            ...
            break;
        case 40:
            if (ch->pcdata->powers[SAMURAI_LAST] == 40)
            {
                send_to_char("You cannot use the same combo twice.\n\r",ch);
                break;
            }
            ch->pcdata->powers[SAMURAI_LAST] = 40;
            do_say(ch, "ISOGI!!!!!!!!");
            one_hit(ch, victim, gsn_supreme, 1);
            one_hit(ch, victim, gsn_supreme, 1);
            one_hit(ch, victim, gsn_supreme, 1);
            one_hit(ch, victim, gsn_supreme, 1);
            one_hit(ch, victim, gsn_elbow, 1);
            one_hit(ch, victim, gsn_knee, 1);
            act("#rYou feel exhausted after your exertion.#n",ch,NULL,victim,TO_CHAR);
            act("#r$n looks exhausted after their exertion.#n",ch,NULL,victim,TO_VICT);
            ch->move -= 50000;
            //      REMOVE_BIT(ch->newbits, SPEED);
            break;
    }
    return;
}
