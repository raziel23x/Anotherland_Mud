...
void do_stomp( CHAR_DATA *ch, char *argument )
{
    ...
    cdamage = number_range(5000,7000);
    hurt_person(ch,victim,cdamage);
    limb = number_range(3,4);
    if (number_range(0,100) <= 25 && !IS_NPC(victim))
    {
        if (limb > 3)
        {
...
void do_breath( CHAR_DATA *ch, char *argument )
{
    ...
    if ((ch->generation) == 0) (*skill_table[sn].spell_fun) (sn,level,ch,victim);
    WAIT_STATE(ch, 18);
    return;
}
...
void do_shapearmor( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if      (!str_cmp(arg,"knife"           )) vnum = 110350;
        else if (!str_cmp(arg,"ring"            )) vnum = 110351;
        else if (!str_cmp(arg,"necklace"        )) vnum = 110352;
        else if (!str_cmp(arg,"bands"           )) vnum = 110353;
        else if (!str_cmp(arg,"jacket"          )) vnum = 110354;
        else if (!str_cmp(arg,"helmet"          )) vnum = 110355;
        else if (!str_cmp(arg,"pants"           )) vnum = 110356;
        else if (!str_cmp(arg,"boots"           )) vnum = 110357;
        else if (!str_cmp(arg,"gloves"          )) vnum = 110358;
        else if (!str_cmp(arg,"shirt"           )) vnum = 110359;
        else if (!str_cmp(arg,"cloak"           )) vnum = 110360;
        else if (!str_cmp(arg,"belt"            )) vnum = 110361;
        else if (!str_cmp(arg,"visor"           )) vnum = 110362;
        else if (!str_cmp(arg,"kane"            )) vnum = 110363;
        else
        {
            do_shapearmor(ch,"");
            return;
...
void do_shift (CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg,"tiger"))
    {
        ...
        SET_BIT(ch->affected_by, AFF_POLYMORPH);
        ch->damroll += (10 - ch->generation)*500;
        ch->hitroll += (10 - ch->generation)*500;
        ch->armor   -= (10 - ch->generation)*500;
    ...
    else if (!str_cmp(arg,"bull"))
    {
        ...
        ch->pcdata->powers[SHAPE_COUNTER]+=10;
        SET_BIT(ch->affected_by, AFF_POLYMORPH);
        ch->damroll += (10 - ch->generation)*750;
        ch->hitroll += (10 - ch->generation)*750;
        ch->armor   -= (10 - ch->generation)*100;
        if (ch->pcdata->powers[SHAPE_COUNTER] < 15)
        {
            send_to_char("#CYour wounds mend and close.\n\r#n",ch);
            ch->hit += UMIN(5000,ch->max_hit*0.1);
    ...
    else if (!str_cmp(arg,"faerie"))
    {
        ...
        ch->pcdata->powers[SHAPE_COUNTER]+=10;
        SET_BIT(ch->affected_by, AFF_POLYMORPH);
        ch->damroll += (10 - ch->generation)*100;
        ch->hitroll += (10 - ch->generation)*100;
        ch->armor   -= (10 - ch->generation)*1000;
        if (ch->pcdata->powers[SHAPE_COUNTER] < 15)
        {
            send_to_char("#CYour wounds mend and close.\n\r#n",ch);
            ch->hit += UMIN(5000,ch->max_hit*0.1);
...
void do_charge(CHAR_DATA *ch, char *argument)
{
    ...
    one_hit(ch,victim,gsn_hooves,1);
    if (victim->position == POS_FIGHTING && number_range(1,3)==1)
    {
