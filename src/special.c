...
DECLARE_SPEC_FUN(   spec_darkzap        );
DECLARE_SPEC_FUN(   spec_sorceror       );
DECLARE_SPEC_FUN(   spec_healer     );
DECLARE_SPEC_FUN(   spec_slotmachine    );
DECLARE_SPEC_FUN(   spec_parasite       );
DECLARE_SPEC_FUN(   spec_parasiteinfected   );
DECLARE_SPEC_FUN(   spec_wall_force     );

void casino_protection(CHAR_DATA * ch);
void slotmachine(CHAR_DATA * ch, OBJ_DATA * pObj);
void sorc_spell_say(CHAR_DATA * ch);
void healer_spell_say(CHAR_DATA * ch, CHAR_DATA * pVictim);
bool kingdom_guard(CHAR_DATA * ch, int iKingNum);
    ...
    for ( victim = ch->in_room->people; victim != NULL; victim = v_next )
    {
        ...
        if (victim->hit < -5)
        {
            ...
            if ( rnd_chat <= 5)
                ...
                sprintf( buf, "Hay Idiot! Your mother is calling you.\n\r");
            else if ( rnd_chat <= 13)
            ...
            if (victim->level <=3)
            {
                do_info(ch,"When killed by a player, its honor. When it's not, its dishonor.\n\r");
                victim->level = 2;
            }
            victim->mdeath++;
            return TRUE;
    ...
    for ( victim = ch->in_room->people; victim != NULL; victim = v_next )
    {
        ...
        if (victim->hit < -5)
        {
            ...
            if ( rnd_chat <= 1)
                ...
                sprintf(buf,  "Hay fuck head guess what? I'm going to kill you!!!\n\r");
            else if ( rnd_chat <= 7)
            ...
            behead(victim);
            victim->level = 3;
            sprintf(buf,"%s has been executed by Seth's Executioner",victim->pcdata->switchname);
            do_info(ch,buf);
            victim->mdeath++;
            return TRUE;
        }
...
bool spec_darkzap(CHAR_DATA * ch)
{
    CHAR_DATA * victim;
    ...
    {
        victim = ch->fighting;
        act("$n zaps you with dark energy!", ch, NULL, victim, TO_VICT);
        act("$n zaps $N with dark energy!", ch, NULL, victim, TO_NOTVICT);
        hurt_person(ch, victim, (number_range(1,3) * ch->level));
    }

    return TRUE;
}

/*Spec Sorceror by Magna*/
bool spec_sorceror(CHAR_DATA * ch)
{
    int sn;
    ...
    if(ch->fighting == NULL)
    {
        ...
        if(number_range(1, 16) == 1)
        {
            ...
            {
                sorc_spell_say(ch);
                spell_sanctuary(sn, ch->level / 4, ch, ch);
            }
        }
        ...
        if(number_range(1, 12) == 1)
        {
            ...
            {
                sorc_spell_say(ch);
                spell_pass_door(sn, ch->level / 4, ch, ch);
            }
        }
        ...
        if(number_range(1, 12) == 1)
        {
            ...
            {
                sorc_spell_say(ch);
                spell_protection(sn, ch->level / 4, ch, ch);
            }
        }
        ...
        if(number_range(1, 12) == 1)
        {
            ...
            {
                sorc_spell_say(ch);
                spell_stone_skin(sn, ch->level / 4, ch, ch);
            }
        }

    }

    /*Combat Neutral Stuff*/
    /*Ensure Health*/

    /*Curse*/
    sn = gsn_curse;

    if(is_affected(ch, sn) && (number_range(1, 8) == 1))
    {
        sorc_spell_say(ch);
        sn = skill_lookup("remove curse");
        spell_remove_curse(sn, ch->level / 4, ch, ch);
    }
    ...
    if(is_affected(ch, sn) && (number_range(1, 8) == 1))
    {
        ...
        spell_cure_blindness(sn, ch->level / 4, ch, ch);
    }
    ...
    if(is_affected(ch, sn) && (number_range(1, 8) == 1))
    {
        ...
        spell_cure_poison(sn, ch->level / 4, ch, ch);
    }
    ...
    if(ch->fighting != NULL)
    {
        ...
        {
            do_say(ch, "Reveal yourself fool!.");
            sorc_spell_say(ch);
            sn = skill_lookup("faerie fog");
            spell_faerie_fog(sn, ch->level / 4, ch, ch->fighting);
        ...
        else
        {
            switch (number_range(1,13))
            {
                case 1:
                case 2:
                    do_say(ch, "May the earth swallow you whole!");
                    sorc_spell_say(ch);
                    sn = skill_lookup("earthquake");
                    spell_earthquake(sn, ch->level / 4, ch, NULL);
                    break;
                case 3:
                case 4:
                    do_say(ch, "Your shield shall not protect you from me!");
                    sorc_spell_say(ch);
                    sn = skill_lookup("sanctuary");

                    if(is_affected(ch->fighting, sn))
                    {
                        sn = skill_lookup("desanct");
                        sorc_spell_say(ch);
                        /*We get 3 tries at this, since we're uber good!*/
                        spell_desanct(sn, ch->level / 4, ch, ch->fighting);
                        spell_desanct(sn, ch->level / 4, ch, ch->fighting);
                        spell_desanct(sn, ch->level / 4, ch, ch->fighting);
                        break;
                    }
                case 5:
                    do_say(ch, "May the mark of darkness be upon you!");
                    sorc_spell_say(ch);
                    sn = skill_lookup("curse");
                    spell_curse(sn, ch->level / 4, ch, ch->fighting);
                    break;
                case 6:
                case 7:
                case 8:
                case 9:
                    ...
                    sorc_spell_say(ch);
                    sn = skill_lookup("web");
                    spell_web(sn, ch->level / 4, ch, ch->fighting);
                    break;
                case 10:
                case 11:
                    do_say(ch, "You will not escape.");
                    sorc_spell_say(ch);
                    sn = skill_lookup("jailwater");
                    spell_jailwater(sn, ch->level / 4, ch, ch->fighting);
                    break;
                case 12:
                    do_say(ch, "Destros te foci karos magna!");
                    sorc_spell_say(ch);
                    sn = skill_lookup("focused manasplice");
                    spell_focused_manasplice(sn, ch->level / 100, ch, ch->fighting);
                    break;
                case 13:
                    do_say(ch, "I tire of this.");
                    sorc_spell_say(ch);
                    do_say(ch, "Destros ro malfesio karos magna!");
                    sorc_spell_say(ch);
                    sn = skill_lookup("manasplice");
                    spell_manasplice(sn, ch->level / 100, ch, ch->fighting);
                    break;
                default:
                    break;
            }
...
void sorc_spell_say(CHAR_DATA * ch)
{
    ...
    {
        case 1:
            act("$n makes a subtle gesture.", ch, NULL, NULL, TO_ROOM);
            break;
        case 2:
            act("$n draws a rune in the air with $s fingertip, which shimmers slightly before fading.", ch, NULL, NULL, TO_ROOM);
            break;
        case 3:
            act("A pulsating aura forms briefly around $n as $e mutters an arcane phrase.", ch, NULL, NULL, TO_ROOM);
            break;
        case 4:
            act("You feel a mana shift in the area.", ch, NULL, NULL, TO_ROOM);
            break;
        case 5:
        default:
            break;
...
bool spec_guard_kingdom1(CHAR_DATA * ch)
{
    return kingdom_guard(ch, 1);
}

bool spec_guard_kingdom2(CHAR_DATA * ch)
{
    return kingdom_guard(ch, 2);
}

bool spec_guard_kingdom3(CHAR_DATA * ch)
{
    return kingdom_guard(ch, 3);
}

bool spec_guard_kingdom4(CHAR_DATA * ch)
{
    return kingdom_guard(ch, 4);
}

bool spec_guard_kingdom5(CHAR_DATA * ch)
{
    return kingdom_guard(ch, 5);
}

bool spec_guard_kingdom6(CHAR_DATA * ch)
{
    return kingdom_guard(ch, 6);
}
...
bool kingdom_guard(CHAR_DATA * ch, int iKingNum)
{
    ...
    if (ch->fighting != NULL)
    {
        ...
        if(!IS_NPC(victim) && ((get_age(victim) - 17) < 5))
        {
            do_say(ch, "Begone young one, I have no time for your shenanigans.");
            act("$n banishes $N from the kingdom.\n\r", ch, NULL, victim, TO_NOTVICT);
            act("$n banishes you from $s kingdom.\n\r", ch, NULL, victim, TO_VICT);
            act("You banish $N from your kingdom.\n\r", ch, NULL, victim, TO_CHAR);
            char_from_room(victim);
            char_to_room(victim, get_room_index(ROOM_VNUM_ALTAR));
        }

        if(!can_see(ch, victim) || IS_AFFECTED(victim, AFF_ETHEREAL) || IS_AFFECTED(victim, AFF_SHADOWPLANE))
        {
            do_say(ch, "I sense your presence, oh unseen one!.");
            spell_faerie_fog(skill_lookup("faerie fog"), ch->level / 4, ch, ch->fighting);
        }

        for(i = (ch->level / 1000) + 1; i > 0; i--)
        {
            multi_hit(ch, victim, gsn_punch);
        }
    ...
    for (d = descriptor_list; d != NULL; d = d->next)
    {
        if (!IS_PLAYING(d)
            ...
            || (victim->pcdata->chobj != NULL)
            || (victim->in_room->area != ch->in_room->area)
            || victim->pcdata->kingdom == iKingNum)
        {
            continue;
        }
            ...
            ch->position == POS_STANDING)
        {
            ...
            char_from_room( victim );
            char_to_room(victim, get_room_index(ROOM_VNUM_ALTAR));
            sprintf(buf, "%s has been banished from the kingdom of %s.", victim->name, kingdom_table[iKingNum].whoname);
            do_yell(ch, buf);
            continue;
        }

        if((get_age(victim) - 17) < 5)
        {
            do_say(ch, "Begone young one, I have no time for your shenanigans.");
            act("$n banishes $N from the kingdom.\n\r", ch, NULL, victim, TO_NOTVICT);
            act("$n banishes you from $s kingdom.\n\r", ch, NULL, victim, TO_VICT);
        ...
        sprintf(buf, "No one invades %s!", kingdom_table[iKingNum].name);
        do_say(ch, buf);
        sprintf(buf, "%s is invading the kingdom of %s!", victim->name, kingdom_table[iKingNum].whoname);
        do_yell(ch, buf);
...
bool spec_healer(CHAR_DATA * ch)
{
    ...
    for(pVictim = ch->in_room->people; pVictim != NULL; pVictim = pVictim->next_in_room)
    {
        /*Curse*/
        sn = gsn_curse;

        if(is_affected(pVictim, sn) && (number_range(1, 4) == 1))
        {
            healer_spell_say(ch, pVictim);
            sn = skill_lookup("remove curse");
            spell_remove_curse(sn, ch->level / 4, ch, pVictim);
        }
        ...
        if(is_affected(pVictim, sn) && (number_range(1, 4) == 1))
        {
            healer_spell_say(ch, pVictim);
            sn = skill_lookup("cure blindness");
            spell_cure_blindness(sn, ch->level / 4, ch, pVictim);
        }
        ...
        if(is_affected(pVictim, sn) && (number_range(1, 4) == 1))
        {
            healer_spell_say(ch, pVictim);
            sn = skill_lookup("cure poison");
            spell_cure_poison(sn, ch->level / 4, ch, pVictim);
        }

        /*Healing*/
        if((pVictim->hit < pVictim->max_hit) && (number_range(1, 2) == 1) && (pVictim->fight_timer <= 0) && (pVictim->fighting == NULL))
        {
            healer_spell_say(ch, pVictim);
            act("$n's wounds are magically healed.", pVictim, NULL, NULL, TO_ROOM);
            act("Your wounds are magically healed.", pVictim, NULL, NULL, TO_CHAR);
            werewolf_regen(pVictim, ch->level / 4);
            regen_limb(pVictim);
        }
        ...
        if(number_range(1, 8) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_sanctuary(sn, ch->level / 4, ch, pVictim);
            }
        }
        ...
        if(number_range(1, 4) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_fly(sn, ch->level / 4, ch, pVictim);
            }
        }
        ...
        if(number_range(1, 4) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_pass_door(sn, ch->level / 4, ch, pVictim);
            }
        }
        ...
        if(number_range(1, 6) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_protection(sn, ch->level / 4, ch, pVictim);
            }
        }
        ...
        if(number_range(1, 6) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_stone_skin(sn, ch->level / 4, ch, pVictim);
            }
        }
        ...
        if(number_range(1, 6) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_armor(sn, ch->level / 4, ch, pVictim);
            }
        }
        ...
        if(number_range(1, 6) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_bless(sn, ch->level / 4, ch, pVictim);
            }
        }
        ...
        if(number_range(1, 6) == 1)
        {
            ...
            if(!is_affected(pVictim, sn))
            {
                spell_darkblessing(sn, ch->level / 4, ch, pVictim);
            }
        }
...
#define SLOT_PIE       1
#define SLOT_PUPPY     2
#define SLOT_CHRONO    3
#define SLOT_MAGNA     4
#define SLOT_ALESYIRA  5
#define SLOT_MARISELLA 6
#define SLOT_SETH      7

#define NUM_SLOT_TYPES 7
#define SLOT_TABLE_SIZE 50

#define SLOT_CHECK(x) ((iaSlots[0] == x) || (iaSlots[1] == x) || (iaSlots[2] == x))
#define SLOT_COUNT(x) (((iaSlots[0] == x) ? 1 : 0) + ((iaSlots[1] == x) ? 1 : 0) + ((iaSlots[2] == x) ? 1 : 0))

char * slot_names[NUM_SLOT_TYPES] =
{ "   Pie    ", "  Puppy   ", "  Chrono  ", "  Magna   ", " Alesyira ", "Marisella ", "   Seth   " };

int slot_table[SLOT_TABLE_SIZE] =
{
    SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE, SLOT_PIE,
    SLOT_PIE, SLOT_PIE, SLOT_PIE,
    SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY, SLOT_PUPPY,
    SLOT_CHRONO, SLOT_CHRONO, SLOT_CHRONO, SLOT_CHRONO, SLOT_CHRONO, SLOT_CHRONO,
    SLOT_MAGNA, SLOT_MAGNA, SLOT_MAGNA, SLOT_MAGNA, SLOT_MAGNA, SLOT_MAGNA,
    SLOT_ALESYIRA, SLOT_ALESYIRA, SLOT_ALESYIRA, SLOT_ALESYIRA, SLOT_ALESYIRA,
    SLOT_MARISELLA, SLOT_MARISELLA, SLOT_MARISELLA, SLOT_MARISELLA,
    SLOT_SETH, SLOT_SETH, SLOT_SETH, SLOT_SETH
};
...
void casino_protection(CHAR_DATA * ch)
{
    ...
    for(pVictim = ch->in_room->people; pVictim != NULL; pVictim = pVictimNext)
    {
        pVictimNext = pVictim->next_in_room;

        if(pVictim->fighting == ch)
        {
            act("Several casino guards arrive and beat $n severely!\n\r", pVictim, NULL, NULL, TO_ROOM);
            act("Several casino guards arrive and beat you severely!\n\r", pVictim, NULL, NULL, TO_CHAR);

            stop_fighting(pVictim, TRUE);

            act("The guards proceed to carry $n away.\n\r", pVictim, NULL, NULL, TO_ROOM);
            act("The guards proceed to carry you away...\n\r", pVictim, NULL, NULL, TO_CHAR);
        }
    }
...
bool spec_slotmachine(CHAR_DATA * ch)
{
    OBJ_DATA * pObj;
    OBJ_DATA * pObjNext;

    casino_protection(ch);
    ...
    for(pObj = ch->carrying; pObj != NULL; pObj = pObjNext)
    {
        ...
        {
            slotmachine(ch, pObj);
            if(get_item_count(ch) > 0)
            {
                act("The $n spits out it's excess tokens.", ch, NULL, NULL, TO_ROOM);
                do_drop(ch, "all");
            }
            return TRUE;
        }
...
void slotmachine(CHAR_DATA * ch, OBJ_DATA * pObj)
{
    ...
    CHAR_DATA * pVictim;
    char buf[MAX_STRING_LENGTH];
    int iTokenValue = pObj->value[0];
    bool bWin = FALSE;
    bool bFound = FALSE;
    int nIndex = 0;
    int iaSlots[3];
    int iRewardMultiplier = 0;

    if(pObj->questowner == NULL)
    {
        return;
    }
    ...
    if(iTokenValue > 1000)
    {
        sprintf(buf, "Sorry %s, the betting limit is 1000 QPs!", pObj->questowner);
        return;
    }

    sprintf(buf, "The $n accepts %s's token and begins to spin...", pObj->questowner);
    act(buf, ch, NULL, NULL, TO_ROOM);

    for(nIndex = 0; nIndex < 3; nIndex++)
    {
        iaSlots[nIndex] = slot_table[number_range(0, SLOT_TABLE_SIZE - 1)];
    }

    /*First, check for a 3 of a kind*/
    if((iaSlots[0] == iaSlots[1]) && (iaSlots[0] == iaSlots[2]))
    {
        bWin = TRUE;
        switch(iaSlots[0])
        {
            ...
            case SLOT_PUPPY:
                ...
                break;
            case SLOT_CHRONO:
                iRewardMultiplier = 15;
                break;
            case SLOT_MAGNA:
                iRewardMultiplier = 20;
                break;
            case SLOT_ALESYIRA:
                iRewardMultiplier = 30;
                break;
            case SLOT_MARISELLA:
                iRewardMultiplier = 40;
                break;
            case SLOT_SETH:
                iRewardMultiplier = 50;
                break;
        }
    }
    ...
    /*Combos*/
    /*Coder Combo*/
    if(SLOT_CHECK(SLOT_MAGNA) && SLOT_CHECK(SLOT_SETH) && SLOT_CHECK(SLOT_CHRONO))
    {
        bWin = TRUE;
        act("#RCoder Combo!#n", ch, NULL, NULL, TO_ROOM);
        iRewardMultiplier = 10;
    }

    /*Foxy Ladies Combo*/
    if(SLOT_CHECK(SLOT_MARISELLA) && SLOT_CHECK(SLOT_ALESYIRA) && SLOT_CHECK(SLOT_PUPPY))
    {
        bWin = TRUE;
        act("#RFoxy Ladies Combo!#n", ch, NULL, NULL, TO_ROOM);
        iRewardMultiplier = 5;
    }

    /*Awesome Threesome Combo*/
    if(SLOT_CHECK(SLOT_MARISELLA) && SLOT_CHECK(SLOT_SETH) && SLOT_CHECK(SLOT_ALESYIRA))
    {
        bWin = TRUE;
        act("#RAwesome Threesome Combo!#n", ch, NULL, NULL, TO_ROOM);
        iRewardMultiplier = 5;
    }

    /*Pie is the Limit Combo*/
    if(SLOT_CHECK(SLOT_MAGNA) && SLOT_CHECK(SLOT_PIE) && SLOT_CHECK(SLOT_MARISELLA))
    {
        bWin = TRUE;
        act("#RPie is the Limit Combo!#n", ch, NULL, NULL, TO_ROOM);
        iRewardMultiplier = 5;
    }

    /*Puppy Torture Combo!*/
    if(SLOT_CHECK(SLOT_MAGNA) && SLOT_CHECK(SLOT_ALESYIRA) && SLOT_CHECK(SLOT_PUPPY))
    {
        bWin = TRUE;
        act("#RPuppy Torture Combo!#n", ch, NULL, NULL, TO_ROOM);
        iRewardMultiplier = 5;
    }

    /*Super Friends Combo*/
    if(SLOT_CHECK(SLOT_SETH) && SLOT_CHECK(SLOT_ALESYIRA) && SLOT_CHECK(SLOT_MAGNA))
    {
        bWin = TRUE;
        act("#RSuper Friends Combo!#n", ch, NULL, NULL, TO_ROOM);
        iRewardMultiplier = 5;
    }

    if(SLOT_COUNT(SLOT_PIE) == 2)
    {
        bWin = TRUE;
        iRewardMultiplier = 2;
    }
    ...
    if(bWin == TRUE)
    {
        act("#7----------------------------------#n", ch, NULL, NULL, TO_ROOM);
        sprintf(buf, "#7|#a#P%s#n#7|#a#P%s#n#7|#a#P%s#n#7|#n", slot_names[iaSlots[0] - 1], slot_names[iaSlots[1] - 1], slot_names[iaSlots[2] - 1]);
        act(buf, ch, NULL, NULL, TO_ROOM);
        act("#7----------------------------------#n", ch, NULL, NULL, TO_ROOM);

        iTokenValue *= iRewardMultiplier;
        free_string(pObj->name);
        pObj->name = str_dup("casino token");
        free_string(pObj->short_descr);
        sprintf(buf, "A %d QP Casino Token", iTokenValue);
        pObj->short_descr = str_dup(buf);
        free_string(pObj->description);
        pObj->description = str_dup("A casino token is lying here.");
        pObj->value[0] = iTokenValue;

        sprintf(buf, "%s wins %d QPs!\n\r", pObj->questowner, pObj->value[0]);
        act(buf, ch, NULL, NULL, TO_ROOM);

        for (d = descriptor_list; d != NULL; d = d->next)
        {
            if (!IS_PLAYING(d)
                || (pVictim = d->character) == NULL
                || (strcmp(pVictim->name, pObj->questowner))
                || IS_NPC(pVictim)
                || pVictim->in_room == NULL
                || pVictim->pcdata->chobj != NULL)
            {
                continue;
            }
            else
            {
                act("The $n spits a token into $N's hands.\n\r", ch, NULL, pVictim, TO_ROOM);
                obj_from_char(pObj);
                obj_to_char(pObj, pVictim);
                bFound = TRUE;
                break;
            }
        }
        ...
        if(bFound == FALSE)
        {
            sprintf(buf, "Unable to find %s, the $n retains the token.  If this is yours, please CALL TOKEN.\n\r", pObj->questowner);
            act(buf, ch, NULL, NULL, TO_ROOM);
        }
    }
    else
    {
        act("#7----------------------------------#n", ch, NULL, NULL, TO_ROOM);
        sprintf(buf, "#7|#P%s#7|#P%s#7|#P%s#7|#n", slot_names[iaSlots[0] - 1], slot_names[iaSlots[1] - 1], slot_names[iaSlots[2] - 1]);
        act(buf, ch, NULL, NULL, TO_ROOM);
        act("#7----------------------------------#n", ch, NULL, NULL, TO_ROOM);

        act("The $n flashes \"#RLOSER#n\" across it's screen.\n\r", ch, NULL, NULL, TO_ROOM);
        obj_from_char(pObj);
        extract_obj(pObj);
    }
...
bool spec_parasite(CHAR_DATA * ch)
{
    CHAR_DATA * pVictim;
    int i;

    /*First, attempt to infect anyone the parasite is fighting*/
    pVictim = ch->fighting;

    if(pVictim != NULL)
    {
        /*Parasites don't fight each other!*/
        if(IS_NPC(pVictim) && ((pVictim->spec_fun == spec_parasite) || (pVictim->spec_fun == spec_parasiteinfected)))
        {
            stop_fighting(ch, TRUE);
        }
        /*Attempt to infect the character.*/
        else if(IS_NPC(pVictim) && (pVictim->spec_fun == 0))
        {
            /*Hard to infect during a fight.*/
            if(number_percent() > 75)
            {
                act("$n lunges at $N and latches on to $S body, before burrowing quickly into $S flesh.", ch, NULL, pVictim, TO_ROOM);
                pVictim->spec_fun = spec_parasiteinfected;
                pVictim->max_mana = 0;
                extract_char(ch, TRUE);
                return FALSE;
            }
        ...
        {
            /*Un-infectable, try to flee.*/
            do_flee(ch, "");
        }
    }

    /*Attempt to infect a random thing in the room.  Try up to 5 times till successful.*/
    for(i = 0; i < 5; i++)
    {
        pVictim = RandomInRoom(ch->in_room, ch);

        if(IS_NPC(pVictim) && (pVictim->spec_fun == 0))
        {
            act("$n lunges at $N and latches on to $S body, before burrowing quickly into $S flesh.", ch, NULL, pVictim, TO_ROOM);
            pVictim->spec_fun = spec_parasiteinfected;
            pVictim->max_mana = 0;
...
bool spec_parasiteinfected(CHAR_DATA * ch)
{
    MOB_INDEX_DATA *pMobIndex;
    /*Max mana is used as a counter*/
    ch->max_mana += 1;

    /*Pump out a new parasite every 20 seconds*/
    if((ch->max_mana % 5) == 0)
    {
        pMobIndex = get_mob_index(MOB_VNUM_PARASITE);

        if(pMobIndex == NULL)
        {
            log_string("spec_parasiteinfected: No parasite mobile");
        }

        char_to_room(create_mobile( pMobIndex ), ch->in_room);
        act("A parasitic worm erupts from $n's flesh and falls to the ground.", ch, NULL, NULL, TO_ROOM);
    }

    /*Character dies after 5 minutes(15 parasites)*/
    if(ch->max_mana >= 75)
    {
        act("$n's skin begins to bubble, and $e falls to the ground while dissolving into a pile of putrid flesh.", ch, NULL, NULL, TO_ROOM);
        extract_char(ch, TRUE);
        return FALSE;
    }
...
const   struct  spec_type   spec_table  [ ] =
{
    ...
    { "spec_darkzap",       spec_darkzap },
    { "spec_sorceror",      spec_sorceror },
    { "spec_guard_kingdom1",    spec_guard_kingdom1 },
    { "spec_guard_kingdom2",    spec_guard_kingdom2 },
    { "spec_guard_kingdom3",    spec_guard_kingdom3 },
    { "spec_guard_kingdom4",    spec_guard_kingdom4 },
    { "spec_guard_kingdom5",    spec_guard_kingdom5 },
    { "spec_guard_kingdom6",    spec_guard_kingdom6 },
    { "spec_healer",        spec_healer     },
    { "spec_slotmachine",       spec_slotmachine    },
    { "spec_parasite",      spec_parasite       },
    { "spec_parasiteinfected",  spec_parasiteinfected   },
