...
void do_implant( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' && arg2[0] == '\0')
    {
        sprintf(buf,"Current implant levels: #pFace: %d, Legs: %d, Body: %d.#n\n\r",
            UMIN(ch->pcdata->powers[CYBORG_FACE],6),
    ...
    if (arg2[0] == '\0')
    {
        if (!str_cmp(arg1,"face"))
        {
            ...
            if ((ch->pcdata->powers[CYBORG_FACE] == 6))
                send_to_char("You have laserbeams attached to your eyes.(auto).\n\r",ch);
            return;
        }
    ...
    if (!str_cmp(arg2,"improve"))
    {
        ...
        if (!str_cmp(arg1,"face" )) {implant = CYBORG_FACE; max = 6;}
...
void do_dridereq( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if      (!str_cmp(arg,"whip"           )) vnum = 110300;
        else if (!str_cmp(arg,"ring"                )) vnum = 110301;
        else if (!str_cmp(arg,"collar"              )) vnum = 110302;
        else if (!str_cmp(arg,"bracer"              )) vnum = 110303;
        else if (!str_cmp(arg,"armor"               )) vnum = 110304;
        else if (!str_cmp(arg,"helmet"              )) vnum = 110305;
        else if (!str_cmp(arg,"leggings"            )) vnum = 110306;
        else if (!str_cmp(arg,"boots"               )) vnum = 110307;
        else if (!str_cmp(arg,"gloves"              )) vnum = 110308;
        else if (!str_cmp(arg,"sleeves"             )) vnum = 110309;
        else if (!str_cmp(arg,"cloak"               )) vnum = 110310;
        else if (!str_cmp(arg,"belt"                )) vnum = 110311;
        else if (!str_cmp(arg,"mask"               )) vnum = 110312;
        else
        {
            do_dridereq(ch,"");
            return;
