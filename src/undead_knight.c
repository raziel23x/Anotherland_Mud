...
void do_knightarmor( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if      (!str_cmp(arg,"longsword"           )) vnum = 110250;
        else if (!str_cmp(arg,"ring"                )) vnum = 110251;
        else if (!str_cmp(arg,"collar"              )) vnum = 110252;
        else if (!str_cmp(arg,"bracer"              )) vnum = 110253;
        else if (!str_cmp(arg,"plate"               )) vnum = 110254;
        else if (!str_cmp(arg,"helmet"              )) vnum = 110255;
        else if (!str_cmp(arg,"leggings"            )) vnum = 110256;
        else if (!str_cmp(arg,"boots"               )) vnum = 110257;
        else if (!str_cmp(arg,"gauntlets"           )) vnum = 110258;
        else if (!str_cmp(arg,"sleeves"             )) vnum = 110259;
        else if (!str_cmp(arg,"cloak"               )) vnum = 110260;
        else if (!str_cmp(arg,"belt"                )) vnum = 110261;
        else if (!str_cmp(arg,"visor"               )) vnum = 110262;
        else if (!str_cmp(arg,"shortsword"          )) vnum = 110263;

        else
        {
            send_to_char("Please specify which piece of unholy armor you wish to make: plate ring bracer collar helmet leggings boots gauntlets cloak belt visor longsword shortsword.\n\r",ch);
...
void do_unholyrite (CHAR_DATA *ch, char *argument)
{
    ...
    {
        ch->hit += number_range(15000,2000);
...
void do_weaponpractice (CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->pcdata->powers[WEAPONSKILL] > 15)
    {
...
void do_powerword (CHAR_DATA *ch, char *argument)
{
    ...
    if (!str_cmp(arg,"stun") && ch->pcdata->powers[INVOCATION] > 4)
    {
        if (!(arg2[0] == '\0') || ch->fighting != NULL)
        {
            ...
            if (IS_NPC(victim) && IS_SET(victim->act, ACT_DUMMY))
            {
                send_to_char( "It'a a DUMMY, it can't die!\n\r",ch );
                return;
            }
    ...
    else if (!str_cmp(arg,"blind") && ch->pcdata->powers[INVOCATION] > 0)
    {
        if (!(arg2[0] == '\0') || ch->fighting != NULL)
        {
            ...
            af.modifier  = -600*(ch->pcdata->upgrade_level + 1);
            af.duration  = 60;
    ...
    else if (!str_cmp(arg,"kill") && ch->pcdata->powers[INVOCATION] > 2)
    {
        if (!(arg2[0] == '\0'))
        {
            ...
            if (IS_NPC(victim) && IS_SET(victim->act, ACT_DUMMY))
            {
                send_to_char( "You can't kill a Dummy...\n\r", ch );
                return;
            }
            if (ch->pcdata->powers[POWER_TICK] > 0)
            {
            ...
            if (!IS_NPC(victim) || victim->level > 100)
            {
                ...
                if (IS_NPC(victim) && dam > 50000) dam = 50000;
                if (!IS_NPC(victim) && dam >((ch->pcdata->upgrade_level+1)*9000)) dam = ((ch->pcdata->upgrade_level + 1) * 9000);
                if (!IS_NPC(victim) && dam <= 10000) dam = 10000;
                if (IS_AFFECTED(victim, AFF_PROTECT)) dam *= 0.75;
