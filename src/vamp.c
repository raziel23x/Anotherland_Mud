...
void do_vampirearmor( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if      (!str_cmp(arg,"longsword"           )) vnum = 110550;
        else if (!str_cmp(arg,"ring"                )) vnum = 110551;
        else if (!str_cmp(arg,"collar"              )) vnum = 110552;
        else if (!str_cmp(arg,"bracer"              )) vnum = 110553;
        else if (!str_cmp(arg,"plate"               )) vnum = 110554;
        else if (!str_cmp(arg,"helmet"              )) vnum = 110555;
        else if (!str_cmp(arg,"leggings"            )) vnum = 110556;
        else if (!str_cmp(arg,"boots"               )) vnum = 110557;
        else if (!str_cmp(arg,"gloves"              )) vnum = 110558;
        else if (!str_cmp(arg,"sleeves"             )) vnum = 110559;
        else if (!str_cmp(arg,"cape"                )) vnum = 110560;
        else if (!str_cmp(arg,"belt"                )) vnum = 110561;
        else if (!str_cmp(arg,"visor"               )) vnum = 110562;
        else if (!str_cmp(arg,"dagger"              )) vnum = 110563;
        else
        {
            do_vampirearmor(ch,"");
            return;
...
void do_spew( CHAR_DATA *ch, char *argument )
{
    ...
    level = ch->power[DISC_VAMP_THAU]*1200;;
...
void do_dragonform( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_EXTRA(ch, EXTRA_DRAGON))
    {
        ...
        ch->damroll = ch->damroll - 500;
        ch->hitroll = ch->hitroll - 500;
...
void do_obj( CHAR_DATA *ch, char *argument )
{
    ...
    if (obji->item_type == ITEM_GEMSTONE ||
        ...
        obji->item_type == ITEM_XPTOKEN ||
        obji->item_type == ITEM_PRTOKEN ||
        obji->item_type == ITEM_STORETOKEN ||
        obji->item_type == ITEM_TITANIUM ||
        obji->item_type == ITEM_DIAMOND ||
        obji->item_type == ITEM_EXCAVIER ||
...
void do_zombie(CHAR_DATA *ch, char *argument )
{
    ...
    victim->max_hit = getMight(ch)*10;
    victim->hit = getMight(ch)*5;
...
void do_lamprey( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(victim))
    {
        dam = ch->power[DISC_VAMP_OBTE] * 30;
    }
...
void do_assassinate(CHAR_DATA *ch, char *argument)
{
    ...
    if (ch->generation == 5) dam *= 1.01;
    if (ch->generation == 4) dam *= 1.02;
    if (ch->generation == 3) dam *= 1.03;
    if (ch->generation == 2) dam *= 1.04;
    if (ch->generation == 1) dam *= 1.05;
    if (ch->generation == 0) dam *= 1.06;
    if (ch->pcdata->rank == AGE_TRUEBLOOD) dam *= 1.05;
    if (victim->position == POS_SLEEPING) dam *= 1.1;
    if (IS_ITEMAFF(victim, ITEMA_RESISTANCE)) dam *= 0.9;
    if (IS_AFFECTED(victim, AFF_SANCTUARY)) dam *= 0.75;
...
void do_tongue( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(victim))
    {
        dam = ((ch->pcdata->condition[COND_THIRST] / ch->power[DISC_VAMP_SERP])*2.5);
    }
    else if (IS_NPC(victim))
    {
        dam = ((ch->pcdata->condition[COND_THIRST] / ch->power[DISC_VAMP_SERP])*2.5);
...
void do_spit( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(victim))
    {
        dam = ((ch->pcdata->condition[COND_THIRST] / ch->power[DISC_VAMP_QUIE]));
    }
    else if (IS_NPC(victim))
    {
        dam = ((ch->pcdata->condition[COND_THIRST] / ch->power[DISC_VAMP_QUIE]));
    ...
    if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_WEREWOLF) )
    {
        if (victim->power[DISC_WERE_BOAR] > 2 ) dam /= 1;
    }
...
void do_summon(CHAR_DATA *ch, char *argument)
{
    ...
    {
        send_to_char( "Stupid, a Dummy can't move, idiot.\n\r",ch);
        return;
    }

    if ( !IS_SET(ch->in_room->room_flags, ROOM_NOTRANS))
    {
        send_to_char( " Sorry, you can't seem to locate your spell book.\n\r",ch );
        return;
    }
...
void do_drain( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(victim))
    {
        ...
        if (dam > 500) dam = number_range( 900, 1100 );
        ch->hit = (ch->hit + dam);
