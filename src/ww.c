...
void do_klaive( CHAR_DATA *ch, char *argument )
{
    ...
    if ( caCommand[0] == '\0' )
    {
        send_to_char("Syntax: klaive <greater/lesser>.\n\r",ch);
        return;
    }

    if (!str_cmp(caCommand, "Lesser"))
    {
        if ( 60 > ch->practice)
        {
    ...
    else if (!str_cmp(caCommand, "Greater"))
    {
        if ( 120 > ch->practice)
        {
            send_to_char("It costs 120 points of primal to create a Greater Klaive.\n\r",ch);
            return;
        }
        ch->practice   -= 120;
...
void do_moonbeam( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_GOOD(victim))
        hurt_person(ch,victim, 12000);
    if (IS_EVIL(victim))
        hurt_person(ch,victim, 20000);
    if (IS_NEUTRAL(victim))
        hurt_person(ch,victim, 9500);
    ch->mana -= 500;
    WAIT_STATE(ch,8);
    return;
...
void do_moonarmour( CHAR_DATA *ch, char *argument )
{
    ...
    {
        if      (!str_cmp(arg,"mask"                )) vnum = 110700;
        else if (!str_cmp(arg,"ring"                )) vnum = 110701;
        else if (!str_cmp(arg,"collar"              )) vnum = 110702;
        else if (!str_cmp(arg,"bracer"              )) vnum = 110703;
        else if (!str_cmp(arg,"plate"               )) vnum = 110704;
        else if (!str_cmp(arg,"helmet"              )) vnum = 110705;
        else if (!str_cmp(arg,"leggings"            )) vnum = 110706;
        else if (!str_cmp(arg,"boots"               )) vnum = 110707;
        else if (!str_cmp(arg,"gloves"              )) vnum = 110708;
        else if (!str_cmp(arg,"sleeves"             )) vnum = 110709;
        else if (!str_cmp(arg,"cape"                )) vnum = 110710;
        else if (!str_cmp(arg,"belt"                )) vnum = 110711;
        else
        {
            do_moonarmour(ch,"");
            return;
...
void do_shred( CHAR_DATA *ch, char *argument )
{
    ...
    one_hit(ch,victim,gsn_shred,2);
    one_hit(ch,victim,gsn_shred,2);
    one_hit(ch,victim,gsn_shred,3);
...
void do_talons( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_NPC(victim)) dam = number_range(5000,6000);
    else dam = number_range(40000,100000);

    if ( !IS_NPC(victim) && IS_CLASS(victim, CLASS_VAMPIRE) )
    {
        if (victim->power[DISC_VAMP_FORT] > 2 ) dam /= 1.5;
    }
    dam = dam*2;                                            //Removed one hurt_person and multiplied dam*2
...
void do_disquiet( CHAR_DATA *ch, char *argument )
{
    ...
    /*  if (arg[0] == '\0')
      {
          send_to_char("Disquiet who?\n\r",ch);
          return;
      }*/

    if ( is_safe( ch, victim) ) return;

    if ( ch->gnosis[GCURRENT] < 4 )
    {
        stc("You need four points of gnosis power to use the gift of Disquiet.\n\r",ch);
        return;
    }

    sn = skill_lookup("disquiet");

    if(is_affected(victim, sn))
    {
        act("$N is already disquieted.", ch, NULL, victim, TO_CHAR);
        return;
    }
    ...
    af.modifier  = -( ch->power[DISC_WERE_OWL] * 50 * (ch->pcdata->upgrade_level + 1));
    af.bitvector = 0;
    ...
    af.modifier  = -( ch->power[DISC_WERE_OWL] * (ch->pcdata->upgrade_level + 1));
    af.bitvector = 0;
    affect_to_char( victim, &af );
    ...
    ch->gnosis[CURRENT] -= 4;
    act("Your eyes glow a sinister #Rred#n as you glare at $N.",ch,NULL,victim,TO_CHAR);
    act("$n's eyes glow a sinister #Rred#n as $e glares at you.\n\rYou feel odd.",ch,NULL,victim,TO_VICT);
    act("$n's eyes glow a sinister #Rred#n as $e glares at $N.",ch,NULL,victim,TO_NOTVICT);
    WAIT_STATE(ch,4);
    return;
...
void do_reshape(CHAR_DATA *ch, char *argument)
{
    ...
    if (IS_SET(obj->quest, QUEST_ARTIFACT) || IS_SET(obj->quest, QUEST_PRIZE) || (obj->item_type == ITEM_GEMSTONE) ||
...
void do_wither( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC( victim ) )
    {
        ...
        if ( !blah )
        {
            ...
            act("$n's eyes glow dark red as $e gazes at $N, and their arm begins withering!",ch,NULL,victim,TO_NOTVICT);
